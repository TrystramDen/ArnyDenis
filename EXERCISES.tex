...
{\Arny Solution needed}
     \medskip\item
{\em Every sequence $(k+1), \ldots, (k+p)$ of $p$ consecutive integers contains a multiple of $p$.}

{\Arny Solution needed}

      \medskip\item 
This problem calls for ``second-order" insights from the sieve.
...
\medskip\item
\ignore{**************
Euler's sieve affords us an alternative proof that there are infinitely many primes.  In Euler's sieve, each prime and all of its multiples are removed from the sieve as soon as their existence is acknowledged.  Our earlier construction of Eratosthenes's version of the sieve now becomes:
...
{\em  For every positive integer $n$:  If you remove {\em any} $n+1$ integers from the set $S = \{ 1, 2, \ldots, 2n\}$, then the set of removed integers contains at least one pair $p$ and $q > p$ such that $p$ divides $q$. }
...
\ignore{*********
Let $
\alpha_i$ be the elements of this set of $n+1$ elements.
...
Now, since set $S$ contains the first $2n$ integers, it contains $n$ {\em odd} integers.  Since
we remove $n+1$ elements from $S$, the Pigeonhole Principle assures us that some two of the removed integers ``share" the same odd number $m$.  That is, some removed integer has the form $2^{k_1} \times m$ while another has the form $2^{k_2} \times m$.
...
{\em Hint}.
How can you employ the injections used to prove Proposition~\ref{thm:|NxN|=|N|} to help with this problem?
\end{itemize}
...
\item 
{\bf To prove}.
{\em The conventional algorithm computes the product of two $n$-bit integers uses $\Theta(n^2)$ multiplications.}
...
\medskip\item
{\bf To prove}.
{\em Karatsuba's algorithm computes the product of two $n$-bit integers uses {\em asymptotically fewer than} $\Theta(n^2)$ multiplications.}
