...
Because there are $2n$ handshakers (the pigeons) and $2n-1$ hands to shake (the boxes), some two shakers must shake the same numbers of hands.
...
A {\it bicolored necklace} is composed of $2n = 2(a+b)$ jewels: $2a$ black jewels and $2b$ white jewels.  Let us be given such a necklace adorned with a solid tube of the sort depicted in Fig.~\ref{fig:sample-necklace}, which contains precisely half of the jewels, i.e., $n$ jewels.
...
Slide the tube around the necklace, one jewel per step, and count both the black jewels and the white jewels at each step.  How can these numbers change in a single step?

\smallskip

Fix a position of the tube in which there are some number, call it $c$, of black jewels inside the tube.  By definition, then, there are $n-c$ white jewels inside the tube.

\smallskip

The effect of a single shift is to insert a new jewel into the tube, at its advancing extremity, and to eliminate an old jewel from the tube, at its training extremity.  Now, if the new and old jewels are of the same color, then the internal and external tallies of black and white jewels is unchanged by this move; if they are of different colors---one is black and the other is white---then the tallies change.  If, for instance, the new jewel is black while the old one is white, then the number of whites in the tube decreases by $1$, and the number of blacks in the tube increases by $1$.  The phrase ``by $1$" is the key to the argument.
...
There are $n$ ways to choose the first element of the permutation.
Once it has been chosen, it remains $n-1$ ways to choose the second and so on.
\medskip
\item
...
There are $n(n-1)$ ways to choose two different numbers among $n$. 
\end{itemize}

{\em Strengthen your argument by listing all permutations of  $S' =  \{1,2,3,4,5\}$.}
\smallskip

as there are a lot of permutations, we should be systematic in the enumeration.
For instance, we can fix the first number (among the 5 possibilities), and then, take fix the second smallest one, etc.
\smallskip
...
{\em Prove the following assertion about the described game.}
\begin{itemize}
\item 3.
{\em The asymptotic behavior of $B(n)$ is given by}
