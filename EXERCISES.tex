...
\ignore{***************
We prove in Proposition~EX \ref{thm:sum-finite-geometric-series}(b) that this infinite summation converges to the value ${1 \over 3}$.  
...
$a=1$ and $b=4$.  
By Proposition~EX \ref{thm:sum-of-arithmetic-series},
...
triangles in the manner depicted in Fig.~EX \ref{fig:Delta(n)4}.
\begin{figure}[ht]
\begin{center}
...
\ignore{***************
In Section~EX \ref{sec:summing-geometric-series:techniques}, we used Thales's theorem about similarity in triangles (Theorem~EX \ref{thm:Thm-of-Thales-similarity}) to sum the simple infinite geometric series $\sum_{i=0}^\infty \ b^i$.  It turns out that a modest modification of that evaluation strategy allows us also to evaluate the truncated versions of that series, namely, the summations
...
We have specified two cost models for traveling within our idealized mesh-structured city.  As we travel from site $\langle i,j \rangle$ to site $\langle h,k \rangle$:
  \begin{itemize}
  \item
In the {\em metro} model, we pay a fee every time we encounter a new site along our trip.
  \medskip\item
In the {\em driving} model, we pay a per-kilometer fee of $n^{(1/2)^\ell}$ units every time we traverse a level-$\ell$ edge along our trip 
  \end{itemize}
...
We must begin by characterizing which vertex/site is the most distant from $\langle i,j \rangle$. 
Of course, the answer will depend on where this site resides within its quadrant.

\smallskip

Let us focus, with no loss of generality, to a specific quadrant, say the southeastern one. 
There are two situations to consider, which are illustrated in Fig.~\ref{fig:routingWorstCase}:
 \begin{figure}[hbt]
\begin{center}
       ...
       The origin site $\langle i,j \rangle$ is represented in dark grey and the destination site is on the shaded quadrant.}
  \label{fig:routingWorstCase}
\end{center}
\end{figure}

the shaded region identifies the location of the destination site $\langle h,k \rangle$.
