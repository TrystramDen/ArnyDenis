...
\end{array}
\]
The specifications of the four arc-generating functions, $\underline{F}^{(n)}_0$, $\underline{F}^{(n)}_1$, $\overline{F}^{(n)}_0$, and $\overline{F}^{(n)}_1$, have some interesting lessons encoded within the functions' domains of definition.
\begin{itemize}
\item
The geometric relations ``up" vs.~``down" and ``left" vs.~``right" are elegantly encoded in the pattern of strong ($<$) and weak ($\leq$) relations.
\item
The strong relations in the specifications---which are the origins of the functions' partialness---remove the self loops on vertices 1 and 15 in Fig.~\ref{fig:deBruijn3coding}.  This ensures that the subgraphs of interest are, indeed, trees!
\end{itemize}
