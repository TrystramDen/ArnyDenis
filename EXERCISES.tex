...
Pour chaque triangle de rang $k$, on dénombre $3$ triangles de rang $k-1$ imbriqués (soit, $3 N_{k-1}$).
Deux de ces triangles de rang $k-1$ ont une partie commune, c'est un triangle de rang $k-2$, donc 
il faut l'enlever (ce qui correspond à $ -3 N_{k-2}$).
Par contre, il y a une partie commune aux trois, c'est un triangle de rang $k-3$  (soit, $+ N_{k-3}$). 
Il faut de plus rajouter le grand triangle ($+1$), et quand $k$ est pair,
il y a un triangle supplémentaire de rang $k-2$ qui apparaît inversé au milieu (donc, dans ce cas $+1$).
\smallskip
...
Solving the recurrence leads to the following expression:
