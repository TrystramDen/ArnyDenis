...
{\Denis straightforward, I don't see what this exercice really adds...}
...
*********************
 ...
 {\Denis I don't really understand this one...}
...
{\Denis yes, we have to write these exercices...}
...
\subsection{A variation of geometric summation}
...
referring to Fig~\ref{fig:sumGeo1sur4}: 
The area of the infinite sum represented in dark grey is obviously one third of the total area
...
\noindent \textit{Hint.}
The idea is to use the double counting principle (Fubini).

Write the expression in a developed form using a triangle shape as shown in 
...
Fubini is used by summing up the successive rows.
...
\noindent \textit{The aim.}
Show another way to prove that the sum is infinite.
\medskip

\noindent \textit{The problem}
Recall the definition of Harmonic series:

$H_{n} = \sum_{k=1}^{n} \frac{1}{k}$.

Show that the series diverges (it goes to infinity).
\medskip
...
Use a pictural to prove a nonobvious identity of arithmetic sums
...
enforcing the ability of using geometrical proofs.
\medskip

\noindent \textit{The problem.}

Prove the irrationality of $\sqrt{2}$ using a geometrical proof.
\medskip
...
\noindent \textit{Lesson learned.}
Training for mastering different types of proofs, here Pigeonhole...
...
Show that it is possible to compute the product of two complex numbers using only {\em three}
...
\noindent \textit{The aim:}
We exploit our ability to evaluate geometric summations to
illustrate a somewhat surprising, nontrivial fact.  One can deduce
information about the divisibility of an integer $n$ from $n$'s
positional numerals.  
...
\noindent \textit{Lesson learned:}
We hope that this ``fun'' result will inspire
the reader to seek kindred numeral-encoded properties of numbers.
