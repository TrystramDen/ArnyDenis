...
A extended Pascal's triangle modulo the prime $m=5$ is illustrated in Fig.~EX \ref{fig:TriangleFractal}.
The first $m$ levels of the triangle replicate endlessly, with periodic inverted $(m-1)$-level triangles whose entries are all $0$.  (In the figure, the inverted triangle of $0$s is depicted in grey.)  
One observes that the original triangle becomes a fractal-like repetitive structure whose pattern of repetitions is dictated by the parameter $m$.
...
We give below a representation of the next dimension $\d4$.
We let the reader to develop the same analysis as we did for $\d3$.
\begin{figure}[h]
\begin{center}
...
\begin{figure}[h]
\begin{center}
        \includegraphics[scale=0.4]{FiguresGraph/deBruijn4Tree}
        \caption{On of the tree that cover $\d_4$.}
