...
Therefore, one must analyze the distribution(s) that the sequences come from, to determine the probability that two targeted sequences will achieve the observed distance between the two targeted sequences.
\item
...
\noindent \fbox{
\begin{minipage}{0.96\textwidth}
Traditionally, the statement which asserts that the observed sequence and the expected sequence come from the same distribution is called the ``{\em null hypothesis}".

\smallskip

Accepting the targeted sequences as related is {\em accepting the null hypothesis}.
\end{minipage}
}
...
\noindent
We can illustrate the described steps by returning briefly to the Arbuthnot-Laplace question of Section~\ref{sec:Arbuthnot}.  
...
\index{multinomial distributions}  \index{statistical tests!the $\chi^2$ test} 
\begin{itemize}
\item
