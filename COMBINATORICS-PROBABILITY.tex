...
rolls of dice, guesses of various sorts.  ``Why is one specific genre of deal in poker (say, a straight)
worth more than another (say, three of a kind)?"  The arithmetic required for such a discussion is elementary,
and the references to ``gedanken" gambling are easily understood and are of some
interest even to non-gamblers.  One can also introduce in such a setting concepts
...
\begin{itemize}
\item
We are going to focus on games of pure chance---because our underlying interest is in
...
\item
Our definition of the notion of discrete probability, or, likelihood, as a ratio:
\[ 
\frac{\mbox{number of targeted events}}{\mbox{number of possible events}}
\]
has technical consequences:
\begin{itemize}
\item
The probability of any targeted event is $\leq 1$.  (Every targeted event is a possible event.)
\item
The probability of any targeted event is $\geq 0$.  When the probability equals $0$, then
the event is a ``null event", i.e., cannot occur.
\item
The more likely an event is, the closer its probability is to $1$.
\item
Invoking the addition rule for counting sets, the joint probability of two disjoint events equals
the sum of the events' probabilities.
\item
...
same suit and follow one another in value.  Thus, a royal flush is a straight flush whose highest
ranked card is an {\sc Ace}.
...
The multiplication principle thus tells us that there are precisely $13 \times 48 = 624$ {\it four-of-a-kind}
poker hands, so the probability of being dealt such a hand is
\[ \frac{624}{2598960 }  \ \ \approx \ \ 2.4 \time 10^{-4} . \]
...
\item
We finally look at two closely related hands together because their analyses are so similar.
\begin{itemize}
\item
...
and $|\{X, Y, Z\}| =3$.  The final equation is a ``cute", succinct, way of saying that $Y$ and $Z$
