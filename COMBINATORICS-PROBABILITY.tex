...
\[ |A \cup B \cup C| \ \ = \ \ 
\big(|A| + |B| + |C| \big) - \big( |A \cap B| + |A \cap C| + |B \cap C| \big) + 2 |A \cap B \cap C|. \]
...
\begin{tabular}{l}
{\bf Conjecture}.
The value of a pattern in poker tracks the likelihood of a player's \\
receiving the pattern in a random deal.
\end{tabular}
}
...
\subsection{Additional Single-Number ``Summarizatione"}
