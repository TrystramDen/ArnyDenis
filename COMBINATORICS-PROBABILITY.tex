...
We focus on three notions of arrangement within the context of a set of $n$ items.
With no loss of generality, we focus on a set of $n$ abstract objects
\[ \{ x_1, x_2, \ldots , x_n\} \]

\medskip

\noindent {\it Permutations}.
A permutation of an $n$-element set is a {\em fixed ordering} of the elements of the set:  
each element of the set appears precisely once in the ordering.  We denote a permutation
of an $n$-element set via the notation
\[ (x_1,x_2, \ldots , x_n) \]
where each element $x_i$ appears precisely once.  Note that we use parentheses for grouping,
...
\begin{proof}
Just for the practice, let us consider two inductive proofs of this simple result.  We merely
sketch the inductions, leaving details for your practice.

\medskip

\noindent {\bf 1.}
Let us construct a permutation of the $n$-element set $\{ x_1, x_2, \ldots , x_n\}$.
\begin{itemize}
\item
We can select the first element of our permutation in $n$ ways; any $x_i$ will work.
\item
...
\noindent {\bf 2.}
Assume that the proposition is true for $n$-element sets, and consider how to 
construct a permutation $(x_1, \ldots , x_n, x_{n+1})$ of an $(n+1)$-element set,
starting with a permutation $(x_1, \ldots , x_n)$ of an $n$-element set.  We take a new
element $x$.  It can occupy any of $n+1$ positions in the new permutation: it can reside
before any preexisting element, or after all of the preexisting elements, or in between any
adjacent preexisting elements.  Each of these $n+1$ choices creates a unique
permutation of the $(n+1)$-element set $\{x_1, \ldots , x_n, x_{n+1}\}$.
The arithmetic is the same as in case {\bf 1}.  \qed
\end{proof}
