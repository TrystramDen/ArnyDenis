...
Let us abstract the environment in which statistics is practiced by focussing on a
sequence $S$ of $n$ numbers which represents the outcomes of an $n$-trial 
experiment.   Because our real concern
For convenience of analysis, we order the 

\smallskip

Sample sequences $S$: (1) the $n$-element sequence of numbers from the set
$\{1,2,3,4,5,6\}$ which result from $n$ rolls of a $6$-side die; (2) the $n$-element 
sequence of elements from the set $\{\mbox{\sc heads, tails}\}$ which result from $n$
tosses of a $2$-side coin.  
...
\noindent \fbox{
\begin{minipage}{0.95\textwidth}
This might be an opportune time for the reader to review our discussions
of {\it encodings} and of {\it pairing functions}.  Both of those topics involve creating small 
sets of numbers to encode large sets of numbers.  The fact that the small sets must
retain all of the information resident in the large sets guarantees that the encoding numbers
must become large.  You cannot shrink bits!
\end{minipage}
}
...
\subsubsection{The {\it mean} of sampled outcomes}
\label{sec:mean}
...
\noindent
The {\em median} of $S$, denoted {\sc median}($S$), is the middle value in $S$:
\[ 
|\{ y \in S \ | \ y \leq \mbox{\sc median}(S)\}| \ = \
|\{ z \in S \ | \ z \ \geq \mbox{\sc median}(S)\}| 
\]
