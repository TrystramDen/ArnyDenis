...
\subsection{Probability functions: frequencies and distributions}
\label{sec:prob-distributions}

\index{probability!frequency function} 

Let us focus on a random variable $X$ that ranges over a sample space $S$.  Each value
$x \in S$ has an associated probability which we denote $Pr[X=x]$.  The function $f$ on 
$S$ defined as follows for $x \in S$:
\[ f(x) \ = \ Pr[X=x] \]
is called a {\it probability frequency function (for $S$)}.

\smallskip

We have already seen probability frequency function, which we discussed without using the
term, because it had not yet been introduced.  The binomial coefficients, which have arisen
in many of our discussions, provide access to one such example.  We know the following 
about the binomial coefficients:
\begin{itemize}
\item
$\displaystyle {n \choose k}$ is the number of ways of selecting $k$ items from a set of 
$n$ items.  Hence, it is also the number of $k$-elements subsets of a set of $n$ elements.
\item
The binomial coefficients with upper argument $n$ sum to $2^n$:
\[ \sum_{k=0}^n \ {n \choose k} \ = \ 2^n \]
\end{itemize}
These facts combine to afford us a complete analysis of the following simple 
{\sc select-and-replace} game, which is played by a single-player $P$.  The game
involves a set $S$ of $n$ tokens, labeled $1$, $2$, \ldots, $n$.  (The important 
thing is that the labels are distinct, not that they comprise the first $n$ integers.)
The  game has $n$ $3$-stage moves.  In each move, $P$:

(1) {\em removes a token from set $S$};

(2) {\em records the label of the removed token};

(3) {\em replaces the token into set $S$}.

\noindent
At the end of the $n$ moves, we assign player $P$ a score that is the number of 
{\em distinctly labeled} tokens that $P$ had removed (and replaced) during this
play of the game.

Let us denote by $X$ the random variable that on a particular play of the game
assumes the value $k \in \{1, 2, \ldots, n\}$ just when player $P$ achieves score $k$ 
during that play.  Elementary reasoning verifies that for any play,
\[ f(k) \ \eqdef \ Pr[X = k] \ = \ {n \choose k} \cdot 2^{-n}  \]
By definition, then, random variable $X$ admits the thus-defined probability frequency 
function $f$.

We illustrate the probabilities associated with the {\sc select-and-replace} game for 
$n = 10, 15$ in Table~\ref{tab:select-replace}.  

\begin{table}[htb]
\label{tab:select-replace}
\caption{Probabilities associated with the {\sc select-and-replace} game for 
$n = 10, 15$}
\begin{tabular}{|c||r|l||r|l|}
\hline
Value of $k$ & $\displaystyle {10 \choose k}$ & $f_{10}(k)$ 
& $\displaystyle {15 \choose k}$ & $f_{15}(k)$ \\
\hline
\hline
...
In Fig.~\ref{fig:gaussiandistribution}, we illustrate via histograms the values of 
$\displaystyle {n \choose k}$ that are associated with the probabilities in 
Table~\ref{tab:select-replace}: these values are, of course, rows of the 
appropriate instance of Pascal's triangle.

**HERE



The distribution of some rows of the Pascal's triangle is given 
