...
\noindent \fbox{
\begin{minipage}{0.95\textwidth}
Binomial coefficients have made yet anther appearance in our journey!  The many situations
...
provides a powerful argument for the value of abstracting beyond the superficial details of 
...
via counting" is by calculating game-related likelihoods---deals of cards,
rolls of dice, guesses of various sorts.  ``Why is one specific genre of deal in poker
(say, a straight) worth more than another (say, three of a kind)?"  The arithmetic required
for such a discussion is elementary, and the references to ``gedanken" gambling are easily
understood and are of some interest even to non-gamblers.  One can also introduce in such
a setting concepts such as randomness, bias, etc., that are so important in the design of
...
reap the rewards of that material. 
...
Before we begin to play, though, let us establish some expectations.
...
\noindent
We are going to focus on {\em games of pure chance}---because our underlying interest is in
...
\subsection{Illustration: The Game of Poker}
...
$9$ \\
picture cards: &
{\sc Jack}, {\sc Queen}, {\sc King}, {\sc Ace} \ (in increasing value)
\end{tabular} 
\end{tabular} 
...
\frac{624}{2,598,960 }  \ \ \approx \ \ 2.4 \time 10^{-4} . \]
...
\subsection{Illustration: The Monty Hall Puzzle}
...
\index{sample space} \index{random variable}
...
The field of statistics usually encodes results as sets of numbers whose values
range over a suitable {\it sample space}.  A variable that ranges over the frequencies
of outcomes in the sample space is called a {\it random variable}.  Statistics strives to 
capture important elements of the truth exposed via experimentation by analyzing the 
behavior of random variables.  Specifically, each experiment is associated with a
random variable that ranges over the set of possible outcomes of the experiment.  

\smallskip

Let us abstract the environment in which statistics is practiced by focussing on:
\begin{itemize}
\item
a sequence $S = \langle s_1, s_2, \ldots, s_n \rangle$ of $n$ numbers which 
represent the outcomes of an $n$-trial experiment.

\smallskip

Sample sequences $S$: (1) the sequence of numbers from the set $\{1, 2, 3, 4, 5, 6\}$ 
which result from $n$ rolls of a $6$-side die; (2) the noontime temperatures observed in 
Grenoble over a period of $n$ days; (3) the observed diameters of $n$ roller bearings 
after a given run at Factory $X$.  
\item
...
\end{equation}
then for each $i \in \{1, 2, 3, 4\}$ (the set that underlies sequence $S$), $f_S(i) = i$.
  \item
...
\subsubsection{Further measures of ``average"}
\label{sec:median-mode}

There are two sources of discontent with the arithmetic mean as a summarization of a 
sequence $S$. 
\begin{enumerate}
\item
Not only does {\sc mean}($S$) not have to be an element of $S$, it can actually be 
maximally far from any element of $S$: the admittedly degenerate but nonetheless 
legitimate examples $S = \langle 0, 0, \ldots, 0, 1, 1, \ldots, 1 \rangle$, where there are 
equally many $0$s and $1$s, illustrate this point, as all have $\mbox{\sc mean}(S) = 1/2$.

\item
The descriptive value of {\sc mean}($S$) diminishes as the ``skewness" of sequence $S$
increases.  For illustration consider the sequences that share the construction formula of
...
so for $i \in \{1,3,5,7,9,11\}$, $f_S(i) = i-1$
The mean of $\mbox{\sc mean}(S) = 8.1$, as the following calculation indicates. 
\begin{eqnarray*}
\mbox{\sc mean}(S)
 & = & 
{1 \over 31}
...
\frac{251}{31}
\end{eqnarray*}
Not only does $\mbox{\sc mean}(S)$ not reside in sequence $S$, it also ``points"
to a place in $S$ that is very far from the sequence's ``center of gravity".
\end{enumerate}
The second of the preceding complaints can be mitigated, even if not eliminated, by using
the {\it median} of $S$, rather than its mean, to summarize the sequence.
\index{median}

\noindent
The {\em median} of $S$, denoted {\sc median}($S$), determines the middle of 
...
\noindent When $n$ is even:
\begin{itemize}
\item
Some say that there are two medians:
...
\end{itemize}
For our $56$-element sequence $S$, we have $\mbox{\sc median}(S) = 11$



A sequence $S$ can obviously have two medians; in such cases, some sources average
the two values while others just accept the existence of two medians.  One must choose
between two undesirable options: having two distinct medians and having a single median
which does not belong to sequence $S$.
...
\noindent
If there is a single maximum element $x \in S$, then that element $x$ is called the 
{\em mode} of $S$ and is denoted {\sc mode}($S$).
...
\subsubsection{Refined summarizations via higher statistical moments}
\label{sec:mean-plus-moments}

Applied to statistics:

\begin{eqnarray*}
\mbox{\sc mean}(S)             & \eqdef & \bar{x} \ \eqdef  \ {1 \over n} \sum_{x \in S} \ x  \cdot f_S(x) \\
...
\end{eqnarray*} 
 ...
 more numbers: mean is weak because, e.g., $\{0,1\}$ with equiprobable elements has 
 average element $1/2$, even though that element does not occur in the set. SO we add 
 {\em higher moments}.  The first---and by far most popular---candidate is the variance (often
 represented by its square root, the standard deviation, for scaling reasons std is linear in the
 $x_i$ while the variance is quadratic)
 
 variances, skews --- each higher moment compensates for the weaknesses of lower moments


\subsection{Probability distributions}
\label{sec:prob-distributions}

a function: what is the probability that a randomly selected value has a given value; this gives 
us the notion of a {\em distribution}
