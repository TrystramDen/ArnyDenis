...
Let us denote by $X$ the random variable that assumes the value $k \in \{1, 2, \ldots, n\}$
on a particular play of the {\sc select-and-replace} game just when player $P$ achieves 
score $k$ during that play.  Elementary reasoning provides an explicit expression for
the probability $p(k)$ of achieving the score $k$ during a play of the game:
\[ p(k) \ \eqdef \ Pr[X = k] \ = \ {n \choose k} \cdot 2^{-n}  \]
By definition, $p$ is a probability frequency function for random variable $X$.
