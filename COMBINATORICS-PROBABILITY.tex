...
The tools we develop for determining the cardinalities of finite sets give us access to the important (and, often, fun!)~field known as {\em combinatorial probability}. In reality, probability theory is an {\em applied} spin-off of combinatorics.  In its most elementary form, combinatorial probability uses counting to answer myriad important questions about the relative frequencies of occurrences that range from the important---Is one more likely to die while crossing the street or while crossing the ocean?---to those some might call frivolous---Why does the game of ($5$-card) poker value three-of-a-kind more highly than two-pair?  By the end of the section, you will have the wherewithal to answer myriad questions of this ilk.
...
\item
Design methodologies for complex electronic circuits must be aware of the {\em mean times to failure} of the critical components of the circuits.
\item
...
\item
Analyzing and understanding large corpora of data require methodologies that build on the  concepts of {\em clustering} and/or {\em decomposition}.
\end{itemize}
...
calculate the likelihoods of various events' happening, statistics looks as the events ``in the large".  One mechanism used in statistics are parameters---aptly called ``statistics"---that can be viewed as {\em summarizing} large corpora of data.  Such summarizing measures include:
\begin{itemize}
\item
...
\end{itemize}
A second mechanism used in statistics are {\em distributions}.  These can be viewed as families of functions that measure probabilities of kindred events.  Perhaps the most familiar probability distribution in daily life is the {\it normal distribution} that is readily recognized via its {\it Bell-shaped curve}.  The curve tells us visually that the probabilities of the items that are produced by the event of interest are very small when the items are very small or very large---and the probabilities are (roughy) symmetric around the ``average" item.  The items could be, for illustration, the observed diameters of roller bearings produced by a particular manufacturing process: Almost all bearings have diameters that are very close to the target, and very few bearings have diameters that deviate a lot from the target.
...
where $X, Y,  Z \in \{2, \ 3, \ 4, \ 5, \ 6, \ 7, \ 8, \ 9, \ 10, \ \mbox{J}, \ \mbox{Q}, \ \mbox{K}, \ \mbox{A}\}$, 
...
\noindent {\em The median of a sequence.} 
...
\noindent {\em Modes and geometric means.}
...
Empirical reasoning does not convey the certitude that deductive reasoning does:  Empirical truths are established only up to {\em levels of confidence}---which are established based on mathematical analyses.  
...
\noindent \fbox{
\begin{minipage}{0.96\textwidth}
We note that the normal probability frequency function involves two of the fundamental constants of mathematics: $e$ and $\pi$.  The ubiquity of these constants throughout disparate areas in mathematics testifies eloquently to how fully they deserve to be labeled {\em fundamental}.
\end{minipage}
}
...
\item[{\sf Commentary:}]
Note that we do not demand any uniformity among the $X_i$ other than that they all come from the same distribution.  Since these variable represent outcomes of distinct trials from the same experiment, this demand is satisfied automatically.
...
The significance of the {\it Law} is commensurate with that of the Central Limit Theorem!  In fact, the two results are linked by formal reasoning that can be used to prove both results.  As with the Theorem, we state the Law only semi-formally, and without proof, because it too invokes rather advanced mathematical concepts.

\bigskip

\noindent \fbox{
\begin{minipage}{0.96\textwidth}
For those readers who would like to get a sense of the sweeping Law of Large Numbers, we provide the following annotated semi-formal statement.
...
In Section~\ref{sec:stat-tests}, we informally introduce the important topics of {\em statistical tests}.  A main role of such tests is to determine whether observed data---say, e.g., observations of two distinct negative reactions to pharmaceuticals---can be correlated statistically.  In its simplest form, this question could simply ask whether observed values come from similarly distributed random variables.
...
The Central Limit Theorem and the Law of Large Numbers highlight situations in which one does not need to know the details of how a random variable's values are distributed, specifically when one samples those values repeatedly and randomly.  But there are situations where one does need to know whether the values of two (or more) random variables' values come from the same distribution.

**HERE

Advanced probability theory and statistics does not occupy itself with only with means, or even only with normal distributions, or even {\em bi}-nomial distributions.
\index{multinomial distributions} \index{statistical tests}  \index{statistical tests!the $\chi^2$ test} 
\begin{itemize}
\item
{\em multinomial distributions} -- a generalization of the binomial distribution to many variables
\item \index{null hypothesis} 
Statistical tests are basically distance measures---these measures are often computed via expressions that look sort of like the expression [REFERENCE] for the variance---between an ideal (expected) sequence of values and an observed sequence of values.  Each specific test provides the probability that the ideal sequence of values and the observed sequence of values come from the same distribution.

\hspace*{.2in}The traditional name for the statement which asserts that the observed \\
\hspace*{.2in}sequence and the expected sequence come from the same distribution \\
\hspace*{.2in}is the ``{\em null hypothesis}".  

\noindent
The computed probability provides a {\em confidence level} for accepting the null hypothesis.  For insance, if the probability is $95\%$, then one can accept the null hypothesis with a $5\% \ [= (100-95)\%]$ level of confidence.

\item
The $\chi^2$ test measures distance via a {\em relative error}.  Say that there are $m$ trials:  The $m$-element sequence  of {\em observed} values is $\langle o_1, o_2, \ldots, o_m \rangle$.  The  corresponding sequence of {\em expected} values is $\langle e_1, e_2, \ldots, e_m \rangle$.  The $\chi^2$ distance/error corresponding to these sequences is
\[ \chi^2 \ = \ \sum_{i=1}^m \ \frac{(o_i- e_i)^2}{e_i} \]
The $\chi^2$ test invokes an analysis of the multinomial distribution---analogous to our analysis of the {\em bi-nomial} distribution---to calculate the probability that the observed sequence $\langle o_i \rangle_{i=1}^m$ and the {\em expected} sequence $\langle e_i \rangle_{i=1}^m$ come from the same multinomial distribution.

\bigskip

EXAMPLE FROM \cite{Hoel58}:  We role a die $60$ times.  We expect to see $10$ each of the $6$ possible outcomes.  The $\chi^2$ test computes the distance between:
...
\end{array}
\]
The $\chi^2$ test calculates the probability that the observed sequence $\langle f_1, f_2, f_3, f_4, f_5, f_6 \rangle$ comes from the same multinomial distribution as does the expected sequence $\langle 10, 10, 10, 10, 10, 10 \rangle$. 
\end{itemize}

INCLUDE STATISTICAL TESTS -- $\chi^2$, Student's t-test --- check what they test for.
