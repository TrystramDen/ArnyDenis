...
\noindent \fbox{
\begin{minipage}{0.95\textwidth}
Binomial coefficients have made yet anther appearance in our journey!  The many meanings
of this construct are only suggested by their occurrence here, in respect to probabilities, as well
as in Chapter~\ref{ch:Summation} within the context of evaluating summations and in
Chapter~\ref{ch:Recurrences} within the context of solving recurrences.  The multiple occurrences
of binomial coefficients within these apparently unrelated contexts provides a powerful argument
for the value of abstracting beyond the superficial details of phenomena in the direction of
their deep essentials.  This insight will only be reinforced as we proceed with our study of
...
The challenge that Professor $X$ is facing is known as the {\em derangement problems}.
...
the issues one must deal with when studying the broad spectrum of games go beyond the scope of our introductory text.
...
classic \cite{vonNeumann-Morg} by American polymath John von Neumann \index{von Neumann, John}
and German economist Oskar Morgenstern, \index{Morgenstern, Oskar} which established the
...
John Nash, \index{Nash, John} which introduced the now-familiar notion of ``Nash equilibrium."
...
\begin{itemize}
\item
A {\it royal flush}\index{royal flush (in poker)} is a poker hand of the form
...
\item
A {\it straight flush}\index{straight flush (in poker)} is a poker hand in which the cards share the
...
\item
{\it Four-of-a-kind} \index{four of a kind (in poker)}  is a poker hand of the form \\
...
\begin{itemize}
\item
A {\it Full house} \index{full house (in poker)}  is a poker hand of the form \\
...
\item
{\it Three of a kind} \index{three of a kind (in poker)} is a poker hand of the form \\
...
{\em dual} \index{dual problem} form of our problem.  That is:
...
We discuss two ancient dice games: The first game, known as ``{\it craps}''
\index{Dice games!craps} in North America, involves rolling two dice
at a time; the second game, which we call ``{\it sum-of-three}'',
...
on the upward-oriented faces of the rolled dice.  (For example, the sums
...
which the dice reveal their spots is not relevant.  This means that the
...
by enumerating both favorable and possible cases.  This approach can be made to work also for examples composed of repeated occurrences of an event, such as repeated rolls of a die or repeated tosses of
a coin. 
