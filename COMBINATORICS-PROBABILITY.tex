...
about, so we hope only to hint at its goals and methodology.
...
\begin{itemize}
\item
{\em experimentation}, which gives one access to how the real world actually operates.
\item 
{\em summarization} of the observed outcomes of the experiments, in a manner that 
enables to draw inferences from them.
\end{itemize}
The field of statistics usually encodes results as sets of numbers whose values
range over a suitable space, called the {\it sample space}.  A variable that ranges over 
the values in the sample space is called a {\it random variable}.  Statistics strives to 
...
\begin{itemize}
\item
an $n$-element sequence $S$ of numbers which represent the sequence of outcomes
of an experiment, each element belonging to the set of possible outcomes.

\smallskip

Let $S$ be, for instance: (1) the sequence of occurrences of the numbers $\{1,2,3,4,5,6\}$ 
which result from repeated rolls of a $6$-side die; (2) the sequence of occurrences of 
elements from the set $\{\mbox{\sc heads, tails}\}$ which result from repeated tosses of a 
$2$-side coin.  
\item
a {\em frequency function} $f_S$ that exposes how often a possible outcome from the
experiment occurs as an element of  $S$.

Of course, $f_S$ comes automatically with $S$; it is not specified independently!  But,
it does provide a useful ``summary" of $S$---and a useful tool for discussing statistical
measures of $S$. 
\end{itemize}
We introduce the basics of statistics using set $S$ and function $f_S$.
...
The simplest possible framework for summarizing set $S$ and function $f_S$ is via a
single summarizing number.  Several such numbers have been introduced, each being 
valuable within certain domains of inquiry.  The reader should be forewarned that 
summarizing numbers are idealizations in certain ways that we point out during our 
discussion.  We illustrate the following definitions using the sequence
\[ S \ = \  011010011001011 \]  

\index{mode}

\noindent
The {\em mode} of $S$, denoted {\sc mode}($S$), is the element that has maximum frequency:
\[ f_S(\mbox{\sc mode}(S)) \ = \ \max_{x \in S} \ f_S(x) \]

\index{median}

\noindent
The {\em median} of $S$, denoted {\sc median}($S$), is the element $x \in S$ whose 
frequency is the midpoint of all frequencies:
\[ |\{ y \in S \ | \ f_S(y) \ \leq \ f_s(\mbox{\sc median}(S))\}| \ = \
|\{ z \in S \ | \ f_S(z) \ \geq \ f_s(\mbox{\sc median}(S))\}| 
\]

\index{mean}

\noindent
The {\em mean} of $S$, denoted {\sc mean}($S$), is a number---which does not necessarily
belong to $S$---that has average frequency; it is usually denoted $\bar{x}$:
\[ \bar{x} \ = \ {1 \over n} \sum_{x \in S} \ x \cdot f_S(x) \]
The mean of $S$ is also often called its {\it expected value}, denoted {\sc expected value}($S$),
or, for short, its {\it expectation}, denoted {\sc expectation}($S$).

\index{expected value} \index{expectation}
 
\subsection{Summarization by a Fixed-Size Set of Numbers}
