...
\end{itemize}
These facts combine to afford us an analysis of the following simple single-player 
{\sc select-and-replace} game.  The game involves a set $S$ of $n$ tokens, labeled $1$, 
$2$, \ldots, $n$.  (The  important thing is that the labels are distinct, not that they comprise
the first $n$ integers.)  The  game has $n$ $3$-stage moves.  In each move, the player: \\
(1) {\em removes a token from set $S$}; \\
(2) {\em records the label of the removed token}; \\
(3) {\em replaces the token into set $S$}.  \\
At the end of these moves, we tally the number of {\em distinct} tokens that the player had 
removed (and replaced) over the course of the $n$ moves.
**HERE
