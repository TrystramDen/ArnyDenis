...
\index{characteristic vector!of a set} \index{power set of a set}
...
Next, let us create the isosceles triangle $T_1$ of Fig.~\ref{fig:countingTriangles}(center). by taking four copies of $T_0$ and placing them together in the manner depicted in the figure. Now, $T_1$ is similar to $T_0$, in the geometric sense of the word, and its height and base are twice those of $T_0$.  What really interests us, though, is that the {\em drawing} of $T_1$ in the figure contains {\em five} triangles---namely, the four copies of $T_0$ {\em plus} the isosceles triangle that contains these four copies.

As our next step, we iterate the replication process by append to $T_1$ an additional layer consisting of five copies of $T_0$ arranged as indicated in Fig.~\ref{fig:countingTriangles}(right).  The resulting isosceles triangle, $T_2$, has a base and a height that are three times those of $T_0$.  Once again, what really interests us is that the {\em drawing} of $T_2$ in the figure contains {\em thirteen} triangles---namely, the four copies of $T_0$ {\em plus} the isosceles triangle that contains these four copies.  Specifically, we can discern {\em nine} copies of $T_0$, 
{\em three} copies of $T_1$, {\em plus} the all-encompassing big isosceles triangle.

Of course, we can now append to $T_2$ an additional layer consisting of seven copies of $T_0$, and we can then append an additional layer consisting of nine copies of $T_0$, and on and on.

\smallskip

As we make these successive augmentations, how does the progression of numbers of triangles grow.  It is clear that there is a recurrence lurking here.  Your challenge is to discover it.  To this end, let $N_i$ denote the number of triangles within triangle $T_i$.  We have just seen that $N_0 =1$, $N_1 = 5$, and $N_2 = 13$.
  \begin{enumerate}
  \item
{\em Compute} $N_3$.
  \medskip\item
...
\medskip\item
$\oplus$ {\bf The Monge shuffles: a mathematical party trick}

{\sc Lesson:} Experience analyzing combinatorial patterns

\smallskip

\index{Monge, Gaspard} \index{Monge shuffle}

The following card manipulations are associated with the 18-19th century French mathematician Gaspard Monge.  
We employ it as a vehicle for honing your skills in detecting combinatorial patterns.  
Others have used it as a party trick.

\smallskip

We are given a deck of $2n$ distinct cards.  We employ the running example of the following deck, where $n=4$:
\[ (1, \ 2, \ 3, \ 4, \ 5, \ 6, \ 7, \ 8) \]
The {\it Monge shuffle} of the cards operates as follows.  We cut the deck in the middle, to create two $n$-card decks:
\[ (1, \ 2, \ 3, \ 4), \ (5, \ 6, \ 7, \ 8) \]
We then merge the two decks by alternating the ``top" cards from the righthand deck and the lefthand deck; see Fig~\ref{fig:suffleMonge}.
\begin{figure}[h]
\begin{center}
...
The party-game incarnation of the Monge shuffle involves your suggesting to your audience that the cut-then-merge is a good first step in really mixing up the cards.  If this were so, then repeating the step several times should be a good way to obtain a random mixture of the cards.

\smallskip

Let's see what happens after several steps of cut-then-merge:
...
\end{array}
\]
So, this does not look too random!  
After {\em three} (which, ``coincidentally" equals $n-1$) cut-then-merge steps, the deck has been 
reversed, and after $2n-1$ cut-and-merge steps, it has been replicated in its original order!
...
\begin{prop}
Let $p$ be a prime ($p>2$), and let $n = {1 \over 2} (p-1)$.  Let us begin with a deck of $2n$ distinct cards and perform the Monge shuffle on the deck.  After a number of cut-then-merge steps that divides $p-1$, the Monge shuffle replicates the initial deck.
\end{prop}
\smallskip

Let now consider a more sophisticated variant of shuffle.
...
\[ \begin{array}{ccccccccccccccccc}
(1 & 2 & 3 & 4 & 5 & 6 & 7 & 8) & \rightarrow & ( - & - & - & - & 1 & - &  - & - ) \\
( - & 2 & 3 & 4 & 5 & 6 & 7 & 8) & \rightarrow & ( - & - & - & 2 & 1 & - & - & - ) \\
( - & - & 3 & 4 & 5 & 6 & 7 & 8) &  \rightarrow& ( - & - & - & 2 & 1 & 3 & - & - ) \\
( - & - & - & 4 & 5 & 6 & 7 & 8) &  \rightarrow& ( - & - & 4 & 2 & 1 & 3 & - & - ) \\
( - & - & - & - & 5 & 6 & 7 & 8) &  \rightarrow& ( - & - & 4 & 2 & 1 & 3 & 5 & - ) \\
( - & - & - & - & - & 6 & 7 & 8) &  \rightarrow& ( - & 6 & 4 & 2 & 1 & 3 & 5 & - ) \\
( - & - & - & - & - & - & 7 & 8) &  \rightarrow& ( - & 6 & 4 & 2 & 1 & 3 & 5 & 7 ) \\
( - & - & - & - & - & - & - & 8) &  \rightarrow& ( 8 & 6 & 4 & 2 & 1 & 3 & 5 & 7 ) \\
\end{array}
\]
Fig.~\ref{fig:suffleMonge1},~\ref{fig:suffleMonge2},~\ref{fig:suffleMonge3} show picturally the three first steps of the process for $n=4$.

The question here is to prove the following properties. 
...
\begin{figure}[h]
\begin{center}
        \includegraphics[scale=0.4]{FiguresArithmetic/suffleMongeStep1}
        \caption{Step 1. The Monge shuffle for $8$ cards ($n=4$).}
...
\begin{prop}
Let $n$ be an integer.
If $4n+1$ is a prime, then performing $2n$ cut-and-merge steps of the $2n$ Monge's shuffles on a deck of $2n$ distinct cards 
restores the initial deck.
\end{prop}
