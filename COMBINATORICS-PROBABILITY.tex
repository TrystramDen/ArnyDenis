...
In this section, we assume that we know the cardinalities of certain sets---call them $A$, $B$, and $C$---and
we want to know the cardinality of a new set which is formed from these sets by the basic operations of the
algebra of sets, as discussed in Section~\ref{sec:operations-on-sets}.  There are a few commonly invoked counting laws
...
Finite sets whose elements can be put into bijective correspondence have the same cardinality.

\item
{\it The Additive rule} --- also called the {\it Law of inclusion/exclusion}:

The cardinalities of unions of sets can be expressed by additions and subtractions of the cardinalities of the individual sets.

  \begin{itemize}
  \item
When sets $A$ and $B$ are disjoint---i.e., have no common elements---then
\[ |A \cup B| \ \ = \ \ |A| \ +  \ |B| \]

 \item
if the sets {\em intersect}---i.e., have non-null intersection---then
\[ |A \cup B| \ \ = \ \ |A|  \ + \ |B| \ - \ |A \cap B| \]
We {\em include} the elements of both sets, and we {\em exclude} the common elements, which
...
were double-counted ; and finally, we {\em include} the triple intersection, which was double excluded.

\item
{\em The Multiplicative rule}:
...
%\end{figure}
\[ |A \times B| \ \ = \ \ |A| \cdot |B| \]
The multiplicative rule operation extends immediately to multiplicities of sets, for three sets, e.g.:
\[  |A \times B \times C| \ \ = \ \ |A| \cdot |B| \cdot |C| \]

As an important special case of the Multiplicative rule, we note that
\[ |A \times A \times \cdots  \times A| \ \  \mbox{ ($p-1$ times) }    \ \ = \ \ |A|^p \]
The exciting feature here is that the set $A \times A \times \cdots  \times A$ ($p-1$ times) can be viewed as
representing 
the set of length-$p$ strings of elements of $A$.  Thereby, the Multiplicative rule gives us an alternative way to 
