...
\subsubsection{Summing $c$th powers: general $c \geq 0$}
...
for arbitrary positive numbers $c$.  To garner intuition for our
bounds, the reader can readily extrapolate to general $c >0$ the case
$c=2$ that is illustrated in Fig.~\ref{fig:riemann-n2}.  We obtain our
upper bound on $S_c(n)$ by evaluating the integral that yields the
area $\overline{C}_c(n)$ under the lefthand continuous curve ($a$) in
...
\subsubsection{Summing $c$th powers: $c$ a positive integer}
...
\paragraph{\sf B. Refining approximate sums using {\em undetermined
  coefficients}}
...
Section~\ref{sec:Sums-Induction}.
...
powers, for any positive integer $c$,
i.e., for the quantity $S^{(c)}_n$, for any positive integer $k$.
