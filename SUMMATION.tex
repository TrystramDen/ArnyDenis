...
\noindent \fbox{
\begin{minipage}{0.96\textwidth}
Full disclosure: Fig.~\ref{fig:anotherSumOdds} provides the basis for a proof of 
...
What is notable about Fig.~\ref{fig:anotherSumOdds},
even with this caveat, is that it truly facilitates the {\em discovery} of the
Proposition.  It really illustrates well the use of pictorial reasoning in actually {\em doing}
mathematics---even if the facts that such reasoning uncovers require
additional modalities of reasoning for their formal verification.
\end{minipage}
}
...
interest.  Using a strategy inspired by Fubini's work, we obtain a charming nonobvious proof of
Proposition~\ref{thm:squares-odd-integers-Gauss}---as we simultaneously evidence another remarkable property of integers.

\ignore{\Denis This subterfuge will also evidence another amazing property of integers}

\medskip

Let us take the odd integers in order and arrange them into groups whose successive sizes
increase by $1$, as depicted in the following triangular table.
\[
...
\end{array}
\]
Thus: the first group has size $1$ and comprises just the smallest odd number, $1$;
the second group has size $2$ and comprises the next two odd numbers, $3$, $5$; and so on.

%We view the groups as arranged in a triangular array, as depicted in the following table.
%Table~\ref{tab:SumOddsTriangle}.

What we observe is that---at least within the illustrated portion of the triangular table---the $i$ elements
