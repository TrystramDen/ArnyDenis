...
The desired sums $f(n)$ are commonly denoted $\Delta_n$.
\index{$\Delta_n$: sum of the first $n$ integers}
We usually prefer the notation $S_1(n)$ because it exposes the current
goal as an instance of a related family of such expressions and
summations that will occupy through this chapter.

We derive the following (closed-form) expression for $\Delta_n$ in
multiple ways.
