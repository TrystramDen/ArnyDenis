...
\index{Sissa ibn Dahir, legend of}  Sissa, goes the legend, has invented a marvelous game that is played on a chessboard, i.e., an $8 \times 8$ array of
unit-size squares.  An entrepreneur proposes to buy the rights of this marvelous game from Sissa.
The entrepreneur offers
Sissa a one-time payment of {\em one million million (i.e., $10^{12} =
...
\paragraph{A. Summing the first $n$ integers: the case $a=b=1$}
...
The remainder of this subsection is devoted to developing multiple ways
...
Starting with such a picture, we take each row of $2k-1$ tokens and
fold it at its midpoint so that it becomes a reversed letter ``$L$''.
The row of $2k-1$ tokens becomes an ``$L$'' whose horizontal portion
(at the bottom of the reversed ``$L$'') is a row of $k$ tokens and
whose vertical portion (at the right of the reversed ``$L$'') is a
column of $k$ tokens (one token is in common).  See
...
Let us take the odd integers in order and arrange them into groups of successive sizes: the first group, of size $1$, 
...
bigger sizes.  We view the groups as arranged in a triangular array, as depicted in
...
table---the $k$th group, of size $k$, adds up to $k^3$.
...
$k$th group consists of the $k$ consecutive odd numbers beginning with
$2 \Delta_{k-1} +1$ ($\left( \Delta_{k-1} +1 \right)$th largest odd number) and ending with
$2\Delta_k -1$.
...
base of our induction.
\begin{figure}[ht]
\begin{center}
       \includegraphics[scale=0.35]{FiguresMaths/SumCubes1}
\caption{The base for the inductive pictorial proof}
       \label{fig:sumCubes1}
\end{center}
\end{figure}
We next represent the number $8 = 2^3$ (which is the {\em cube} of $2$)

{\Arny How do we know to do this?  What do these two squares have to do with $2^3$?}
...
%
Since consecutive odd numbers differ by $2$, we know that the $k$th
...
Using Fubini's rearrangement stratagem, we now count all the integers in
...
\paragraph{C. A nonobvious identity for arithmetic sums}
...
$\Delta_{2n-1}$ triangle of tokens from four copies of the
$\Delta_n$-triangle plus the extra row of $n$ tokens is not an
...
\caption{8 copies of $\Delta_n$ are filling a square but one token.}
...
\paragraph{A. The extended geometric sum $S^{(1)}_b(n) =
  \sum_{i=1}^n i b^i$}
...
\paragraph{B. The general extended geometric sum $S^{(c)}_b(n) =
  \sum_{i=1}^n i^c b^i$}
...
\paragraph{C. Validating approximate summations via induction} 
...
\paragraph{C. Negative powers $c = -1$: {\em harmonic} summations}
