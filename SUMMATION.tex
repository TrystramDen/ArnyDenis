...
\begin{enumerate}
\item
Fig.~\ref{fig:riemann-n2} illustrates our stratagem as it applies to the first seven terms of the summation $S_2(n) = \sum_{i=1}^n i^2$, namely, $1$, $4$, $9$, $16$, $25$, $36$ and $49$.  The figure represents these terms by means of seven abutting unit-width rectangles whose respective heights are given by the terms.  Summation $S_2(n)$ is, then, represented as the aggregate area of the abutting rectangles.  If we were to extend the figures rightward (which increases $n$ and thereby extends the summation by encompassing more addends), then the next rectangle would have height $64$.
