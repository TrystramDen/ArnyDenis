...
which we discuss briefly in the exercises for this chapter.
\end{minipage}
}
...
Thales of Miletus is credited with the following theorem about similar triangles.  The subject triangles are depicted in the following figure.

\begin{center}
       \includegraphics[scale=0.265]{FiguresArithmetic/ThalesGeometricSum}
\end{center}

\begin{prop}[The Theorem of Thales]
Let us be given a right triangle whose height has length $1$ and whose base has length $S$.   The ratio $S/(S-1)$ is equal to the ratio $1/b$ where $b$ is the heigh of the similar triangle at distance $1$ from its right angle.
\end{prop}
...
\begin{enumerate}
\item
Fig.~\ref{fig:riemann-n2} illustrates our stratagem as it applies to the first seven terms of the summation $S_2(n) = \sum_{i=1}^n i^2$, namely, $1$, $4$, $9$, $16$, $25$, $36$, $49$.  The figure represents these terms by means of seven abutting unit-width rectangles whose respective heights are given by the terms.  Summation $S_2(n)$ is, then, represented as the aggregate area of the abutting rectangles.  If we were to extend the figure rightward (which increases $n$ and thereby extends the summation by encompassing more addends), then the next two rectangle would have respective heights $64$ and $81$.

\smallskip

Our stratagem mandates embellishing the rectangles in Fig.~\ref{fig:riemann-n2} by two continuous curves (labeled (a) and (b) in the figure) that connect the rectangles' upper corners.
\begin{figure}[htb]
\centerline{
\includegraphics[scale=0.3]{FiguresMaths/SumSquaresContinuous}
%\includegraphics[scale=0.3]{FiguresMaths/SumSquaresContinuous1}
}
\caption{The summation $S_2(n) = \sum_{i=1}^n i^2$ represented as the aggregate area of a sequence of unit-width shaded rectangles.  The summation is {\em bounded from above} by the area under the continuous curve (a) that connects the upper lefthand corners of the rectangles.  In detail, curve (a) begins at the upper lefthand corner of the leftmost rectangle; it continues to what would be the upper lefthand corner of the rightmost rectangle.  The area under curve (a) is $\int_0^n \ (x+1)^2 dx$.  The summation is {\em bounded from below} by the area under the continuous curve (b) that connects the upper righthand corners of the rectangles.  In detail, curve (b) begins at the upper righthand corner of the leftmost rectangle; it continues to the upper righthand corner of the rightmost rectangle.  The area under curve (b) is $\int_1^n \ x^2 dx$.}
\label{fig:riemann-n2}
%\label{fig:SumIntegral1}
\end{figure}
Because curve (a) completely ``covers'' the rectangles, the area under the curve is an {\em upper bound} on the aggregate area of the rectangles; this area is
\[ \int_1^n \ x^2 dx \]
Because curve (b) lies completely within the rectangles, the area under  curve (b) is a {\em lower bound} on the aggregate area of the rectangles; this area is
\[ \int_0^{n-1}  \ x^2 dx \]
...
\item
Fig.~\ref{fig:riemann-harmonic} illustrates our stratagem as it applies to the first ten terms of the {\em harmonic} sum \index{harmonic sum}
\[ S^{(H)}(n) \ = \ \sum_{i=1}^n \ i^{-1} \ = \ \sum_{i=1}^n \ 1/i \]
namely, $1$, $1/2$, $1/3$, \ldots, $1/10$.  The figure represents these terms by means of ten abutting unit-width rectangles whose respective heights are given by the terms.  Summation $S^{(H)}(n)$ is, then, represented as the aggregate area of the abutting rectangles.  If we were to extend the figure rightward (which increases $n$ and thereby extends the summation by encompassing more addends), then the next two rectangles would have respective heights $1/11$ and $1/12$.

\smallskip

Our stratagem mandates embellishing the rectangles in Fig.~\ref{fig:riemann-harmonic} by two continuous curves (labeled (a) and (b) in the figure) that connect the rectangles' upper corners.
\begin{figure}[htb]
\centerline{
\includegraphics[scale=0.3]{FiguresMaths/RiemannSum}
%\includegraphics[scale=0.3]{FiguresMaths/RiemannSum1}
}
\caption{The summation $S^{(H)}(n) \ = \ \sum_{i=1}^n \ 1/i$ represented as the aggregate area of a sequence of unit-width rectangles.  The summation is {\em bounded from above} by the combined area of the leftmost rectangle and the area under the continuous curve (a) that connects the upper righthand corners of the rectangles.  In detail, curve (a) begins at the upper righthand corner of the leftmost rectangle; it continues to the upper righthand corner of the rightmost rectangle.  (The figure's resolution makes the righthand end of curve (a) invisible.)  The upper-bounding area is $1 \ + \ \int_1^n \ 1/x dx$.  The summation is {\em bounded from below} by the area under the continuous curve (b) that connects the upper lefthand corners of the rectangles.  In detail, curve (b) begins at the upper lefthand corner of the leftmost rectangle and continues to the upper lefthand corner of the rightmost rectangle.  The area under curve (b) is $\int_0^n \ 1/(x+1) dx$.}
\label{fig:riemann-harmonic}
\end{figure}
Because the lefthand rectangle {\em plus} curve (a) completely ``cover'' the rectangles, theit combines areas provide an {\em upper bound} on the aggregate area of the rectangles.
\ignore{**********
; the area under curve (a) is
\[ \int_1^n \ \frac{1}{x} dx \]
...
\end{figure}
*****************}
Because curve (b) lies completely within the rectangles, the area under the curve is a {\em lower bound} on the aggregate area of the rectangles.
\end{enumerate}
%
...
\noindent {\bf Step 2.}
Construct a continuous curve $\overline{C}(x)$ that passes through the corners of the unit-width rectangles specified by the summation, in such a way that the rectangles lies completely within the area under $\overline{C}(x)$.
...
The curves labeled (a) in Figs.~\ref{fig:riemann-n2} and~\ref{fig:riemann-harmonic} are instances of the mandated continuous curve $\overline{C}(x)$.
\end{minipage}
}
\medskip

Because the aggregate areas of the abutting rectangles lies completely under curve $\overline{C}(x)$, the area under the curve---which we obtain by integrating $\overline{C}$ between appropriate limits---yields an {\em upper bound} on the value of the summation of interest.
...
The curves labeled (b) in Figs.~\ref{fig:riemann-n2} and~\ref{fig:riemann-harmonic} are instances of the mandated continuous curve $\underline{C}(x)$. 
\end{minipage}
}
\medskip

Because the area under the curve $\underline{C}(x)$ lies completely within the abutting rectangles, the area under the curve---which we obtain by integrating $\underline{C}$---affords {\em a lower bound} on the summation of interest.

\medskip

In the next subsection, we apply this stratagem to summations of fixed powers of successive integers, i.e., summations of the form
...
\[ S_c(n) \ \eqdef \ \sum_{i=1}^n \ i^c \]
for arbitrary positive numbers $c$.  The reader can garner intuition for the upcoming bounds from the general shape of the rectangles and continuous curves in Fig.~\ref{fig:riemann-n2}.  We obtain our upper bound on the summations $S_c(n)$ by evaluating the integrals that yield the area $\overline{C}_c(n)$ under the continuous curve (a) in Fig.~\ref{fig:riemann-n2}, namely,
\begin{eqnarray}
...
\end{eqnarray}
We obtain our lower bound on $S_c(n)$ by evaluating the integral that yields the area $\underline{C}_c(n)$ under the continuous curve (b) in Fig.~\ref{fig:riemann-n2}, namely,
\begin{equation}
