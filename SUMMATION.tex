...
e.g.: numbers, Riemann sums, slices of pie, tokens arranged in stylized ways, basic geometrical structures
\end{enumerate}
...
aid of the calculus that was invented by Newton and Leibniz for such
dealings.
...
{\Denis Again, as I said before, I am not really convinced by this example.
...
{\Denis add cross reference for computing the sum of powers of 2 in DOINGMATHS}
...
{\em Now is an opportune moment to step back from the specific result
  ...
  series.  We show now that the pattern can be exploited to other,
  related, ends.}

\medskip


In the spirit of the previous proof, we provide below a graphical variant of this result.
The idea is to look at this problem by a simple (in the \textit{virtuous sense}) estimation of surfaces.
The integers are represented by the concatenation of basic $1$ by $1$ squares.
The result is immediate as shown in the three figures~\ref{fig:sumIntegersGeo1, fig:sumIntegersGeo2, fig:sumIntegersGeo3},
it is obtained by the surface of half the big square plus $n$ times the surface of half the unit squares:
$$S_1(n) = \frac{n^2}{2} + \frac{n}{2}$$

\begin{figure}[h]
\begin{center}
       \includegraphics[scale=0.4]{FiguresMaths/SumIntegersGeometricBasis}
              \caption{Representation of the $n$ first integers using basic unit squares ($n=6$).}
...
\begin{center}
       \includegraphics[scale=0.4]{FiguresMaths/SumIntegersGeometricIntermediate}
              \caption{The surface of lower-right triangle (light grey) is half of the big $n$ by $n$ square.}
...
\begin{center}
       ...
       \caption{The surface of the triangles on the upper diagonal (dark grey) is $\frac{n}{2}$.}
...
Let us now develop another type of proofs, namely, a combinatorial proof.
It consists in using a counting argument for proving $S_1(n) = {n+1 \choose 2}$.

Let consider any $i$ from $0$ to $n$, and count all possible situations for $j > i$.

On the first hand, there are $n$ possible values of $j$ for $i=0$,  $n-1$ for $i=1$, and so on.
Thus, the number of such situations is equal to $n+(n-1)+ ... + 1 = S_1(n)$.
On the other hand, all these situations correspond to the possibilities to take two different values ($i$ and $j$) among the $n+1$ 
available positions, that is ${n+1 \choose 2}$.

{\Denis if necessary, I can develop the simple example for $n=3$...}
...
We present below a third alternative proof using Fubini's principle (see section~ref{sec:xx}). {\Denis complete the reference,
the notations are the same as before}

\begin{figure}[h]
\begin{center}
       \includegraphics[scale=0.4]{FiguresMaths/SumOddsBasis}
              \caption{Representation of the $n$ first odd integers using bullets ($n=5$).}
       \label{fig:sumOdds1}
\end{center}
\end{figure}
...
\begin{center}
       ...
       \caption{The final picture organized as a big $n$ by $n$ square.}
       \label{fig:sumOdds3}
\end{center}
\end{figure}
...
%There are several other ways to prove this result.
The previous proof was nothing else that a smart graphical way to draw and arrange the bullets of the integers. 
...
For the concern of computing the sum of the first odd numbers, we organize them as shown in Table~\ref{SumOddsTriangle}.
...
We develop below a graphical way for computing the geometrical series $\frac{1}{2}$~ref{fig:sumGeoBasis}.
The result is immediate while considering a basic unit square and its successive decompositions while divided by $2$.
...
\begin{center}
       \includegraphics[scale=0.4]{FiguresMaths/SumGeometric1sur2Bis}
              \caption{Geometric series obtained by an adequate arrangement of successive rectangles.}
       \label{fig:sumGeoBasis}
\end{center}
\end{figure}

This procedure for $\frac{1}{2}$ is hard to extend, but as depicted in figure~\ref{ig:sumGeoGeneral}, it is possible to modify it in order to consider any $r$ (for $0 \leq r <1$).
The sum is the length of the line. 

\begin{figure}[h]
\begin{center}
       \includegraphics[scale=0.4]{FiguresMaths/SumGeometric2tiers}
              \caption{Geometric series obtained by an adequate cascading of successive squares (for $r=\frac{2}{3}$).}
...
Represent the summands seriatim as abutting unit-width rectangles.  
  \begin{itemize}
  \item
In Fig.~\ref{fig:riemann-harmonic}, for instance, these rectangles have
heights $12$, $6$, $4$, $3$, $16/3$, and $2$.  If we were to extend the
figure, then next rectangle would have height $16/7$.

  \item
In Fig.~\ref{fig:riemann-n2}, for instance, these rectangles have
heights $1$, $4$, $9$, $16$, and $25$.  If we were to extend the
figure, then next rectangle would have height $36$.
  \end{itemize}
...
\subsection{Summing positive powers of consecutive integers}
\label{sec:sum-of-i2k}
...
\subsection{Summing negative powers of consecutive integers}
\label{sec:sum-i2c<1}

\paragraph{A. Summing consecutive $(1/i)^c$ for fixed $c > 1$}
TO BE DONE

\paragraph{B. Summing consecutive $(1/i)$: harmonic sums}
