...
\index{evaluating geometric sums and series!a pictorial evaluation for
  $S^{(\infty)}_{1/2}$ using cascading shrinking squares}
\begin{proof}
{\bf A pictorial summing of $S^{(\infty)}_{1/2} = \sum_{i=0}^\infty
  2^{-i}$.}
%
...
\noindent
{\bf Enrichment: variations on the theme.}
%
The preceding pictorial derivation of the sum $S^{(\infty)}_{1/2}$ can
be formulated using geometric shapes other than squares.  We now
suggest suggest two of these which the interested reader may enjoy
exploring further.
...
\noindent{\bf Using a vigorously sliced pie.}
%
This variation focuses on the modified series
...
Our evaluation of $\bar{S}^{(\infty)}_{1/2}$ is depicted in
Fig~\ref{fig:sumGeo1sur2circle}; the inverse powers of $2$ are
represented by appropriate fractions of the unit-diameter disk.  The
evaluation begins with this disk before it is sliced.
\begin{figure}[ht]
\begin{center}
...
Section~\ref{sec:general-geometric-sums}.  For brevity, we ignore the case
...
the idea is to write this expression in two different ways by isolating the last term as follows:
\begin{equation}
\label{eq:ext-geom-c=1.1}
S_b^{(1)}(n+1) \ = \ S_b^{(1)}(n) \ + \ (n+1) b^{n+1}
\end{equation}
and by isolating the first term as follows:
\begin{eqnarray*}
S_b^{(1)}(n+1)
     & = &
...
\subsubsection{A fun result via geometric sums: When is $n$ divisible
  by $9$?}
