...
coefficient $\Delta_{n+1}$ whose definition we specialize from
...
\]
Hence, this group begins with $2 \Delta_k -1$ and ends with
$2\Delta_k +1$.
...
2 \Delta_{k-1} +2k-1
\]
One verifies easily (say, by induction) that $2 \Delta_{k-1} +2k-1 = 2
\Delta_{k} +1$.
...
\item
We sum the successive rows of our table.  The first rowof the table
...
As we shall see in Section~\ref{sec:sum-of-i2c>0}, one can develop
techniques for computing the sum of the first $n$ perfect cubes that
are smooth extensions of the techniques for summing the first $n$
integers or the first $n$ perfect squares.
...
{\Arny Please do change the r to a b.  The ``b'' happened because of
  the word ``base''}
...
the next rectangle would have height $36$.
  ...
  \item
In Fig.~\ref{fig:riemann-harmonic}, we represent the {\em harmonic} sum
...
$\displaystyle \sum_{i=1}^n i^c$---for various (classes of)values of
the fixed parameter $c$.
%  In
...
\subsubsection{Summing $c$th powers of consecutive integers: $c \geq 0$}
\label{sec:sum-of-i2c>0}

\paragraph{\sf A. General powers $c \geq 0$}
...
for arbitrary positive numbers $c$.  Extrapolating from the example in
Fig.~\ref{fig:riemann-n2}, 
we obtain our upper bound on $S_c(n)$ by evaluating the integral that
yields the area $\overline{C}_c(n)$ under the lefthand continuous
...
\paragraph{\sf B. Powers that are positive integers}
...
affords us a much more detailed version of the bounds
(\ref{eq:bounds-sum-xc}) on $S_c(n)$, via a much more detailed upper
bound (\ref{eq:upper-integral-xc}) on the sum.
...
$k=2$, invoking as a technical tool
...
(\ref{eq:upper-integral-xc}) and (\ref{eq:lower-integral-xc}) we
obtain:
\begin{itemize}
\item
...
\paragraph{\sf A. An explicit expression for $S_1(n)$}
...
\paragraph{\sf B. An explicit expression for $S_2(n)$}
