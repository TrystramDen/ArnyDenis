...
Proposition~\ref{thm:squares-odd-integers-Gauss}.  In that proof, we represented each odd integer $2k+1$ as a length-$(2k+1)$ ``bar", and we bent the ``bar" into a reversed-L whose horizontal and vertical sub-``bars" each had length $k+1$.  In the current process, we are performing the exact inverse process.
\end{minipage}
}
