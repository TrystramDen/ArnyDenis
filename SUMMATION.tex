...
the value of the infinite series $S^{(\infty)}_{1/b} =
\sum_{i=0}^\infty \ b^{-i}$.
\end{proof}
...
S_2^{(1)}(n) \ = \ \sum_{i=1}^n \ \sum_{k=0}^{i-1} 2^i
\end{equation}
...
expression:
\[
S_2^{(1)}(n) \ = \ \sum_{k=0}^{i-1} \ \sum_{i=k}^{n} 2^i.
\]
...
\index{Napier, John}
{\it Napierian},\footnote{The natural logarithm (to the base $e$) is
  commonly referred to as the {\it Napierian logarithm}, in honor of
  the Scottish polymath John Napier.}~logarithm $\ln n$, i.e., the
logarithm whose base is Euler's constant \index{Euler's constant} $e =
2.718281828 \ldots$.

\begin{prop}
\label{thm:harmonic}
The growth behavior of the harmonic summation $S^{(H)}(n)$ is given by
\[ S^{(H)}(n) \ \approx \ \ln n. \]
...
\paragraph{{\bf Enrichment.} {\it More about the harmonic summation
    and series}}
%
...
In order to better understand $S^{H}(n)$, let us group the summation's
consecutive terms into groups whose sizes are consecutive powers of $2$:
\[
1 \ + \ \left( \frac{1}{2} \right) \ + \
 \left( \frac{1}{3} + \frac{1}{4} \right) \ + \
\left(\frac{1}{5} + \frac{1}{6} + \frac{1}{7} + \frac{1}{8} \right) \ + \cdots
\]
Then we isolate each of these grouped partial sums into the following
sequence:
\begin{eqnarray*}
A_0 & = &  \frac{1}{2} \\
...
\vdots & & \vdots \\
A_k & = &  \frac{1}{2^i+1} + \cdots + \frac{1}{2^{i+1}} \\
\vdots & & \vdots \\
\end{eqnarray*}
...
the drawing in Fig~\ref{fig:HarmonicSumUpperbound}. shows that all the $A_i$ are lower than $1$
