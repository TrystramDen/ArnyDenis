...
\label{eq:bounds-(-1)<c<0}
 \frac{1}{c+1} n^{c+1} \ - \  \frac{1}{c+1} n^c
  \ \ \leq \ \ S_c(n)
  \ \ \leq \ \
...
\begin{proof}
We see as in Eq.~(\ref{eq:bounds-(-1)<c<0}) that, for $c<-1$, as $n$ grows without bound, $S_c(n)$ is majorated by ${\displaystyle \frac{c}{c+1}}$.  \qed
\end{proof}
