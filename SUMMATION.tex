...
The original sum is on the left, each $i \cdot 2^i$ is distributed on column $i$.
The second sum is obtained by introducing a new index $k$ and to scan each term of the sum rom by rom (on the right)}
\label{fig:Sumi2i}
\end{figure}
...
{\it Napierian},\footnote{The natural logarithm, i.e., the logarithm
  ...
  Napier.}~logarithm $\ln n$, i.e., the logarithm whose base is
