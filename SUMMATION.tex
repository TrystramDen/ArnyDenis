...
{\Arny I did not totally succeed -- $e$ snuck into my calculation.  I do not know whether I have made a mistake or $e$ really belongs here.  I have assumed the latter -- just to pick a side!Please check.}

We can combine the bounds (\ref{eq:upper-integral-xc}) and (\ref{eq:lower-integral-xc}) into a perspicuous two-sided bound on $S_c(n)$.  The lower bound comes directly from Eq.~(\ref{eq:lower-integral-xc}).  We derive the upper bound by manipulating 
Eq.~(\ref{eq:upper-integral-xc}) into a more convenient form.  We begin by multiplying the expression by $n/n = 1$, which, does not change its value; we then perform some algebra and invoke a well-known lemma from calculus.
\begin{eqnarray}
\nonumber
\overline{C}_c(n)
  & = &
...
\nonumber
  & = &
\frac{n+1}{c+1} \cdot \left( \frac{n+1}{n} \right)^c \cdot n^c  \ + \ \overline{\kappa} \\
\label{eq:useVALUEe}
  & \leq &
\frac{e}{c+1} \cdot \left( \frac{n+1}{n} \right)^c
 \cdot  \left( n^{c+1} \ + \ n^c \right)  \ + \ \overline{\kappa} \\
\nonumber
  & \leq &
\frac{e}{c+1} \cdot n^{c+1} \ + \ \kappa n^c \ \ \ \ \ \mbox{ for some } \kappa \geq 0
\end{eqnarray}
The lemma we invoke from calculus gives us inequality~(\ref{eq:useVALUEe}), which tells us how to ``sneak up on"  Euler's constant $e = 2.71828\ldots$.

\begin{lemma}
\label{thm:Limit4e}
For all positive $n$, $(1 \ + \ 1/n) \ \leq \ e$.
\end{lemma}

With the help of this lemma, we finally have our two-sided bound on $S_c(n)$:
...
\frac{1}{c+1} n^{c+1} \ + \ \underline{\kappa}
  \ \ \leq \ \ S_c(n)
  \ \ \leq \ \ \frac{e}{c+1} n^{c+1} \ + \ \kappa n^c
\end{equation}
\end{prop}
