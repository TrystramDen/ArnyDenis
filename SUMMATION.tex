...
Eq.~(\ref{eq:upper-integral-xc}) into a more convenient form.  We begin by multiplying the expression by $n^c/n^c = 1$, which, does not change its value; we then perform some algebraic manipulation.
\begin{eqnarray}
\nonumber
\overline{C}_c(n)
  & = &
...
\nonumber
  & = &
\frac{1}{c+1} \cdot (n+1) \cdot n^c \cdot \left( \frac{n+1}{n} \right)^c  \ + \ \overline{\kappa} \\
\nonumber
 & = &
