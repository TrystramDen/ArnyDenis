...
\item
{\bf Sums and sums of sums and \ldots}

{\sc Lesson:} Practice manipulating summations
...
Consider the following summations.
  ...
  \item 
By definition:
\[ Id(n) \ \eqdef \ \sum_{k=1}^n \ 1 \ = \ 1 + 1 + \cdots +1 \  = \ n \]

  \item 
We have proved in multiple ways:
\[ \Delta(n) \ \eqdef \  \sum_{k=1}^n \ k \ = \ 1+2+3+ .\cdots +n \ = \ \frac{1}{2} Id(n \cdot (n+1)) \]

  \item 
{\em Prove that}
\[ \Theta(n) \ \eqdef \  \sum_{k=1}^n \ \Delta(k) \ = \  
\Delta(1) + \Delta(2) + \cdots + \Delta(n) \ = \ \frac{1}{3} \Delta(n \cdot (n+2)) \]

  \item
$\oplus \oplus$
{\em Develop and verify a formula for the sum:}
\[ \Upsilon(n)  \ \eqdef \  \sum_{k=1}^n \ \Theta(k) \ = \  
\Theta(1) + \Theta(2) + \cdots + \Theta(n) \]
  \end{enumerate}
