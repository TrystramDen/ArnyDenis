...
Proposition~\ref{thm:squares-odd-integers-Gauss}---as we simultaneously observe a
remarkable property of integers.

\ignore{\Denis This subterfuge will also evidence another amazing property of integers}

\medskip

This proof combines facts about the odd numbers $\{2k-1\}$, the perfect cubes $\{k^3\}$, and the triangular numbers
$\{ \Delta_k \}$.  As with the preceding proof, pictures will play an important role in discovering and explaining
the facts that we uncover.
...
\]
so that the {\em sum}, call it $G_i$, of the $i$ integers in group $i$ equals
\begin{eqnarray*}
G_i  & = & 2 i \Delta_{i-1} \ + \ \big( 1 + 3 + 5 + \cdots + (2i-1) \big) \\
...
You should now verify by induction that $2 \Delta_{i-1} +(2i-1) \ = \ 2 \Delta_{i} +1$, which gives us another
...
\]
  ...
  In other words, group $i$ consists
of the odd numbers
