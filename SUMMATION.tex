...
The arrows represent the order of execution. The drawing was done for $n=4$}
\label{fig:Sumi2i}
\end{figure}
...
{\Denis I added an interesting comment to finish this section:}
Let remark that the process of obtaining the single summation can be also seen in the figure by scanning the elements of the sum by diagonals (see Fig.~\ref{fig:Sumi2iDiag}).
Each of the $n$ diagonal is exactly the different between the complete geometric series minus the partial geometric series truncated at rank $k$. 
\begin{figure}[htb]
\centerline{
...
\paragraph{\small\sf C. Negative powers $c$ with $c = -1$: the {\em harmonic} summation}
