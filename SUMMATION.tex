...
Guido Fubini \index{Fubini, Guido} was able to make notable mathematical progress by rearranging
the representations of a variety of structured mathematical objects \cite{Fubini}.  Within the context of the current
chapter, we apply such rearrangements to achieve exact values for some
...
\ignore{\Denis My remark only works for particular values of $n$, actually, the $\Delta_k$.
...
\ \ = \ \ 1^3 \ + \ 2^3 \ + \ 3^3 \ + \cdots 
\]
Note the ``Fubini-esque" arrangements and groupings throughout this argument.  \qed
\end{proof}
...
We turn now to a {\em pictorial} illustration that the sum of the perfect cubes up to $k$
is equal to the square of $\Delta_k$.  We verify this charming fact by an induction that
reflects the structure of table (\ref{eq:triagular-table}).
\begin{description}
\item[{\bf Base cases.}]
We display the first two cases of the claimed result.
\[ \begin{array}{ccccc}
1^3                     & = & 1   & = {\displaystyle {2 \choose 2}^2 } \\
1^3 + 2^3           & = & 9   & = {\displaystyle {3 \choose 2}^2 } \\
\end{array}
\]
These cases are represented pictorially in Figs.~\ref{fig:sumCubes1} and~\ref{fig:sumCubes1bis},
which reflect the first two groups in table (\ref{eq:triagular-table}).
\begin{figure}[ht]
\begin{center}
       \includegraphics[scale=0.35]{FiguresMaths/SumCubes1}
\caption{Pictorially depicting the odd numbers  $\{1\}$ (group $1$ in table (\ref{eq:triagular-table}))
and $\{3, 5\}$  (group $2$ in table (\ref{eq:triagular-table})).}
       \label{fig:sumCubes1}
\end{center}
\end{figure}
\begin{figure}[ht]
...
\caption{Illustrating how to form a perfect square from the odd integers of
Fig.~\ref{fig:sumCubes1}.  One arranges the pictorial representation of the integers into the
depicted figure.  The clear portions of the resulting perfect square come from 
group $2$ in table (\ref{eq:triagular-table}); the shaded small square comes from 
group $1$ in table (\ref{eq:triagular-table}).}
...
Say for the sake of the induction that the target equality holds for all $n < k$; i.e.,
\[ 1^3 \ + \ 2^3 \ + \cdots + \ n^3 \ \ = \ \ (\Delta_n)^2 \ \ = \ \ {{n+1} \choose 2}^2 \]
...
$2i \Delta_i$ + ($\Delta_{i-1})^2$

$(\Delta_{i-1} + i)^2 \ = \ (\Delta_{i-1})^2 + 2i \Delta_{i-1} + i^2$ 
