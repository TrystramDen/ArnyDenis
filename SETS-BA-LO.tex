...
\index{{\sf SAT}: the Satisfiability Problem!{\sf 3SAT}: {\sf SAT} with $3$ literals per clause}

\index{{\sf 2SAT}: the Satisfiability Problem!the {\sf SAT} problem with $2$ literals per clause}
\index{{\sf SAT}: the Satisfiability Problem!{\sf 2SAT}: {\sf SAT} with $2$ literals per clause}
...
{\em
Prove that the text's two views of problem SAT are equivalent, i.e., that they specify the same mathematical/computational problem.}
\begin{itemize}
\item
