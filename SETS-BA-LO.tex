...
\end{enumerate}
There is a marvelous result, the Schr\"{o}der-Bernstein Theorem,
\index{The Schr\"{o}der-Bernstein Theorem} which can be quite useful
...
encounter such a scenario, for example, when proving
Lemma~\ref{lem:N-=-F} in Section~\ref{sec:FNS-uncountable}.  The
Theorem says that for any sets $S$ and $T$, if there exists an {\em
  injection} from $S$ into $T$ and an {\em injection} from $T$ into
$S$, then one can ``combine'' these injections in order to craft a
{\em bijection} between $S$ and $T$.
...
\begin{proof}[Sketch]
We sketch the perspicuous proof which appears in \cite{Tonien07}.

Let $S_0 = S$ and $T_0 = T$.  For each index $i$, 
{\Denis we should add the range of $i$ here, start at 0 up to some integer denote by  $\omega$} define
...
set $A$ and function $f$ of $A$,
\[ f(A) \ = \ \{ f(a) \ | \ a \in A \} \]
Because each of the injections maps its domain-set {\em into} its
range-set, we observe that increasing indices lead to nested
...
follows:
\[ \mbox{Each } \ \  U_i \ = \ S_i \setminus S_{i+1} \ \ \ \mbox{ and
} \ \ \ U_\omega \ = \ \bigcap_i S_i
\]
Our reasoning shows that set $S$ is partitioned into the sets
$U_0, U_1, U_2, \ldots$ and $U_\omega$.
\item
The sets $V_\omega$  and $V_0, V_1, V_2, \ldots$ are defined as
follows:
{\Denis you mean $T$ instead of $Y$...}
\[ \mbox{Each } \ \  V_i \ = \ T_i \setminus Y_{i+1} \ \ \ \mbox{ and
} \ \ \ V_\omega \ = \ \bigcap_i T_i
\]
Our reasoning shows that set $T$ is partitioned into the sets
...
\bigcap S_i \ = \ U_\omega,
\]
$F^{(S \rightarrow T)}$ maps $U_\omega$ bijectively onto $V_\omega$.

{\Denis right, but the interchanged of the inverse and the intersection is not obvious, may but a word here would be welcome?}
...
\end{itemize}
{\Denis nice and clear, just add the range of $i$ here (since it is bounded by $omega$}
This sketch provides all of the salient ingredients of the proof.  The
reader can access more details in \cite{Tonien07}.  \qed
\end{proof}
...
\noindent {\em The notion of characteristic sequence leads to a
  quite simple way of counting the number of distinct subsets that an
  $n$-element set has.}
...
\noindent {\em The notion of characteristic vector leads to a rather
  straightforward proof that there does not exist a bijection between
  the set $\N^+$ of positive integers and the set of all functions
  from $\N^+$ to $\{0,1\}$, i.e., the set $\big\{ f:\N^+ \rightarrow
  \{0,1\} \big\}$.}  See Proposition~\ref{thm:R-uncountable}.
