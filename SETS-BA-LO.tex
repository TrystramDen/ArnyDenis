...
{\it Sets.} \index{set} Sets are probably the most basic object of
...
\noindent
We illustrate two sample forms for the mathematical ``laws'' we shall
encounter.
\begin{itemize}
\item
Certain pairs of operations, call them $\circ_1$ and $\circ_2$, can be
applied in various orders without changing the final value:
\[ \big(S_1 \circ_1 (S_2 \circ_2 S_3)\big) \ \
= \ \ \big((S_1 \circ_1 S_2) \circ_2 S_3\big)
\]

\item
An operation $\circ$ which is {\it associative} can be applied to its
arguments in various orders without changing the final value:
...
{\it Structured sets}. \index{set!structured} The first operations on
sets that we study are those that allow us to endow sets with enough
structure to begin to talk about ``real life''.  One can view these
operations as the mathematical analogue of ``systems programs'' within
the realm of computers.  We formulate rigorous notions for concepts
such as {\it relation} and {\it function} that are among the central
concepts of mathematics.  Once we have such notions, we can begin to
formulate realistic {\it mathematical models} for real-life entities.
Two features that will stand out are: how naturally the formal notions
capture the intuitive notions of the vernacular; how technically
simple the formal notions are---which facilitates using them in
sophisticated analyses.

\bigskip

{\it Algebras}.  \index{algebra}\index{algebra!of sets} Once one has
operations which obey laws, one has an {\it algebra},
\index{algebra!definition} i.e., a system of objects, together with a
repertoire of operations on the objects.  There are several natural
algebras whose objects are sets.  We shall discuss two of them at some
length.  First, we focus on the algebra that relates to the most basic
operations on sets: the operation of {\it union}, denoted $S \cup T$,
which combines the membership of its argument sets, $S$ and $T$, and
{\it set difference}, denoted $S \setminus T$, which excludes from set
$S$ all members of set $T$.  Building on these two operations, we can
build myriad others, including {\it intersection}, denoted $S \cap T$, which
isolates all elements that sets $S$ and $T$ share, and {\it symmetric
  difference}, denoted $S+T$, which isolates all elements that belong
to precisely one of $S$ and $T$.  Importantly, we can begin to study
the laws that these operations obey.  One of the central topics in
this study is the class of {\it Boolean} algebras
\index{algebra!Boolean algebras} whose name derives from that of the
British mathematician George Boole. \index{Boole, George}
...
without using terms such as {\it and}, {\it or}, and {\it not}, which
fall naturally within the domain of the simplest variety of {\it
  mathematical logic}---the {\it logic of propositions}.  This branch
of logic, which is known more familiarly as {\it Propositional Logic},
deals purely with the way the truth-values ({\sc true} or {\sc false})
of complex logical expressions combine via operators such as {\it
  and}, {\it or}, and {\it not} to produce a truth-value for the
entire expression.  This is the {\em simplest} variety of logic
becuase it does not entertain any form of subtlety that arises from
issues such as {\it quantifiers} (e.g., {\sc there exists}
[symbolically, $\exists$] or {\sc for all}, [symbolically,
  $\forall$]), or {\it modalities} (e.g., {\sc eventually} or {\sc
  from some moment on}).  The Propositional Logic is a fascinating
special form of Boolean algebra for at least two reasons.  First, it
enjoys a special algebraic feature known as {\it free-ness}, which
enables one to prove theorems in the Propositional Logic using {\em
  truth tables:} this amounts to proving theorems within this system
by symbolic evaluation rather than the axioms plus rules of inference
that we all found so onerous in our high school geometry course.
Second, Propositional logical expressions are powerful to model
certain types of computations faithfully enough to serve as a
foundation for a methodology for measuring computationa complexity.
