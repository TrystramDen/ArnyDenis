...
\index{set!the belongs-to relation $\in$}
...
\index{commutative law} \index{associative law!for addition}
\index{associative law} \index{associative law!for addition} \index{laws of algebra}
\index{laws of algebra!commutative law}
\index{laws of algebra!associative law}
...
The word ``law'' plays at least three mutually inconsistent roles in our lives.
\begin{enumerate}
\item
...
\medskip\item
We learn in school about certain ``laws'' of physics---the ``law'' of gravity, the ``law'' of relativity, the ``law'' of conservation of mass, to name just a few.  As time goes by and new science is discovered, some of these ``laws'' get amended---we learn, for instance, that mass and energy are not conserved: they are, in fact, interchangeable.  Indeed, some ``laws'' are discovered to have been {\em false}---the tale of {\it phlogiston} comes to mind.  These ``laws'' are {\em approximations to/predictors of reality} which survive until new ``laws'' are discovered that are better approximations/predictors.  \index{phlogiston}
...
{\it Structured sets}.  (Section~\ref{sec:structured-set}) The first operations on sets which we study endow sets with enough structure to talk about aspects of  ``real life''.  One can view these operations as the mathematical analogue of ``systems programs'' within the realm of computers.
\begin{itemize}
\item
...
\index{algebra!base operations}

\smallskip

\index{algebra!Boolean Algebra} \index{Boole, George}
Importantly, we can now begin to study the laws that these operations obey.  One of the central topics in this study is the class of algebras on sets which are named in honor of the nineteenth-century English mathematician George Boole.  Boole is generally credited with inventing these {\it Boolean} algebras. 
...
\index{logic!quantifiers}
\index{logic!modalities}
...
\index{set!empty set $\emptyset$} \index{set!doubleton (set)}
\index{singleton (set)} \index{doubleton (set)}
...
\index{power set (of a set): set of all subsets}
...
\index{De Morgan's Laws} \index{De Morgan, Augustus}
...
\index{(algebraic) closure} \index{closure (algebraic)}
...
\index{relation on sets}  \index{binary relation}
...
\index{relation (logical) negation} \index{$\overline{\rho}$: the negation of relation $\rho$}
\noindent
...
\index{partition!and equivalence relation} 
...
\index{equivalence relation!index of} \index{relation!equivalence relation!index of} 
...
\index{inverse of an injection} \index{injection!inverse}
...
% (We prove this as Proposition~\ref{thm:sqrt(2)}.) 
  \end{itemize}
\end{itemize}
...
\index{representation of integers!numeral}
\index{representation of integers!number base}
\index{representation of integers!base-$2$ representation}
\index{representation of integers!binary representation}
\begin{quote}
A {\it numeral} is a sequence of digits that is the ``name'' of a number.  The
numerical value of a numeral $x$ depends on the {\it number 
base}, which is a positive integer $b >1$ that is used to create $x$.  Much
of our focus will be on {\em binary} numerals---which are binary
strings---for which the base is $b=2$.
...
\index{Bernstein, Felix} \index{Schr\"{o}der, Ernst}
\index{Cantor-Bernstein Theorem} \index{Cantor, Georg}
...
The celebrated theorem of this section is usually attributed to Ernst Schr\"{o}der and Felix Bernstein, but it is often attributed alternatively to (Georg) Cantor and Bernstein.  As this complex attribution suggests, the theorem has a rather complicated history.  Picking just a few high points of this history: The theorem was first stated without proof by Cantor in \cite{Cantor87}.  Roughly a decade later, Schr\"{o}der produced a flawed proof in \cite{Schroeder98a}.  As reported in \cite{Deiser2010}, Schr\"{o}der soon thereafter provided a correct proof---as, independently, did Bernstein.

\smallskip

We shall refer to the theorem by its customary name, the {\it Schr\"{o}der-Bernstein Theorem}, for that is the most likely name the reader will encounter. 
\end{minipage} }
...
\index{image of a set under a function}
\index{image of a set under a function!$f(A)$: the image of set $A$ under function $f$}
\index{$f(A)$: the image of set $A$ under function $f$}
...
$F^{(S \leftrightarrow T)}$ maps each set $U_{2i}$ bijectively onto $V_{2i+1}$ in the same way that $F^{(S \rightarrow T)}$ does.
  \medskip\item
$F^{(S \leftrightarrow T)}$ maps each set $U_{2i+1}$ bijectively onto $V_{2i}$ in the same way that $\Big[F^{(T \rightarrow S)}\Big]^{-1}$ does.
\end{itemize}
...
\noindent {\em The notion of characteristic vector leads to a rather straightforward proof that there does {\em not} exist a bijection between the set $\N^+$ of positive integers and the set $\F$ of all functions from $\N^+$ to $\{0,1\}$; symbolically, $\F \ = \ \big\{ f:\N^+ \rightarrow \{0,1\} \big\}$.}  See Proposition~\ref{thm:R-uncountable}.
\end{enumerate}
...
\index{Propositional Logic}\index{Propositional Calculus}
...
As we noted earlier in this chapter, what distinguishes Propositional Logic from more general mathematical logic is the absence of quantifiers ({\sc there exists}, {\sc for all}, etc.).  The objects of Propositional Logic are {\it propositions}, i.e., fixed assertions, as exemplified by the following three sentences.
...
The property of being a ``proposition'' is {\em syntactic}, rather than {\em semantic}.  Our three sample propositions illustrate that the assertion made in a proposition need not be factually true---nor even sensible.  Indeed, our third sample proposition was made famous by American linguist Noam Chomsky---in his seminal monograph {\it Syntactic Structures} (Mouton Press, 1957, {\it Janua Linguarum} series)---as he illustrated the independence of the notions ``grammatical" and ``meaningful".
\end{minipage}
}
...
\medskip\item
{\sc implies}: {\bf\em logical implication} ($\Rightarrow$)
\hspace*{.1in}
{\small\sf (Set-theoretic analogue: {\em subset})}.
...
\medskip\item
{\sc iff}: {\bf\em logical equivalence} ($\equiv$)
\hspace*{.1in}
...
\index{Boolean Algebra!logical operations}
...
Proving Theorem~\ref{thm:Prop-calc-isfree} is beyond the scope of this book.  The interested reader can find a proof in numerous texts on logic.  The proof in  \cite{Rosser53} might be particularly interesting to an aspiring mathematician because that source exists precisely to explain logic in terms that are accessible to a mathematician.
...
\index{logical operation!a functional view}
...
\noindent {\it The law of double negation}
...
\noindent  {\it The law of contraposition}
...
\noindent {\it The distributive laws for Propositional Logic}
...
\noindent {{\sf SAT}: {\it The original {\sf NP}-complete problem}

\index{Satisfiability Problem (for the Propositional Calculus)}
\index{{\sf SAT}: the Satisfiability Problem!definition} 
\index{{\sf SAT}: the Satisfiability Problem} \index{Satisfiability problem!combinatorial formulation}
\index{{\sf SAT}: the Satisfiability Problem!combinatorial formulation}
\index{Satisfiability problem!formal logical formulation}
\index{{\sf SAT}: the Satisfiability Problem!formal logical formulation}
...
\noindent{\bf {\sf 2SAT}: {\sf SAT} with two literals per clause}

Once a framework for discussing the complexity of computations received general acceptance, researchers began to investigate the boundaries within the framework: How, and how much, could one ``weaken'' an {\sf NP}-complete problem before one lost the problem's {\sf NP}-completeness?  One of the earliest examples of this activity occurred within the paper that actually invented the framework, namely, \cite{Cook71}!  A careful analysis revealed the following strengthening of Theorem~\ref{thm:Cook}(a).

\begin{theorem}[Cook's Theorem: stronger version]
\label{thm:Cook-3SAT}
{\sf 3SAT}, the {\sf SAT} problem with three literals per clause, is {\sf NP}-complete.
\end{theorem}
\index{{\sf 3SAT}: the {\sf SAT} problem with three literals per clause}
\index{{\sf SAT}: the Satisfiability Problem!{\sf 3SAT}: {\sf SAT} with three literals per clause}

\index{{\sf 2SAT}: the Satisfiability Problem!the {\sf SAT} problem with two literals per clause}
\index{{\sf SAT}: the Satisfiability Problem!{\sf 2SAT}: {\sf SAT} with two literals per clause}
...
\begin{prop}
Let $\rho$ be a strong order relation on a set $S$, and let $\underline{\rho}$ be the weak version of $\rho$.  Say that $\rho$ is {\em total} in the sense that for distinct $s, t \in S$, either  
$s \rho t$ or $t \rho s$.

\smallskip

{\bf (a)}
$\underline{\overline{\rho}} = \rho$
...
{\bf (b)}
For all $s, t \in S$, \ \ \
$\big[ [s \underline{\rho} t]  \  \mbox{ iff } \ [t \overline{\rho} s] \big]$.
\end{prop}
...
{\em $h(x)$ is not an {\em injection} from $\N^+$ to $\N^+$.}
  ...
  \medskip\item \index{post-inverse function}
...
\medskip\item
{\bf The equivalence of two formulations of {\sf SAT}}
...
{\em
Prove that the text's two views of the problem {\sf SAT} are equivalent, i.e., that they specify the same mathematical/computational problem.}
\begin{itemize}
\item
{\em Viewing a {\bf POS} expression as a {\em set} of clauses}
\medskip\item
{\em Viewing a {\bf POS} expression as a {\em disjunction} of clauses.}
\end{itemize}
\end{enumerate}
