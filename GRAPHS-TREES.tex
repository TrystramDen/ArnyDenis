...
problem, we discuss the so-called {\it Chinese Postman Problem}, which
focuses on ``almost Eulerian'' tours within graphs that do not admit
an Eulerian tour.
...
\hspace*{.5in}$\mbox{\sc indegree}(v) \ = \ \mbox{\sc outdegree}(v)$.
\end{prop}
...
leave this step to the reader, we do want to walk the reader through
the handing of ``small'' graphs.  Until we consider how we will deal
with arbitrary $n$-node graphs, we do not yet know how the general
step of our induction reduces the graph size $n$ at each step.
Because of this, it is a good practice to ``play'' with a few small
graph sizes.  Hopefully, in addition to giving our argument a robust
base case, it will give us valuable intuition for the general case.
\begin{itemize}
\item
When we focus on {\em undirected} graphs, we note that $2$-node graphs
...
\item
When we focus on {\em directed} graphs, we note that $2$-node graphs
can be Eulerian---as witnessed by the $2$-node digraph each of whose
nodes hosts a single arc that points to the other node.  Once again,
one can perform an exhaustive enumeration of $3$-node and $4$-node
graphs andverify that the Proposition correctly separates the Eulerian
ones from the non-Eulerian ones.
\end{itemize}

We now develop a complete proof of the {\em sufficiency} of the
conditions in part {\bf (a)} for general $n$-node undirected graphs.
Throughout the discussion, we intersperse hints regarding the
sufficiency of the conditions in part {\bf (b)}.

So, let us consider a connected multi-node undirected graph $\g$ all
of whose nodes have nonzero even degree.
\begin{description}
\item[{\bf Step 1}]
...
Choose some node of $\g$, call it $v_k$, and follow a walk along edges
of $\g$ beginning at $v_k$.

\item[{\bf Step 2}]
The rules of the walk beginning at $v_k$ are:
  \begin{itemize}
  \item
Each step of the walk will traverse an edge of $\g$ that has not been
used earlier in the walk.
  \item
The walk terminates when it encounters a node of $\g$ all of whose
edges have already been used.
  \end{itemize}
The facts that 

\hspace*{.25in}\begin{tabular}{ll}
(1) & each node of $\g$ has even degree; \\
(2) & the walk begins at node $v_k$; \\
(3) & no edge of $\g$ is traversed more than once
\end{tabular}

\noindent
mean that the last node encountered in this walk is $v_k$.
...
current walk, then we {\it go to {\bf Step 4}}, which invoke procedure
{\bf Build Eulerian Cycle} to stitch our series of walks into an
Eulerian cycle in $\g$.
...
\item[{\bf Step 4}]
We have reached this step because our sequence of walks, beginning,
respectively, at nodes $v_0$, $v_1$, \ldots, $v_{m-1}$ of $\g$, has
terminated with all of $\g$'s edges having been traversed.  We are now
ready to stitch together the $m$ walks into an Eulerian cycle in $\g$.
The resulting procedure {\bf Build Eulerian Cycle} proceeds
recursively as follows.

{\bf Build Eulerian Cycle}($v_k$)
\begin{itemize}
\item
Begin the walk associated with $v_k$ until it encounters $v_{k+1}$ for
the last time
\item
{\bf Build Eulerian Cycle}($v_{k+1}$)
\item
Complete the walk associated with $v_k$
\end{itemize}
\end{description}

When dealing with a {\em directed} graph $\h$, we proceed exactly as
with undirected graphs, with one critical difference: we always enter
a node $v$ of $\h$ along one of $v$'s {\em in-arcs}, and we always
exit a node $v$ of $\h$ along one of $v$'s {\em out-arcs}.  As in the
undirected case, an arc of $\h$ is traversed precisely once during the
entire described process.  \qed
\end{proof}
...
\subsubsection{The Chinese postman problem}
