...
$\q_n$ has diameter $n \ = \ \ln(|\n_{\fq_n}|)$.\footnote{Recall that
  $\ln n = \log_2 n$; see Section~\ref{sec:logarithmic-fns}.}  In
other words, $\q_n$'s diameter is {\em logarithmic} in its size.  This
fact makes $\q_n$ an efficient network for many tasks related to
...
that de Bruijn sequences exist for every $n$, so we provide two
examples---both to illustrate the phenomenon and to allow the reader
to work out how to generate such sequences.
\begin{table}[hbt]
\label{tab:deBruijn-seq}
\caption{Illustrating the order-$2$ and order-$3$ de Bruijn sequences.}
\[
\begin{array}{|l||l|c|}
\hline
...
Table~\ref{tab:deBruijn-seq} spawns many interesting questions: {\Arny
  Where did the crazy label for the table come from?  That's the
  section number, not the table number.}
\begin{itemize}
\item
...
directed graphs called {\it de Bruijn graphs} (or, {\it de Bruijn
  networks}).
...
$\n_{\fd_3}$ appear in Table~\ref{tab:deBruijn-seq}.

$\d_n$ is a regular directed graph; its nodes all have in-degree $2$
and outdegree-$2$.  The $2^{n+1}$ arcs of $\d_n$ come in pairs
specified as follows.  Each node of $\d_n$ is a binary string of
length $n \geq 1$; hence it can be written in the form $\beta x$,
where $\beta \in \{0, \ 1\}$ is a {\it bit} (short for {\it binary
  digit}) and $x$ is a length-$(n-1)$ binary string.  Each pair of
arcs of $\d_n$ have the forms:
\[ (\beta x \rightarrow x0) \ \ \ \mbox{ and } \ \ \ 
(\beta x \rightarrow x1)
\]
We illustrate $\a_{\fd_3}$ in Table~\ref{tab:deBruijn-arcs}.
{\Arny Again the label comes out wrong!}

\begin{table}[hbt]
\label{tab:deBruijn-arcs}
\caption{The arcs of the order-$3$ de Bruijn network $\d_3$.}
\[
\begin{array}{|ccccc|}
\hline
\mbox{\sc Node} & & {\sc Node} & & {\sc Node} \\
\hline \hline
{\displaystyle
\left.
\begin{array}{c}
...
Extrapolating from this example, we can show that $\d_n$ has diameter
$n$.  The structure of $\d_n$'s arcs allows one to ``rewrite'' any
(length-$n$) start-node $x$ to any desired (length-$n$) finish-node
$y$ bit by bit, i.e., in $n$ steps.  This is the best that one can
achieve, as witnessed by the situations when $x$ and $y$ are bitwise
complementary strings, as when $x = 00 \cdots 0$ and $y = 11 \cdots 1$.
