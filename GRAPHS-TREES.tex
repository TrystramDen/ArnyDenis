...
The reader will note that definition (\ref{eq:distance-defn}) has an
``escape clause''---specifically its third clause---which admits the
possibility that there is no path that connects node $u$ with node
$v$.  We say that a graph $\g$ is {\it connected}
\index{graphs!connected} if every pair of nodes $u, v \in \n_{\fg}$ is
connected by a path in $\g$---so that the ``escape clause'' never has
to be invoked.  If graph $\g$ is {\em not} connected, then it is the
disjoint union of some number $c$ of connected subgraphs, usually
called $\g$'s {\it (connected) components}.
\index{graphs!connected components}
Of course, $g$ is connected just when $c=1$; i.e., there is a single
connected component.
...
situations that are modeled using graphs.

\medskip

\index{graphs!matching}
A {\it matching} in an undirected graph $\g$ is a set of edges of $\g$
that have no nodes in common.
\index{graphs!perfect matching}
A matching in $\g$ is {\it perfect} if every node of $\g$ belongs to
some edge of the metching.
