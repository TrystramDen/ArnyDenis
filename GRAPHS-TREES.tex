...
The weaker notion of a {\it Hamiltonian path}
\index{graphs!Hamiltonian path} \index{Hamiltonian path} (defined in
the obvious way) is also of interest.  Hamiltonian paths can easily be
...
of such a cycle to obtain such a path.  However, there are graphs that
admit a Hamiltonian path that do not admit any Hamiltonian cycle!  The
reader can verify that there exist {\it mesh-graphs} (see
Section~\ref{sec:mesh-torus}) that admit no Hamiltonian cycle, even
though every mesh-graph admits a Hamiltonian path.

The existence of graphs that do not admit any Hamiltonian cycle have
spawned two paths of inquiry.  One has sought ``slight'' ways to
weaken the property of {\it hamiltonianicity} \index{graphs!hamiltonianicity}
in a way that encompasses more graphs.  The two major successes along
this path are encapsulated in the following result whose proof is
...
are at distance $\leq 3$ in $\g$ \cite{ChartrandK69}.
...
distance $\leq 2$ in $\h$ \cite{Fleischner74}.
\end{prop}
