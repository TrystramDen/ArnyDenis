...
connectivity (see Fig.~\ref{fig:tree} for an example of tree).  
As one would expect from the vernacular, a set of trees
is called a {\it forest}. \index{forest (of trees)} Just as with
graphs, there is a directed version of trees (Fig.~\ref{fig:outtree}). 
Within a directed tree
...
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/torus}
       \caption{A $4$ by $4$ torus.}
  \label{fig:torus}
\end{center}
\end{figure}
...
\end{eqnarray*}
The construction of the first order hypercubes is given in Fig.~\ref{fig:hypercube}.

\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/hypercube}
       \caption{Construction of the first order hypercubes: take two copies of the previous order and prefix each by 0 and 1.}
  \label{fig:hypercube}
\end{center}
\end{figure}
...
The hypercube $\q_4$ is \textit{isomorph} to the $(4,4)$ torus.
This can easily been checked while looking at the coding scheme in Fig.~{fig:toruslabel}.

\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/toruslabel}
       \caption{A coding scheme for labeling the vertices of the $(4,4)$ torus, which is isomorph to $\q_4$.}
...
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/dB2by2}
       \caption{de Bruijn with 4 vertices.}
  \label{fig:dB2by2}
\end{center}
\end{figure}
...
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/dB2by3}
       \caption{de Bruijn with 8 vertices.}
  \label{fig:dB2by3}
\end{center}
\end{figure}
...
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.7]{FiguresGraph/dBlabelEdge}
       \caption{Labeling the edges of a de Bruijn by concatenation of the vertex coding and then, compacting the intermediate bits (which are the same).
       In the figure, 01-11 becomes 011.}
...
\noindent {\bf (2)}
{\it Eulerian cycles}. \index{Eulerian cycle} A {\it directed Eulerian
  cycle} in a digraph $\g$ is a directed ( cycle that contains each
arc of $\g$ precisely once.  We will see, later in this chapter, a
truly elementary argument, based on node-degrees, which proves that
every de Bruijn digraph has a directed Eulerian cycle.  This
demonstration will combine with
...
{\it Eulerian cycles}.  A cycle in a graph $\g$ that traverses each of
$\g$'s edges precisely once is called an {\it Eulerian
  cycle},\index{graphs!Eulerian cycle} \index{Eulerian cycle} (or,
often, an {\it Eulerian circuit}).  \index{graphs!Eulerian circuit}
\index{Eulerian circuit} These edge-exhausting cycles were introduced
as a topic of study in 1736 by the Swiss mathematician \index{Euler,
  Leonhard} Leonhard Euler, whose name we have already encountered
multiple times.  Euler allegedly discovered the topic while
contemplating how to design a tour of the town of K\"{o}nigsberg that
would cross each of the town's bridges precisely once.  The quest for
Eulerian cycles in graphs is, thus, one of the oldest problems in the
fields now called {\it Operations Research} and {\it graph theory}.
(An edge-exhausting {\em path} in $\g$ is referred to in the obvious
analogous way.)  When one views an Eulerian cycle as a ``map'' for
traversing a graph---as did Euler when contemplating this
...
We turn first to the problems of determining when a connected
unweighted graph $\g$ has an {\it Eulerian cycle} or an {\it Eulerian
  path}---and the allied problem of finding such a cycle or such a
path when one exists.  Note that the problem of finding an Eulerian
cycle in a graph $\g$ is equivalent to the problem of drawing $\g$
without ever lifting one's pencil. \index{graphs!drawing without
  lifting pencil}

\subsection{Characterizing Eulerian Graphs}
...
\begin{prop}
\label{thm:euerian-cycle}
A graph is Eulerian if, and only if, it is connected and regular of
even degree.
\end{prop}

\begin{proof}
The necessity of the Proposition's conditions is established thus.
\begin{itemize}
\item
...
The simplicity---but not the elegance---of the preceding
characteriation degrades a trifle when one seeks Eulerian paths rather
than Eulerian cycles.
...
\paragraph{\small\sf C. Testing general graphs for Hamiltonianicity}
...
This short discussion is a perfect segu\'{e} to the material in the
next section---the Hamiltonianicity problem in weighted graphs.
