...
%Section~\ref{sec:Hamiltonian-unweighted}.C, 
...
edge-weighted graphs: the well-known, so-called {\it Traveling
  Salesman Problem}, often abbreviated {\it TSP}.
\index{Traveling Salesman Problem} \index{TSP: the Traveling Salesman Problem}

The {\it TSP} operates on a set of $n$ cities, $\{C_1, C_2, \ldots,
C_n\}$, together with a set of $\displaystyle {n \choose 2}$ real
numbers $\{c_{i,j} \ | \ 1 \leq i,j \leq n\}$.  The interpretation is
that each $c_{i,j}$ is the {\it cost} of traveling between cities
$C_i$ and $C_j$ (in either direction).  We employ the term ``cost''
here, rather than ``distance''---which is a commonly encountered
substitute for ``cost''---because the intercity costs are not assumed
to obey any of the laws that one commonly associates with the
distances we encounter in our daily lives.
\bigskip

\noindent \fbox{
\begin{minipage}{0.95\textwidth}
The {\it triangle inequality} is the prime example of such a law.
...
\noindent
(Some people like to think of our intercity costs as airfares---which
clearly have no logical basis.)  The challenge of the TSP is to
discover a minimum-cost tour of all $n$ cities.  Such a tour would
be a cycle of the form
...
\item
no tour has smaller cost than the cost
...
Hamiltonianicity-detection problem.  Within the representation, the
TSP's $n$ cities are depicted as an instance of the complete graph
...
{\em For any fixed positive constant $\kappa$, the problem of finding
  a tour for an instance of the  TSP that is within a factor of
  $\kappa$ of minimal is an {\sf NP}-complete problem.}
\end{enumerate}
...
form of the Hamiltonianicity-detection problem.  To see this
intuitively, let us encode a graph $\g$ into an instance of the TSP as
follows.
\begin{itemize}
\item
...
measure Euclidean ``distances''---then there exists a rather efficient
algorithm that {\em approximately} solves such TSP instances.
\index{Euclidean TSP problem}

**INTRODUCE EUCLIDEAN TERMINOLOGY above!!
