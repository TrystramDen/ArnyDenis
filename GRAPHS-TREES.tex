...
\noindent
We now provide a complete proof of the {\em sufficiency} of the
conditions in part {\bf (a)}, with hints interspersed regarding part
{\bf (b)}.  Our argument resides in the following ``streamlined''
induction.

We leave the reader the chore of verifying the sufficiency of the
Proposition's conditions for ``small'' graphs.  In the case of
undirected graphs, ``small'' begins with $3$-node graphs, because
$2$-node graphs cannot have even node-degrees.  In the case of
directed graphs, ``small'' begins with $2$-node graphs, because there
does exist the Eulerian $2$-node digraph each of whose nodes hosts a
single arc that points to the other node.

So, let us consider a connected multi-node undirected graph $\g$.
Choose some node $v$ of $\g$, and follow a walk of the following sort.
