...
We now show sketch a proof that every planar graph can be colored
using $5$ colors.  We settle for a proof sketch because of the
details of the required case analysis.  But, the {\em roadmap} of the
case analysis is a valuable lesson in how mathematics is really done!
...
\begin{proof}[Sketch]
We focus on a planar graph $\g$ and on a node $v \in \n_{\fg}$ of
smallest degree.  We consider the following cases.
\begin{description}
\item[{\sf Case 1}.]
...
In this case, we employ the coloring strategy of
Proposition~\ref{thm:P-6-colorability} to produce a $5$-coloring of
$\g$.  This is possible because, using the terminology of that
strategy, $d_v \leq 4$ in this case.

\smallskip

{\em Thus, when Case 1 holds, one can color $\g$ with $5$ colors.}
\end{description}
Note that, in all remaining cases, node $v$ has precisely $5$
neighbors---or else, we would have invoked Case 1 to color $\g$ with $5$
colors.
\begin{description}
\item[{\sf Case 2}.]  There is a coloring of $\g$ in which two or more
  neighbors of node $v$ get the same color.

\smallskip

Even though $v$ has precisely $5$ neighbors, in this case, only $4$
colors are used to color these neighbors.  This means that when we
invoke the coloring strategy of
Proposition~\ref{thm:P-6-colorability}---removing $v$, coloring the
rest of $\g$, and restoring $v$---we have a $5$th color available to
use on node $v$.  Thus, the described coloring strategy produces a
$5$-coloring of $\g$.

\smallskip

{\em Thus, when Case 2 holds, one can color $\g$ with $5$ colors.}
\end{description}
Note that, in all remaining cases, all $5$
neighbors of node $v$ have distinct colors under the coloring of $\g$
that we are focusing on.
\begin{description}
\item[{\sf Case 3}.]
Two neighbors of $v$, call them $v_1$ and $v_2$, reside in components
of $\g$ that are mutually disjoint once $v$ and its incident edges are
removed from $\g$.

\smallskip

For ease of discussion, let $\g'$ be the (disconnected) graph that
results from removing $v$ and its incident edges from $\g$.  Let
$\g_1$ be the component of $\g'$ that contains node $v_1$, and let
$\g_2$ be the component of $\g'$ that contains node $v_2$.

Say that, under the coloring of $\g$ that we are focusing on, node
$v_1$ is colored {\it red} and node $v_2$ is colored {\it green}.

Let us take subgraph $\g_1$ of $\g$ and recolor its nodes so that
under the new coloring, node $v_1$ is colored {\it green}.  (One needs
only switch the colors {\it red} and {\it green} in the existing
coloring of $\g_1$.)  It is always possible to do this in a way that
does not affect the valid coloring of $\g_2$ because $\g_1$ and $\g_2$
are mutually disjoint.

Once we have effected this recoloring of $\g_1$, we now have a valid
coloring of $\g$ for which Case 2 holds.  (In fact, we can color node
$v$ {\em red} when we reattach node $v$ and its incident edges.)

\smallskip

{\em Thus, when Case 3 holds, one can color $\g$ with $5$ colors.}

\item[{\sf Case 4}.]
The final case of the argument is when all neighbors of node $\g$
remain in the same component of $\g$ when $v$ and its incident edges
are removed.

\smallskip

Dealing with this case requires a rather intricate multi-subcase
analysis of the structure of paths within $\g$.  Each subcase
prescribes a way to recolor the nodes of one or more paths that
interconnect node $v$'s neighbors, in much the manner of Case 3's
mandated recoloring of one of the subgraphs of graph $\g'$.

Because the many subcases of this case expose no significant new ideas
or principles we merely refer the motivated reader to any of the
excellent expositions of this case available on the Web.  We note
only, in closing, that:

\smallskip

{\em In all possible cases, one can color $\g$ with $5$ colors.}
\end{description}
This four-case analysis completes the proof.
\qed
\end{proof}
...
The two problems we discuss in Section~\ref{sec:Relate-CS-Math-Probs}
