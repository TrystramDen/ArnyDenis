...
\]
We illustrate $\a_{\fd_3}$ in (\ref{eqn:deBruijn-arcs}).
\begin{equation}
...
showing that, for any two string-nodes of $\d_n$, say $u$ and $v$, one
can rewrite string $u$ as string $v$ by traversing a sequence of
arcs---i.e., a directed path---of length at most $n$.  Observe, for
instance, that the path in $\d_3$ described schematically as follows
...
For all $n \in \N^+$, $\d_n$ contains a {\em hamiltonian cycle}, i.e.,
a length-$2^n$ direct path that contains each node of $d_n$ precisely
once.
\end{prop}

\begin{proof}{Sketch}
The simplest proof of this result has two steps, both of which depend
on topics we develop later in this chapter:
\begin{itemize}
\item
{\it Eulerian cycles} \index{Eulerian cycle}

An {\it eulerian cycle} in a graph $\g$ is a cycle that contains each
edge of $\g$ precisely once.  This notion is named for its discoverer,
Leonhard Euler \index{Euler, Leonhard}
\item
{\it Line graphs} \index{line graph}

The {\it line graph} of a graph $\g$ is a graph $\l(\g)$.  The nodes
of $\l(\g)$ are the edges of $\g$; $\l(\g)$ contains an edge $(e_1,
e_2)$, for every pair of edges of $\g$, $e_1$ and $e_2$, that share an
endpoint.

The relevance of this topic here is that the line graph of every de
Bruijn network $\d_n$ is the de Bruijn network $\d_{n+1}$. 
\end{itemize}
A truly elementary argument, based on node-degrees will show that
every de Bruijn network has an eulerian cycle.  An invocation of the
second preceding bulleted item will then complete the proof.
\qed
\end{proof}
