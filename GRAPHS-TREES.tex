...
endpoints, as in the following path from node $u_1$ to node $u_n$:
\begin{equation}
\label{e.path}
...
The {\it (directed) distance}
\index{digraph!distance between two nodes}
from node $u_1$ to node $u_n$ in the digraph $\g$ is defined as
follows.
\begin{equation}
\label{eq:di-distance-defn}
 \mbox{distance}(u_1, u_n) \ = \ \left\{
\begin{array}{cl}
0 & \mbox{  if  } \ u_1 = u_n \\
n-1 & \mbox{  if there is a path } \ (\ref{e.path})
\ \mbox{ from $u_1$ to $u_n$} \\
\infty & \mbox{  if there is no path } \ (\ref{e.path})
\ \mbox{ from $u_1$ to $u_n$}
\end{array}
\right.
\end{equation}
The {\it directed diameter} 
\index{directed diameter (in a digraph)}
of a directed graph $\g$ is the largest quantity $q$ such that there
exist nodes $u_1, u_n \in \n_{\fg}$ having distance$(u_1, u_n) = q$.

\medskip

The notions of {\it undirected distance} and {\it undirected diameter}
...
if all nodes of $\g$ have the same degree.  Not surprisingly, there
are symmetric notion of {\it in-regularity}
\index{Graphs!directed!in-regular}
