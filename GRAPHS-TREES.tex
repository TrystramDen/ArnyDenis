...
common.  Happily, many problem involvings selecting matchings that
optimize with various criteria can be solved via efficient algorithms.
We discuss a few such criteria that model significant situations.

A variety of algorithmic problem related to finding matchings in
graphs have historically been called {\it marriage problems:}
\index{graphs!marriage problems} the insinuation is that each edge in
the matching unites (``marries'') the nodes that are the edge's
endpoints.  Within this context, a classical problem considers graphs
...
that adding any further
edge to the matching leaves one with a set of edges that is no longer
a matching.  The ``ultimate treasure'' in the world of matchings is a
matching that is {\it perfect}.  \index{graphs!perfect matching} A
matching in $\g$ is {\it perfect} if every node of $\g$ belongs to
some edge of the matching.

We leave to the reader the challenge of verifying the following


The reader can verify that, if a graph $\g$ admits a perfect matching,
then $|\n_{\fg}|$ must be even, and the cardinality of the matching is
exactly $\frac{1}{2}|\n_{\fg}|$.
{\Arny (Another good exercise)}
...
\index{graphs!spanning tree} \index{spanning tree}
...
\index{graphs!spanning forest} \index{spanning forest}
...
{\em minimum-weight spanning tree} \index{graphs!minimum-weight
  spanning tree} \index{minimum-weight spanning tree} (or, in the
...
{\it The mesh and torus networks}.
