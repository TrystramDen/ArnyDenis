...
\begin{proof}
We present a proof that is methodologically marvelous: it is primarily
a proof by contradiction, but it builds on an inner induction that
employs a sophisticated analysis by cases.

\smallskip

Let us assume, for contradiction, that the proposition is false, i.e.,
that there exists a planar graph that does not admit a {\em valid}
$5$-coloring, i.e., a coloring of the graph's nodes in which
...
\noindent {\em Base of an induction that will refute the presumed
...
non-$5$-colorable graph must have $\geq 6$ nodes.
...
If the proposition were, indeed, false, then there would exist a
planar graph $\g$ having $n+1$ nodes which is not $5$-colorable.  By
...
\noindent
Outside of Cases 1--3, we are left with the minimally constrained
situation:
\begin{itemize}
\item
