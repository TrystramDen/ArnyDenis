...
\label{sec:graph-fundamentals}

\subsubsection{An informal overview}
...
{\it directed edges}) $\a_{\fg}$. \index{$\a_{\fg}$: set of arcs of
  digraph $\g$} Each arc has the form $(u \rightarrow v)$, where $u, v
\in \n_{\fg}$; we say that this arc goes {\em from} $u$ {\em to} $v$.
A {\it path} \index{Graph!path in a digraph} in the digraph $\g$ is a
...
\subsection{Study of particular graphs}
