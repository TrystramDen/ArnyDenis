...
To garner intuition, we remark that $\k_4 = \k_{2 \cdot 2}$ can be
viewed as a $4$-cycle (i.e., nodes $1, 2, 3, 4$ with edges summarized
as $1-2-3-4-1$), augmented by two ``cross-edges'' (i.e., edges $\{1,
3\}$ and $\{2, 4\}$).  Easily, then, $\k_4$ admits $3$ different
perfect matchings, which can be identified (and specified) by the edge
that contains node $1$, i.e., either $\{ 1,2\}$ or $\{ 1,3\}$ or $\{
1,4\}$.  (Given the first edge, there is just one choice for the second
edge.)  Thus, $M_2=3$.

We jump now to any arbitrary $k > 2$.  We remark that there are
precisely $2k-1$ nodes of $\k_{2k}$ that node $1$ can ``choose'' as
its mate in a perfect matching.  Once we set node $1$ and its chosen
mate aside, we confront an independent instance of the problem with
parameter $k-1$, i.e., the problem of counting the number of perfect
matchings in $\k_{n-2} = \k_{2k-2}$.  We thereby note that as $k$
grows, the quantity $M_k$ obeys the following recurrence:
