...
\section*{Some Pervasive Symbols}

\begin{tabular}{c|cl}
{\bf Symbol} &  & {\bf Meaning} / {\bf Usage} \\ \hline
$|$
  & & this symbol means ``such that" or ``given that" in symbolic \\
  & & \hspace*{.45in}specification clauses \\
  & & (1) $\{x \ | \ P(x) \}$ means: the set of $x$ {\em such that} proposition \\
  & & \hspace*{.45in}$P(x)$ holds \\ 
  & & (2) $Pr[P_1(X,Y) \ | \ P_2(Y)]$ means: the probability of event \\
  & & \hspace*{.45in}$P_1(X,Y)$ {\em given that} event $P_2(Y)$ has occurred \\ \hline
\end{tabular}
