...
\item[{\sf Step} 2]
Find the rightmost position in $\alpha^{(00)}$ that consists of two consecutive $0$s.  Say that the portion of $\alpha^{(00)}$ beginning with the discovered two consecutive $0$s and proceeding rightward from there has the form
...
\item
A binary search (\cite{CLRS}) can accomplish this search in $O(\log n)$ steps.
\end{itemize}
