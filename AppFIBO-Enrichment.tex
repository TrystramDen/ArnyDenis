...
\section{Classical computations on the Fibonacci Numbers}
...
\noindent {\sf Base case.}
The expression holds for  $n=1$ since $F(0).F(2) = F(1)^2 +1 = 2$.
\medskip
...
\section{$\oplus$ Two advanced topics}

\subsection{Computing Fibonacci Numbers Fast}
\label{sec:FastFibo}

Using the bilinear recurrence equation needs $n$ steps to compute the n{\em th} Fibonacci number.
We are interested here in computing it faster.
This wis obtained by computing Fibonacci numbers by successive pairs. 
\medskip

\begin{enumerate} 
\item 
\textit{Verify the following expression.}
...
\begin{figure}[h]
\begin{center}
        \includegraphics[scale=0.35]{FiguresMaths/FiboFast.png}
...
\begin{figure}[h]
\begin{center}
        \includegraphics[scale=0.35]{FiguresMaths/FiboFast13.png}
...
\index{Zeckendorf, Edouard} \index{Zeckendorf's Theorem}
