...
number was named $i$ \index{imaginary number $i = \sqrt{-1}$} (for
...
arithmetic, the {\it complex number system}\index{number!complex} was
born.
...
positive integer $q$ such that $n = m \cdot q$.\index{number!integer!divisor} \index{number!integer!divisibility}\index{$m \mid n$: $m$ divides $n$}
\begin{itemize}
\item
$m$ {\it divides} $n$
\item
...
Greatest common divisors are fundamental entities.  How does one
compute them?  To understand this, we reopen our discussion of
divisibility with a wider perspective.
...
school, if one number $m$ does not ``evenly'' divide another $n$, then
it ``leaves a remainder'' when one divides $n$ by $m$.
