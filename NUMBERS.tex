...
easily show that {\em $\N_q$ is never closed under division when the
  modulus $q$ is composite.}


\subsubsection{Quotients exist within $\N_p$ for every prime $p$}
\label{sec:modular-quotientss}

Our main result in this section concerns congruences modulo a prime
number.  We prove that for every prime number $p$, every nonzero $n
\in \N_p$ has a {\em multiplicative inverse}, i.e., an element $m \in
\N_p$ such that $m \cdot n \equiv 1 \bmod p$.  \index{multiplicative
  inverse} Of course, the existence of multiplicative inverses allows
one to {\em divide} within $\N_p$---of course, using nonzero divisors.

\begin{prop}
\label{thm:finite-field}
For every prime number $p$, every nonzero $n \in \N_p$ has a
...
There do {\em not} exist nonzero $m_1$ and $m_2 \neq m_1$ in $\N_p$
such that $m_1 \cdot n \equiv m_2 \cdot n \bmod p$.
\end{lemma}
...
Because $\N_p$ has precisely $p-1$ nonzero elements, the multiples of
$n$ must exhaust these elements.  In other words, one of these
multiples, say $c \cdot n$, must equal $1$.  This element $c \in \N_p$
