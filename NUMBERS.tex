...
Give clock, circle as practical examples of inite number systems




\subsection{Congruences on Nonegative Integers}
\label{sec:congruences}

$\Z_n \ \eqdef \ \{0, 1, \ldots, n-1\}$

For nonnegative integers $m$ and $n$ and positive integer $a$, we say
that {\em $m$ is congruent to $n$ modulo $a$}, \index{congruence}
\index{congruent to} denoted
\[ m \equiv n \bmod a, \]
precisely when $a \mbox{ divides } |n-m|$.
...
integers $m$, $n$, and $q$ and positive integer $a$.

\begin{enumerate}
\item
Congruence is a {\em symmetric} relation on $\N$

Because $|m-n| = |n-m|$, the equalities $[a \mbox{ divides } |n-m|]$
and $[a \mbox{ divides } |n-m|]$ must hold simultaneously.

\item
Congruence is a {\em reflexive} relation on $\N$.

$m \equiv m \bmod a$ because every positive integer divides $m-m =
0$.

\item
Congruence is a {\em transitive} relation on $\N$

Say that $m \equiv n \bmod a$, so that $[a \mbox{ divides } |m-n|]$,
and $n \equiv q \bmod a$, so that $[a \mbox{ divides } |n-q|]$.  We
must consider the relative sizes of $m$, $n$, and $q$ in order to do
the relevant arithmetic.  We supply the details for the case $m > n >
q$ for definiteness, and we leave the other cases as exercises.

In the chosen case $m > n > q$, the two relevant congruences imply that
there exist integers $c_1$ and $c_2$ such that:
  \begin{enumerate}
  \item
$c_1 a = m-n$, which implies that $n = m - c_1 a$;
  \item
$c_2 a = n-q$, which implies that $n = q + c_2 a$.
  \end{enumerate}
We therefore have $q-m = (c2-c_1) a$, which means that $m \equiv q
\bmod a$.  In other words, The relation $equiv$ is transitive.
\end{enumerate}
The preceding three properties define an equivalence relation.
\qed
\end{proof}
...
Our main result in this section is that for every prime number $p$,
every nonzero $n \in \Z_p$ has a multiplicative inverse, i.e., an
element $m \in \Z_p$ such that $m \cdot n \equiv 1 \bmod p$.
\index{multiplicative inverse}

\begin{prop}
\label{sec:finite-field}
For every prime number $p$, every nonzero $n \in \Z_p$ has a
...
Focus on the set $\Z_p$ for some prime $p$.  Let $n$ be any nonzero
element of $\Z_p$.  

\begin{lemma}
\label{lem:multiples-in-Zp-unique}
There do not exist nonzero $m_1$ and $m_2 \neq m_1$ in $\Z_p$ such
that $m_1 \cdot n \equiv m_2 \cdot n \bmod p$.
\end{lemma}

\begin{proof} (of Lemma~\ref{lem:multiples-in-Zp-unique})
Assume for contradiction that there {\em do} exist $m_1$ and $m_2 \neq
m_1$ in $\Z_p$ such that $m_1 \cdot n \equiv m_2 \cdot n \bmod p$.
Say, with no loss of generality, that $m_1 > m_2$ within the set $\N$.
We must then have
\[ p | (m_1 - m_2) \cdot n. \]
The fact that $p$ is a prime ensures, by
Proposition~\ref{thm:p-divides-onefactor}, that $p$ divides either
$m_1 - m_2$ or $n$.  Because both of these integers lie strictly
between $0$ and $p-1$, this divisibility is impossible!  The lemma
follows.  \qed
\end{proof}

Because of Lemma~\ref{lem:multiples-in-Zp-unique}, all of the
following elements of $\Z_p$ are nonzero and distinct:
\[ 1 \cdot n, \ 2 \cdot n, \ldots, \ (p-1) \cdot n. \]
Because $\Z_p$ has precisely $p-1$ non zero elements, the multiples of
$n$ must exhaust these elements.  In other words, one of these
multiples, say $c \cdot n$, must equal $1$.  This element $c \in \Z_p$
is $n$'s multiplicative inverse.  \qed
\end{proof}
