...
\paragraph{\small\sf A. The Fundamental Theorem of Arithmetic}
...
different insights on the result.


\paragraph{\small\sf A. A proof using ``necklaces''}

{\Arny This proof appears in Wikipedia}
...
\item
The {\it (one-place) circular left shift} $c$ of a word in $S(\a,p)$ is
accomplished by placing the last symbol of this word into the first
position and shifting all other symbols one position rightward.
Formally:
...
Note that {\em the period of $\n(w)$ can never exceed $p-1$}.  This is
because an earlier-seen word must recur by the time the length-$p$
word $w$ has been shifted $p$ times.

Say next that the word $w$ is a replicate of another word $u$, i.e.,
$w = uu \cdots u$.  Let $u$ be the shortest such word; say that $u$
has length $m$.  Then, we must have: {\em The length $m$ of $u$
  divides $p$.}  This is obvious from our ability to write $w$ in the
indicated form.

When $w$ is a replicate of a word of length $m$, then {\em the period
  of $\n(w)$ is $m-1$.}  This is because, by the time one has shifted
$w$ $m$ times, one has transferred a copy of $u$ from the end of $w$
to the beginning.  Hence, one has recreated $w$.

In our special situation---where $w$ has prime length: {\em Because
  $p$ is prime, the only candidates for the replicated word $u$ in
  this case have length $1$ or $p$.}  In other words, one of the
following is true:

{\em The word $w$ has the form $w = \alpha \alpha \cdots \alpha$ for
  some symbol $\alpha \in \a$.}  This can occur in $a$ distinct ways.

{\em The word $w$ is not a replicate of any shorter word.}  This
situation must hold for all $a^p - a$ words over alphabet $\a$ that
contain at least $2$ distinct symbols.  Because the period of any
necklace $\n(w)$ for a word that contains at least $2$ distinct
symbols is, thus, exactly $p-1$, the lengths of such necklaces must be
exactly $p$.

This means that the $a^p - a$ words that contain at least $2$ distinct
symbols partition $S(\a, p)$ into disjoint sets of size $p$ each.
This means, however, that $p$ divides $a^p - a$.
\qed
\end{proof}
...
\end{center}
\end{figure}
The former figure depicts all necklaces $\n(w)$ over alphabet $\a = \{
A,B,C \}$ for length-$(p=3)$ words $w$ that use only a single letter;
the latter figure is the analogous depiction for words $w$ that use at
least two letters.


\paragraph{\small\sf B. A proof using Pascal's triangle}
