...
\[ \left\{ \langle i,j \rangle \ \ | \ \ i \in \{ n/2+1, n/2+2, \ldots, n \}, \ j \in \{1, 2, \ldots, n/4 \} \right\} \]
...
Recursively, each level-$\ell$ quadrant partitions into four level-$(\ell +1)$ sub-quadrants, and so on, down to sub-sub-\ldots-sub-quadrants that each contain only a single site.  
    \end{itemize}
    Fig.~ \ref{fig:routingCity} gives an illustration of the previous textual descrption.
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.5]{FiguresGraph/routingCity}
       \caption{Level partitions of the grid.}
