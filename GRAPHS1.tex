...
When graph $\g$ is {\em undirected},  each of its edges connotes some sort of sibling-like relationship among vertices of ``symmetric'' status.  When graph $\g$ is {\em directed} (often referred to as a {\em digraph}), each of its edges connotes an ``asymmetric'' relationship such as parenthood or priority or dependence; edges in directed graphs are often termed {\em arcs}. 
We denote the set of digraph $\g$'s arcs by $\a_{\fg}$.

\index{graph!arc} \index{arc (of a directed graph)}
...
Undirected graphs are usually the default concept, in the following sense: When $\g$ is described as a ``graph,'' with no qualification---i.e., no accompanying adjective ``directed'' or ``undirected''---it is understood that $\g$ is an {\em undirected} graph.
...
would expect from the vernacular, a set of trees is called a {\it forest}. \index{forest (of trees)}
\begin{figure}[hbt]
\begin{center}
...
\index{graph!cycle-free}
Mathematically speaking, a tree is a graph that contains no cycles, i.e., is {\it cycle-free}.  
...
As one might expect from the vernacular, a set of trees is called a {\it forest}. \index{forest (of trees)}
...
Observe, for instance, that the path in $\d_3$ described schematically as follows
...
yet encountered but will study in some detail in Section~\ref{sec:Hamiltonian-cycle}
\index{graph!Hamiltonian cycle} \index{Hamiltonian cycle}.

A {\it Hamiltonian cycle} in an $n$-vertex graph $\g$ is a length-$n$ cycle that contains 
every vertex of $\g$ precisely once.  A {\it directed Hamiltonian cycle} in an $n$-vertex 
digraph $\h$ is a length-$n$ directed cycle that contains every vertex of $\h$ precisely once.
We prove in Section~\ref{sec:hamiltonian-named-graphs} that every de Bruijn network is
directed-Hamiltonian; see Proposition~\ref{thm:named-graph-Hamiltonian}(e).
