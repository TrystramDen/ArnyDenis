...
The special graphs that appear in this chapter were invented---and reinvented---to model a large variety of real-life situations: inter-communication among functional units in multi-computers; the generation of information-rich codes; richly structured digital circuits, \ldots.  We embark here on the design and analysis of a traffic network within a city.

\smallskip

Before we analyze any costs, the complete graph $\k_N$ seems to provide an attractive traffic model for interconnecting $N$ sites: its diameter is tiny (in fact, unity!); routing is no problem; neither is link-directionality.  But, of course, the density of roads and vertex-degrees makes this an untenable option.  If you have driven an automobile in a city such as Catania, Sicily, then you appreciate the full range of problems engendered by any graph-oriented model that is too similar to $\k_N$.

One step away from the chaos of $\k_N$ would be to orient all of the edges of our abstract city map---i.e., to make each street unidirectional.  If this process is not executed with great care, then one could end up with a city which has {\em sinks}---sites, or neighborhoods, that lack exit roads.   If you have driven an automobile in a city such as Boston, Massachusetts, then you will empathize with this potential problem.

\medskip

Here is a {\em highly structured, recursive} proposal for a traffic network.  We will try to assess its costs.  To simplify our work, let us assume that $N$ has the form $n^2$ with $n = 2^{m}$. 
  \begin{itemize}
  \item
We arrange the city's $N$ sites as an $n \times n$ grid.  Thereby:
    \begin{itemize}
    \item
each site has coordinates $\langle i,j \rangle$, where $i, j \in \{1, 2, \ldots, n\}$.
    \item
the city partitions naturally into four identical quadrants (North, East, South, and West), each of which recursively partitions naturally into four sub-quadrants, and so on. 

\smallskip

The city's four quadrants are termed {\em level-$0$ quadrants}.  The four sub-quadrants of each level-$0$ quadrant are termed {\em level-$1$ quadrants}.  Recursively, the four sub-\ldots-sub-quadrants of each level-$i$ quadrant are termed {\em level-$(i+1)$ quadrants}.
    \end{itemize}
    
  \item
We eliminate (from consideration) all ``diagonal" edges.  Phrased positively: we restrict attention to edges whose endpoints share a coordinate, i.e., go from a site $\langle i,j \rangle$ to another site of the form $\langle i,k \rangle$ or the form $\langle h,j \rangle$.
...
We call an edge a {\em level-$0$ edge} if its endpoints are in different level-$0$ quadrants.  If both endpoints are in the same level-$0$ quadrant but in different sub-quadrants of that level-$0$ quadrant, then we call the edge a {\em level-$1$ edge}.  Proceeding the in natural way, we call an edge a {\em level-$i$ edge} if its endpoints are in different level-$\ell$ quadrants of a single level-$(i-1)$ quadrant.

  \item
We impose the following traffic regimen upon the travelers in our city.  In order to go from site $\langle i,j \rangle$ to site $\langle h,k \rangle$, we traverse the at-most three edges that lead site $\langle i,j \rangle$ to some site $\langle i_1,j_1 \rangle$ within the same quadrant as site $\langle h,k \rangle$.

\smallskip

From that point on, we recursively find the acceptable path from site $\langle i_i,j_1 \rangle$ to site $\langle h,k \rangle$.
  \end{itemize}
...
{\em What is the maximum cost of the trip from $\langle i,j \rangle$ to $\langle h,k \rangle$ within the metro model?}

  \item
In the {\em driving} version of the model, we pay a per-kilometer fee of $n^{{1/2}^\ell}$ every time we traverse a level-$\ell$ edge on the way from $\langle i,j \rangle$ to $\langle h,k \rangle$.
 
 \smallskip

{\em What is the maximum cost of the trip from $\langle i,j \rangle$ to $\langle h,k \rangle$ within the driving model?}
...
\noindent \textit{Hint.}
Describe both coding schemes (vertices and edges) successively. 
