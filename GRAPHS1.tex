...
\[ \Phi_1 \ = \ (a \vee \bar{b}) \ \wedge \
 (b \vee \bar{c}) \ \wedge \ (c \vee \bar{a})
\]
...
assignment $t$ for formula $\Phi_1$: $t(a) = t(b) = t(c) = \mbox{\sc true}$
or symmetrically $t(a) = t(b) = t(c) = \mbox{\sc false}$.
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.3\textwidth]{FiguresGraph/2SATyes.png}
\caption{The digraph $\g(\Phi_1)$ and its two
  strong components.}
...
\[ \Phi_2 \ = \ (a \vee \bar{b}) \ \wedge \
 (b \vee \bar{c}) \ \wedge \ (c \vee \bar{a})
 \ \wedge \ (a \vee c) \ \wedge \ (\bar{a} \vee \bar{c})
\]
Because the graph it strongly connected and contains a (directed) path from vertex $a$ to
vertex $\bar{a}$ and another one from $\bar{a}$ to $a$, Proposition~\ref{prop:2SAT} assures us that formula
