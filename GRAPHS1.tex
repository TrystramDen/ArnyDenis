...
Before we analyze any costs, the complete graph $\k_N$ seems to provide an attractive traffic model for interconnecting $N$ sites: its diameter is tiny (in fact, unity); routing is no problem; nor is link-directionality.  But, of course, the density of roads and vertex-degrees makes this an untenable option.  If you have driven in a city such as Catania, Sicily, then you appreciate the problems that accompany a graph-model that is too close to $\k_N$.

\smallskip

To step away from the chaos of $\k_N$ we could orient the edges of our abstract city map---i.e., make each street unidirectional.  If this process is not executed with great care, we could end up with a city which has {\em sinks}---sites that lack exit roads.   If you have driven in a city such as Boston, Massachusetts, then you will appreciate this potential problem.
...
\[ \left\{ \langle i,j \rangle \ \ | \ \ i, j \in \{1, 2, \ldots, n/4 \} \right\} \]

            \item
the {\em southwestern quadrant}, $SW$, comprises the sites
\[ \left\{ \langle i,j \rangle \ \ | \ \ i \in \{ n/4 +1, n/4+2, \ldots, n \}, \ j \in \{1, 2, \ldots, n/4 \} \right\} \]

            \item
the {\em northeastern quadrant}, $NE$, comprises the sites
\[ \left\{ \langle i,j \rangle \ \ | \ \ i \in \{1, 2, \ldots, n/4 \}, \ j \in \{ n/4 +1, n/4+2, \ldots, n \}  \right\} \]

            \item
the {\em southeastern quadrant}, $SE$, comprises the sites
\[ \left\{ \langle i,j \rangle \ \ | \ \ i, j \in \{ n/4 +1, n/4+2, \ldots, n \} \right\} \]
          \end{itemize}
