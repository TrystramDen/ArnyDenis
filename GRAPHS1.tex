...
It is easy to extrapolate from our discussion of {\em directed} paths
in {\em directed} graphs, as illustrated in (\ref{eq:di-path}), to an
anlogous notion of {\em undirected} path in an {\em undirected}
graph.  Informally, one derives each undirected  notion by removing
the arrowheads in from the arcs in a directed graph.  We leave details
to the reader.  Having thus converted a directed graph into an
undirected one, call it $\g$: The {\it distance between} vertex $u_1$ and vertex $u_n$ in $\g$ is the smallest number of edges in any path from $u_1$ to $u_n$.  In detail:
\begin{equation}
