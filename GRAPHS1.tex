...
A {\em path} in an undirected graph is a sequence of vertices within which
...
\index{graph!vertex}
\index{$\n_{\fg}$: set of vertices of graph $\g$}
...
endpoints, as in the following $(n-1)$-arc path from vertex $u_1$ to
vertex $u_n$ in $\g$:
\begin{equation}
\label{eq:di-path}
...
one can choose the first vertex in a pair in $n$ ways and then {\em
  independently} choose the second vertex in $n$ ways.
\medskip

\noindent {\bf (b)}
The stated number is the number of $2$-vertex subsets of $\n$.  To
...
|\e_{\cal G}|.
\]
The equation holds because each edge of $\g$ ``touches'' two vertices
...
We sometimes use the term {\it neighbor} within the context of {\em
  directed} graphs also.  If we say that vertices $u$ and $v$ are
...
is the number of vertices $v \in \n_{\fg}$ such that $(u \rightarrow v)$
is an arc of $\g$.
\item
Symmetrically, the {\it outdegree} of vertex $u \in \n_{\fg}$
