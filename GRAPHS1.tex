...
Here is a {\em highly structured, recursive} proposal for a traffic network.  The design may appear quite inefficient in terms of inter-site costs---but only a complete analysis will verify or refute that impression.  To simplify our work, let us assume that $N$ has the form $n^2$ with $n = 2^{m}$. 
  \begin{itemize}
  \item
...
\medskip\item
Recursively, each level-$\ell$ quadrant partitions into four level-$(\ell +1)$ sub-quadrants, and so on, down to sub-sub-$\cdots$-sub-quadrants that each contain only a single site.  
    \end{itemize}
Fig.~ \ref{fig:routingCity} illustrates the preceding textual description.
\begin{figure}[hbt]
\begin{center}
