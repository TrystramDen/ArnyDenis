...
In the course of analyzing a genre of search tree called {\it 2,3-trees},\footnote{These search trees are the lowest-index instances of the {\it B-trees} that have proved so useful in database implementations \cite{CLRS}.}~in [Miller, Pippenger, Rosenberg, Snyder, 1979] and [Rosenberg, Snyder, 1978], a new number sequence was discovered.  Named {\it Tree-Profile numbers} [Rosenberg, 1979], this family of positive integers was found to be a close relative of the family of
binomial coefficients, both in its defining recurrence and in the quite similar properties that the two families share.
...
\begin{center}
        ...
        \caption{Graphical construction of the triangle profile numbers (light shaded boxes correspond to initial known values).}
...
Let us assume that (\ref{eq:TP=sum-of-bincoeff}) holds for all $n$ up to (but not including) some integer $m$.  
...
\[
S(n) \ \ = \ \ 1 \ + \ 2 \cdot \sum_{k=0}^{2n-1}   \big( P(n,k) \ - \ 2^{k-n} {n \choose {k-n+1}} \big).
\]
