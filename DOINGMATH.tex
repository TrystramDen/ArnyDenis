...
\subsubsection{Operations on infinity}
...
{\Arny This story appears in SUMMATION.  Should we repeata sortened
  version here?  It's pretty short already}
...
\subsubsection{The Ross-Littlewood paradox}
\label{sec:littlewood}

The following story about balls and hats is known as the {\it
  Ross-Littlewood paradox}, after its creators.  A version of the
story appeared first in the enlightening and entertaining book {\it
  Littlewood's Miscellany} \cite{Littlewood-misc} \index{Littlewood,
  John E.}  The story was amplified to its present form by S.M.~Ross
\cite{Ross76} \index{Ross, Sheldon M.}

Let us consider a system that contains
\begin{itemize}
\item
a {\em really big} hat (as large as is needed for the story)
\item
an unboundedly long series of ordered balls,  labelled 1, 2, \ldots
\item
a {\em very precise} clock.
\end{itemize}
The process we observe proceeds as follows.

At midnight minus 1 minute, we place the first ten balls into the hat,
and we {\em immediately} remove the first ball.  We repeat this
process at midnight minus 30 seconds (put the 10 next balls into the
hat and remove the second one), then at midnight minus 45 seconds (put
the 10 next balls into the hat and remove the third one), and so on,
\ldots, decreasing at each step, by one-half, the number of seconds
remaining until midnight.

The question is, \textit{How many balls are present in the hat at midnight?}

This is a trick question: It has the aroma of a {\it paradox:} ``a
...
senseless, logically unacceptable, or self-contradictory.'' (Apple
dictionary)  To wit:

On the one hand, each step of the process inserts $10$ balls but
removes only $1$ ball.  It appears, then, that the number of balls
grows without bound.  In fact, this is true at every {\em
  finite-numbered} step: at step $k$, there are $N = 9k$ balls in the
hat.  It is intuitive, then, that, if we were actually able to reach
midnight---after infinitely many steps---then we would observe
infinitely many balls in the hat!

But this intuition is wrong!  In fact, at midnight {\em The hat is
  empty}.  This is because {\em every ball is eventually removed from
  the hat!}  Specifically, ball \# $n$ is removed at step
$\frac{1}{n}$, i.e., at time midnight minus $\frac{1}{n}$ seconds.
