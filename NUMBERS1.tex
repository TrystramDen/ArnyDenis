...
nuances will be appreciated by the reader only incrementally, through
Chapter~\ref{ch:Summation} (especially
...
on base $b$, a {\it real number} \index{number!real} is an {\em
  infinite} summation of the form
\begin{equation}
\label{eq:real-defn}
\begin{array}{l}
{\displaystyle
...
\begin{array}{ll}
\mbox{where: } & n \in \N \\
               & \mbox{each } \ \ \alpha_i, \beta_j \ \in \ \{0, 1, \ldots, b-1\}
\end{array}
\end{array}
\end{equation}
...
defines a finite number.  This proof can be crafted using the tools of 
Section~\ref{sec:geometric-sums}, but we will accept the fact on faith
