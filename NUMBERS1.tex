...
\begin{itemize}
\item
For any positive integers $n$ and $q$, there exist integers $s_0$ and $t_0$, not necessarily positive, such that
\[ s_0 n + t_0 q \ = \ \mbox{\sc gcd}(n, q) \]
\item
For all integers $s$ and $t$, not necessarily positive,
\[ s n + t q \ \geq \ s_0 n + t_0 q \]
\end{itemize}
\end{prop}

\begin{proof}
Let $m$ denote the smallest positive linear combination of $n$ and $q$.

We prove in turn that $m \geq$ {\sc gcd}$(n, q)$ and $m \leq$ {\sc gcd}$(n, q)$. 

\begin{enumerate}
\item 
By definition, {\sc gcd}$(n, q)$ divides $n$ and {\sc gcd}$(n, q)$ divides $q$, 
then, from the previous proposition~\ref{thm:m-common-divisor-nq} 
{\sc gcd}$(n, q)$ divides all the linear combinations $s \cdot n +t \cdot q$ for any $s$ and $t$ 
(and thus, in particular the smallest combination). 

{\sc gcd}$(n, q)$ divides $m$ and thus, $m \geq$ {\sc gcd}$(n, q)$.
\item
First, remark that $m \leq n$ because $n = 1.n + 0.q$ is a particular linear combination
and $m$ is the smallest positive one.
We show now that $m$ divides $n$. 

By the division theorem~\ref{thm:division-thm}, there exists a decomposition $n=k.m+r$ (where $0 \leq r<m$). 
Recall also that $m=s \cdot n+t \cdot q$ for some $s$ and $t$. 
Thus, a simple artithmetic manupilation shows that $r$ can be written as $(1-k \cdot s)n+(-k \cdot t)q$ 
which is a linear combination of $n$ and $q$. 

However, as $m$ is the smallest positive one, we get $r=0$.

Symmetrically, we show by exactly the same analysis that $m$ divides also $q$.

Then, $m \leq$ {\sc gcd}$(n, q)$.
\end{enumerate}
% \qed
