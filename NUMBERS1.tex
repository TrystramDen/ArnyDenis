...
\index{Gauss, Carl Friedrich!queen of the sciences, queen of mathematics}
...
\index{right angle} 
\index{right angle!$90$ degrees ($90^\circ$)}
...
\medskip\item
Square $\widehat{S}$, being built from four copies of the grey triangle, has area $4 \cdot (1/2) \ = \ 2$.
\end{itemize}
Because the hypotenuse of the grey triangle is a side of an area-$2$ square, we have just proved the following special case of the Pythagorean Theorem.   \qed
...
\caption{Octagons approximate circles much better than squares do}
...
\index{real number!non-rational}
\index{real number!irrational}
...
\index{positive integers!Well-Ordering Principle}
\index{Well-Ordering Principle for positive integers}
...
\medskip\item
The relation ``divides'' is {\em transitive}.\footnote{See Section~\ref{sec:order-relation}.}  Specifically, if $m$ divides $n$ and $n$ divides $q$ for some integer $q$, then $m$ divides $q$.
...
\item
For all integers $s$ and $t$, not necessarily positive,  such that $s n + t q > 0$, we have $s n + t q \ \geq \ s_0 n + t_0 q$.
\end{itemize}
\end{prop}
...
\index{number!rational!denominator} \index{number!fraction!denominator}
