...
\begin{enumerate}
\item
There are finite objects whose sizes can change dynamically or whose
sizes are known only approximately.  An example that enjoys both of
these characteristics are social networks.  Such networks expand and
contract in unpredictable manners, so one cannot know their population
statistics exactly.  One requires both a language for talking about
such objects and a system that supplies an ananalogue of arithmetic
for ``calculating'' with these statistics.  {\it Asymptotics}
(Section~\ref{sec:asymptotics}) affords us such a language and such a
system for a large variety of dynamic mathematical
phenomena---including the growth rates of numerical functions.
\item
We often have to reason about objects that are actually infinite.
Most commonly within the expected population of readers of this book,
such objects are some combination of
  \begin{itemize}
  \item
mathematical---e.g., classes of numbers or of functions
  \item
logical---e.g., aggregates of logical expressions with particular
properties, such as theorems or proofs
  \item
linguistic---e.g., collections of sentences that share some syntactic
or semantic characteristics, such as ``true'' sentences
  \end{itemize}
Even in antiquity---but increasingly beginning in the late $19$th
century---we have been confronted by instance after instance of
infinite objects that behave in very different ways than the finite
objects of daily discourse.  As just two examples:
  \begin{itemize}
  \item
We have had to explain why a shot arrow actually reaches its target,
despite a seemingly cogent argument shwing that it does not.
  \item
...
their intuitive coincidence.
  \end{itemize}
Section~\ref{sec:coping-infinity} provides the background necessary
to navigate the unfamiliar world of the infinite.
\end{enumerate}
...
{\em Reasoning qualitatively about quantitative phenomena.}
%
An often-underappreciated aspect of human reasoning is our ability to
abstract by blurring descriptions.  We notice, for instance, the ways
in which all phenomena that experience {\em linear} growth differ from
