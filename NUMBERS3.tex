...
\index{carry-free addition} We should clarify the term ``carry-free''
...
ones during an increment and from higher-order digits to lower-order
ones during an decrement.  Therefore, a more accurate term for these
adders would be something like {\em buffered carry}.  By changing the
form of numerals, one can design adders that buffer carries in a way
that achieves a bounded---i.e., $O(1)$---delay between button-pushes,
no matter how long you keep pushing a button or which one you choose
to push.

In order to achieve a ``carry-free'' adder for the counter, we have to
design the adder using a genre of {\it redundant number system},
\index{redundant number systems} known as a {\it signed-digit number
  system}.  \index{igned-digit number system}

**HERE

$\monus   \ \ \ \  a \monus b \ \ \ a \ominus b$
...
**HERE
adding $b$-ary numerals $x_n x_{n-1} \cdots x_1 x_0$ and $y_n y_{n-1}
\cdots y_1 y_0$: so each $x_i, y_i \in B_b = \{ 0, 1, \ldots \bar{b} \}$
 (a) via carry-ripple, (b) via carry-free.  We look at
the component of the adder that deals with digit $i$
...
$\iota_i$ \ \ and \ \ $t_{i+1}$ \\
via the relation \\
$b \cdot t_{i+1} + \iota_i \ = \ x_i + y_i$
\end{tabular}
} & & \\
  & & & \downarrow & & \\  
  & & & \iota_i & & \\
  & & & \downarrow & & \\ 
\mbox{\bf stage $2$:   } & & &  \fbox{Compute $\iota_i + t_i$} & \longleftarrow & t_i 
  \\
  & & & \downarrow & &
  \\
  & & & z_i & &
...
\end{array}
\]
\caption{Digit-adder $i$ of a $(b > 2)$-ary carry-free adder.  The outgoing
  {\it transfer digit} $t_{i+1}$ and the {\it interim sum} $\omega_i$
  are computed from the input digits $x_i$ and $y_i$, using the
  relation specified in the upper box.  These results are transmitted,
  respectively, to stage $1$ of digit-adder $i+1$ and stage $2$ of the current
  digit-adder $i$.}
...
There is no ``carry digit''.  In its place, there is a {\it transfer
  digit} $t_i$ \index{carry-free adder!transfer digits} which is
generated in stage $1$ of the adder and consumed in stage $2$.
\item
In addition to the transfer digits, there are {\it interim sum} digits
$\iota_i$, \index{carry-free adder!interim-sum digits} 
...
\begin{enumerate}
\item
computes {\it transfer digit} $t_{i+1}$ and {\it interim-sum digit}
$\iota_i$ via the equation
\[ b \cdot t_{i+1} + \iota_i \ = \ x_i + y_i \]
\item
transmits $t_{i+1}$ to digit-adder $i+1$
\item
transmits $\iota_i$ to stage $2$.
\end{enumerate}
\item
Stage $2$ of the adder
computes the $i$th output digit $z_i$ via by adding $\iota_i$ to $t_i$
(which came from digit-adder $i-1$)
\end{enumerate}
