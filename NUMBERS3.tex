...
\begin{quote}
``What's in a name?" \\
\hspace*{1in}William Shakespeare, {\it Romeo and Juliet}
\end{quote}
...
We employ the overline notation, ``$\overline{b-1}$'', to remind ourselves that ``$b-1$'' is a digit  here, not a string; e.g., when $b = 10$ (the {\em decimal}, or, $10$-{\it ary} base), $\overline{b-1}$ is the digit $9$.
\end{minipage}
}
...
\noindent
This numeral has the {\it numerical value}\footnote{The notation ``$\mbox{\sc val}_{b}(x)$'' in (\ref{eq:real-numeral-number}) denotes an operator that produces the {\em numerical value} of the base-$b$ numeral $x$.}
\begin{equation}
...
\begin{itemize}
\item
The numerical value of the integer part of the numeral in (\ref{eq:real-numeral}) is:
\[
...
\]
\item
The numerical value of the fractional part of the numeral in (\ref{eq:real-numeral}) is:
\[
...
To elaborate on our comment about ``simplifying notation":  
...
\index{diagonalization} \index{diagonalization argument}
...
\smallskip
 ...
 \index{Lucas, Edouard (Fran\c{c}ois Edouard Anatole Lucas)} \index{Flavius Josephus}
...
\begin{enumerate}
\item
Write $P(x)$ using the auxiliary expressions $C_i$.  {\em To simplify this task, say that the degree $d$ is a power of $2$.}
\item
Determine how many additions and multiplications are required to evaluate $P(x)$.
\end{enumerate}
}
