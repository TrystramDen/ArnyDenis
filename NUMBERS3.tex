...
\index{redundant number systems} then you can eliminate your annoyance
(almost) completely by designing your counter to implement {\it
  carry-free addition}.  This is a bit of a misnomer: adders of this
sort are not really {\em carry-free}; it is more accurate to say that
they implement carries in a {\em buffered} fashion so as to achieve a
bounded---i.e., $O(1)$---delay between button-pushes, no matter how
long you keep pushing the button.

Redundant number systems are positional systems of the same genre as
-ary systems: Their numerals are evaluated by the same {\sc val}
function as defined in (\ref{eq:real-numeral-number}).  These systems
achieve their bounded-delay, buffered, carries by augmenting their
sets of digits to allow {\em negative} digits.  The base-$b$ redundant
system has the following digit set; parentheses are added to enhance
legibility.
\[ \widehat{B}_b \ = \ \{-(\overline{b-1}), -(\overline{b-2}), \ldots,
-1, 0 , 1, \ldots, (\overline{b-2}), (\overline{b-1})\}
\]
If we focus on base $b=2$, then we note that $\widehat{B}_2 \ = \ \{
-1, 0, 1\}$.  The ``redundancy'' in this system is witnessed by the
fact that the number $1$ is the numerical value of {\em every} numeral
of the form
\[ 1 \ (-1) \ (-1) \ \cdots \ (-1) \ (-1), \]
...
number systems can be found in sources such as \cite{PhatakK94}.
