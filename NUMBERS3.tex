...
the set $\N^+$ of positive integers.  {\em The price that these
  ...
  the number $0$; they can represent all other integers.}

Each bijective base-$b$ system is sometimes called the {\it $b$-adic
  number system}. \index{positional number system!$b$-adic} One also
finds some -adic number systems being named using Greek-inspired names
for the base $b$, in imitation of the Latin-inspired names of -ary
...
discussions of {\it G\"{o}del numbers} \index{G\"{o}del numbers} in
...
\noindent 1. {\it Shortest numerals}.
We verify by direct enumeration that the result is true for numerals
having one or two digits.

\smallskip

\begin{tabular}{|c|c|}
\hline
{\bf Dyadic Numeral} & {\bf Numerical Value} \\
\hline
$1$  & $1$ \\
$2$  & $2$ \\
$11$ & $2 + 1 = 3$ \\
...
\noindent 2. {\it The largest and smallest $d$-digit numbers}.
We invoke the summation techniques in Section~\ref{sec:geometric-sums}
...
\item
The {\em largest} integer representable by a $d$-digit dyadic numeral:
...
\noindent  3. {\it The inductive hypothesis}.
Assume, for the sake of induction that all dyadic numerals of lengths
$\leq d$ represent distinct positive integers.

\medskip

\noindent  4. {\it Extending the induction}.
Let us be given distinct dyadic numerals,
\[ \gamma_r \gamma_{r-1} \cdots \gamma_1 \ \ \ \mbox{ and }  \ \ \
\delta_s \delta_{s-1} \cdots \delta_1 \]

4(a) {\it [$r > s$]}.
Say specifically that $r = s +c$ for some $c > 0$.
In this case, we have  **HERE
\[ \mbox{\sc dyadic-num}(\gamma_r \gamma_{r-1} \cdots \gamma_1) \ \geq
\ 2^{r+1} - 1 \]
while
\[ \mbox{\sc dyadic-num}(\delta_s \delta_{s-1} \cdots \delta_1) \ \leq
\ 2^{s+2} - 2 \]
...
\subsubsection{A system that enables {\em carry-free addition}}
\label{sec:carry-free}

\cite{Hwang79}

\subsection{$\oplus$ Systems Based on Special Number Families}
