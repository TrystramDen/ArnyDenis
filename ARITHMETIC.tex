...
{\Arny Can we get a picture of a slide rule?}

As a historical note, Proposition~\ref{thm:log-add-multiply} is the
mathematical underpinning of the analogue computer known as a {\it
  slide rule}. \index{slide rule}  Without going into details, a slide
rule is built from two straight sticks that are both marked off into a
logarithmic scale and that are attched in a way that allows each to
slide along the other (whence the name of the device).  When one
positions the origin of stick $A$ at the position of stick $B$ that
corresponds to number $a$, then the number 
is $a \times b$.
\[
...
\begin{array}{ccccccccccc}
  & \fbox{$1$} & \fbox{$b$} & \fbox{} & \fbox{$2$} & \fbox{} &
  \fbox{$3$} & \fbox{} & \fbox{$4$} & \fbox{}  & \fbox{$5$}  \\
  & \updownarrow & \updownarrow &   &   &   &   &   &   &   &  \\
\fbox{$1$} & \fbox{$a$} & \fbox{$a \cdot b$} & \fbox{$2$} & \fbox{} &
  \fbox{$3$} & \fbox{} & \fbox{$4$} & \fbox{}  & \fbox{$5$} &
\end{array}
\]
