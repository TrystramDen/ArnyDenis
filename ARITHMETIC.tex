...
\index{Roubaud, Jacques}

%``Il n'y a d'id\'{e}es math\'{e}matiques qu'ultimement calculables."
%
%\smallskip
%
%[There are no mathematical ideas save those which involve computing.]
%
%\smallskip
%
%
%\hfill Jacques Roubaud, in {\em Bords} (1997)
%\end{tabular}
...
\index{arithmetic!arithmetic identities, $0$ and $1$} 
\index{number!zero ($0$)!the additive identity} 
\index{number!one ($1$)!the multiplicative identity}
\index{$0$: zero!the additive identity} \index{$1$: one}
\index{$1$: one!the multiplicative identity}
...
%{\Arny I corrected the CE's correct observation.}
%.  It replaces a number $a$ by its negative, $-a$, whenever both $a$ and $-a$ belong to the nonnegative subset being operated on.
\end{itemize}
...
\noindent {\it i. The operations of {\em taking floors and ceilings}} are total operations on the sets $\N, \Z, \Q$, and $\R$.
\begin{itemize}
\item
...
\index{arithmetic!basic operations!absolute value}
\index{arithmetic!basic operations!magnitude}
\noindent {\it ii. The operation of taking {\em absolute values/magnitudes}}:
...
\index{number!identity!additive}\index{identity!additive}
...
\index{arithmetic!integer!successor} \index{arithmetic!integer!predecessor}
...
%{\Arny This is a lengthy response to a (correct) criticism by the CE}

\index{functional inverse}

\noindent {\em Functional inverses.}
...
\index{arithmetic!integer!successor and predecessor are mutually inverse}
...
\index{arithmetic!integer!additive inverse} 
\index{arithmetic!integer!addition and subtraction are mutually inverse}
...
\index{number!identity!multiplicative} \index{identity!multiplicative}
...
The reader has already shown---by proving Proposition~\ref{thm:uniqueONE}---that $1$ is the {\em unique} multiplicative identity.
...
\[ a \times 0 \ = \ 0 \]
Using reasoning which parallels that of Propositions~\ref{thm:unique-add-iden} and~\ref{thm:unique-mult-iden}, the reader can prove that $0$ is {\em the unique} multiplicative annihilator.
...
\index{arithmetic!integer!multiplicative inverse}
\index{arithmetic!integer!multiplication and division are mutually inverse}
...
%{\Arny I have edited this a lot --- especially regarding power 0.  I believe that this makes it stronger.  Let's talk.}
...
There are laws of arithmetic that deal with {\em imaginary}---hence, by extrapolation, also with {\em complex}---powers of numbers.  These laws are traditionally beyond the scope of an introductory text on discrete mathematics, so we shall not develop this topic here.

\smallskip

That said, we will need to refer briefly to the topic a few times---when we cite the landmark
Theorem~\ref{thm:Euler's-formula}, which exposes a wondrous relationship among the three fundamental constants $e$, $\pi$, and $i$---and when we briefly discuss the challenge of computing the roots of polynomials.
\end{minipage}
}
...
\smallskip
%
%{\Arny I just added the following sentence --- culture!}

{\em This note points out one of the most beautiful patterns in mathematics.}
...
The pattern we have just exposed is summarized by the following suggestive figure.
\[ 
\begin{array}{|ccccc|}
\hline
\mbox{addition}
...
Let $n$ and $k \leq n$ be nonnegative integers (i.e., elements of $\N$).  The {\it binomial coefficient} usually articulated as ``{\it $n$ choose $k$}'' is a number $m$ which is usually denoted by one of the following notations 
\[ {n \choose k}, \ \ \ \ C(n,k), \ \ \ \ \Delta_{n,k} \]
The number $m$ denoted by these notations is defined by the following equation, which invokes the notation that we use most frequently in this text.
\begin{equation}
\label{eq:binom-coeff}
m \ = \ 
{n \choose k} \ \eqdef \ \frac{n!}{k!(n-k)!} \ = \
\frac{n (n-1)  \cdots (n-k+1)}{\big(k  (k-1)  \cdots  1 \big)
\big((n-k)  (n-k-1) \cdots  1 \big)}
\end{equation}
...
\index{binomial coefficient!symmetry rule} \index{binomial coefficient!addition rule}
\begin{prop}
...
\index{Pascal, Blaise} \index{Pascal's Triangle}
...
The array in Fig.~\ref{fig:pascal-triangle} is usually attributed to Blaise Pascal in the West, with a date in the seventeenth century.  In fact, the array appears in {\it Siyuan Yujian} \cite{Zhu-1303},
a Chinese book from the early fourteenth century; the array is there attributed to Jia Xian in the eleventh century.  \index{Zhu Shijie} \index{Jia Xian}
\end{minipage} }
...
By invoking the preceding laws multiple times, we can derive a recipe for multiplying complicated arithmetic expressions.  We illustrate this via the ``simplest'' complicated expression, $(a+b) \cdot (c+d)$.
...
\index{algebraic function!monomial} \index{monomial} \index{monomial!powers}
...
The problem of finding the roots of given polynomials has garnered tremendous attention for centuries, for both its practical applications and its theoretical implications.  Historically, two of the major problems regarding roots of polynomials are:
\begin{itemize}
\item
...
\index{Diophantus of Alexandria!Diophantine analysis} \index{Diophantus of Alexandria}
...
\index{polynomial!univariate}

\index{radical} \index{quadratic polynomial} \index{polynomial!univariate!quadratic}
\index{cubic polynomial} \index{polynomial!univariate!cubic}
\index{quartic polynomial} \index{polynomial!univariate!quartic}
\index{quintic polynomial}\index{polynomial!univariate!quintic}
...
\index{Laplace, Pierre-Simon} \index{Argand, Jean-Robert}
...
Euler, Lagrange, and Laplace.  The theorem was finally proved in the early nineteenth century by the amateur(!)~French mathematician Jean-Robert Argand.  Argand publicized his proof in 1806 \cite{Argand}, but the proof became well known only when it appeared in the famous {\it Cours d'analyse} \cite{Cauchy21} of Augustin-Louis Cauchy.
...
\index{al-Khw$\bar{\mbox{a}}$rizm$\bar{\mbox{i}}$, Muhammad ibn M$\bar{\mbox{u}}$s$\bar{\mbox{a}}$!eponym of ``algorithm''} 
\index{al-Khw$\bar{\mbox{a}}$rizm$\bar{\mbox{i}}$, Muhammad ibn M$\bar{\mbox{u}}$s$\bar{\mbox{a}}$!Hindu-Arabic numerals}
...
\index{polynomial!solution by radicals}
\index{solution by radicals}  \index{radical!notation: radical sign}
\index{polynomial!quadratic} \index{polynomial!cubic} \index{polynomial!quartic}
...
\index{Galois, Evariste} \index{polynomial!quintic} \index{Galois theory} 
...
\index{Vi\`{e}te, Fran\c{c}ois (Vieta, Franciscus)}
...
\index{Euler, Leonhard!Euler's Formula} \index{Euler, Leonhard}
...
\index{Euler, Leonhard!Euler's constant}\index{$e$: the base of natural logarithms}
\end{itemize}
...
\end{equation}
\end{theorem}
\index{Binomial Theorem!binomial coefficients}
\index{binomial coefficient!the Binomial Theorem}
\index{polynomial!bivariate!the Binomial Theorem}
...
\medskip\item
One given integer, again say $z$, is the product of the other two ($z = x \times y$).
\end{itemize}
\end{minipage}
}
...
\index{Turing, Alan M.} \index{uncomputable function} \index{Cantor, Georg}
...
\index{$\ln(a)$: natural, base-$e$ logarithm of number $a$}
...
We leave to the reader the easy verification, from Eq.~(\ref{eq:logarithm-defn}), that the {\it base-$b$ logarithmic function} $f(x) \ = \ \log_b x$ is the functional inverse of the base-$b$ exponential function.

\subsection{Learning from Logarithms (and Exponentials)}

Definition (\ref{eq:logarithm-defn}) exposes and---even more importantly---explains many facts about logarithms which we often take for granted.
...
\index{Euclid!{\it The Elements}} \index{Euclid!Euclidean/$L^2$ measure!norm}
\index{$L^2$ measure!norm} \index{Euclidean measure!norm}
...
\end{prop}
  ...
  \medskip\item
Certain properties of polynomial $P_1(x)$, such as its dominant growth rate, are easier to analyze when one writes $P_1(x)$ in the following form.
\[ P_3(x) \ = \
x^d \cdot \left( a_d \ + \ \frac{a_{d-1}}{x} \ + \ \frac{a_{d-2}}{x^2} \ + \cdots + \ \frac{a_1}{x^{d-1}} \ + \ \frac{a_0}{x^d} \right)
\]
{\em Prove that $P_3(x) \ = \ P_1(x)$.}

  \medskip\item
Prove that the inequality (\ref{eq;bound-monic-roots}) holds for any monic polynomial $P(x)$ with real coefficients.
  \end{enumerate}
...
\medskip\item
{\bf An identity for binomial coefficients}

{\sc Lesson:} Experience with algebraic manipulation
...
\begin{prop}
For all $n$ and all $k \in \{ 1, \ldots, n\}$
