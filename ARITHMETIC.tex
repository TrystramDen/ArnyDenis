...
We thus have both of the proposition's rules for binomial coefficients.
\qed
\end{proof}
...
\end{figure}
The formation rule of the array is as follows. \index{Pascal's triangle!frmation rule}

\smallskip
\noindent {\small\sf Formation rule for Pascal's triangle.}
...
\end{itemize}
We shall, therefore, see a lot more about binomial coefficients
throughout the text; see, among other places,
Sections~\ref{sec:powers+polynolmials} and~\ref{sec:arithmetic-series}
and Chapter~\ref{ch:prob-stat}.  With each subsequent encounter, our
respect for these numbers will grow.


\subsection{The Laws of Arithmetic, with Applications}
...
\subsubsection{The commutative, associative, and distributive laws} 
...
\subsubsection{A fun result: A ``trick'' for squaring some integers}
...
(abbreviated via the shorthand operator $\pm$). \index{$\pm$: plus or
  minus}
\begin{equation}
