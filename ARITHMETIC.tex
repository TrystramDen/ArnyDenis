...
invent the {\it rational numbers} \index{number!rational}
%
...
%
We survey a number of the most important properties of:
\begin{itemize}
\item
the set denoted $\Z$
\index{$\Z$: the set of all integers}
which comprises {\em all integers}---the positive and negative integers
and zero ($0$);
\item
the set denoted $\N$
\index{$\N$: the set of nonnegative integers}
which comprises the {\em nonnegative integers}---the positive integers
and $0$;
\item
the set denoted $\N^+$
\index{$\N^+$: the set of positive integers}
which comprises the {\em positive integers}.
\end{itemize}
Several essential properties of these three sets are consequences of
their behaviors under their natural order relations:
\begin{itemize}
\item
the {\em strong, or strict, less-than} relation ($<$) and the {\em
  weak less-than} relation ($\leq$)
\index{$<$: the strong less-than relation}
\index{$<$: the strict less-than relation}
\index{$\leq$: the weak less-than relation}
\item
their {\em converses:} the {\em strong, or strict, greater-than}
relation ($>$) and the {\em weak greater-than} relation ($\geq$)
\index{$>$: the strong greater-than relation}
\index{$>$: the strict greater-than relation}
\index{$\geq$: the weak greater-than relation}
\end{itemize}
Indeed, order within a number system is among one's biggest friends
when reasoning about the numbers within the system.
...
Paraphrasing an oft-used quip by the stand-up comic Rodney
...
For instance, let us consider {\em ordered pairs}
\index{integer!ordered pairs}
of integers---the sets $\Z \times \Z$, $\N \times \N$, and $\N^+
\times \N^+$.
\begin{quote}
{\em The set $\Z \times \Z$, for instance, consists of elements
  $\langle a_1, a_2 \rangle$, where $a_1$ and $a_2$ belong to $\Z$.
  We say that $a_1 = \mbox{first }(\langle a_1, a_2 \rangle)$, while
  $a_2 = \mbox{second }(\langle a_1, a_2 \rangle)$. }
\end{quote}
then, among other structures, one can represent:
\begin{itemize}
\item
{\em ordered $k$-tuples of integers}
\index{tuples of integers}
\index{integer!tuples}
\index{tuples of integers!via ordered pairs}
\index{integer!tuples!via ordered pairs}

One way to accomplish this is by recursion, with {\em ordered pair} as
the base case.  We then represent the ordered $k$-tuple $\langle a_1,
a_2, \ldots, a_k \rangle$ as the ordered pair whose {\em first} is the
ordered $(k-1)$-tuple $\langle a_1, a_2, \ldots, a_{k-1} \rangle$ and
whose {\em second} is the integer $a_k$:
\[ \langle a_1, a_2, \ldots, a_k \rangle \ \ = \ \
\langle \langle a_1, a_2, \ldots, a_{k-1} \rangle, a_k \rangle \]

\item
{\em strings of integers}
\index{integer!string}
\index{string of integers}
\index{integer!string!via ordered pairs}
\index{string of integers!via ordered pairs}

One way to represent the string $a_1 a_2 \cdots a_n$ using ordered
pairs is:
\[ a_1 a_2 \cdots a_n \ \ = \ \
...
\index{complete binary tree!via ordered pairs}
\index{binary tree!complete!via ordered pairs}
...
\index{comb-structured binary tree}
\index{comb-structured binary tree!via ordered pairs}
...
objects---for instance a length-$4$ string and a $4$-leaf
comb-structured binary tree.  Indeed, one of the biggest strengths of
mathematics is its ability to expose often-unexpected structural
similarities.  }
\end{quote}
...
and $\N^+$---in the eyes of the readers.

\medskip

\noindent
{\em We develop {\em easily computed} bijections (cf.,
  Chapter~\ref{sec:function}) between any of the sets $\Z \times \Z$,
  $\N \times \N$, or $\N^+ \times \N^+$ and the set $\N^+$ of positive
  integers.}

\smallskip

\noindent
{\em We thereby exhibit easily computed mechanisms for \underline{encoding}
  ordered pairs of integers---hence, also, strings and binary trees of
  integers---as single integers.  }

\smallskip

\noindent
{\em An extra benefit of our bijections is that they endow our sets of
  ordered pairs with natural, easily computed, \underline{linear
    orders}.  (Of course, our sets of ordered pairs do not have native
  natural linear orders.)  }
...
system is among one's biggest friends when reasoning about the
