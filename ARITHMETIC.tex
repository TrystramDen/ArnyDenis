...
\subsubsection{Comparing $\Z$ and $\Q$ via the laws they obey}
\label{sec:Q-Z-laws}
...
\subsubsection{Comparing $\Z$ and $\Q$ via cardinalities}
...
Even harder question: Should we present (prove?) the
Schroeder-Bernstein theorem, which states: If there is an injection
from $A$ to $B$ {\em and} an injection from $B$ to $A$, then there is
a {\em bijection} between $A$ and $B$.  If we decide to include
that,then itwould likely go into the chapter on SETS.}
...
Note that the {\it pairing functions} of Section~\ref{sec:pairing}
provide somewhat more interesting alternatives to part 2 of the
preceding proof.
