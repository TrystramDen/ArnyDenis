...
\end{eqnarray}
The absence of a quadratic term in $P^{(2)}(y)$---i.e., a term
involving $y^2$---defines the polynomial's being {\it in reduced
  form}.  For simplicity, we rewrite $P^{(2)}(y)$ in the form
\begin{equation}
...
\noindent {\it Step 3.} Convert $P^{(2)}(y)$ to its associated
...
Deriving all three solutions for the variable $z$ in equation
(\ref{eq:solve-cubic-for-z3}) requires a bit of background.
Theorem~\ref{thm:fund-thm-algebra} assures us that the equation has
three roots.  In order to compute these roots, we must invoke a truly
remarkable result that is known as \index{Euler's formula} {\it
  Euler's formula}, in honor of its discoverer, the much-traveled
$18$th-century mathematician Leonhard Euler. \index{Euler, Leonhard}
This result/formula exposes a fundamental relationship among:
\begin{itemize}
\item
