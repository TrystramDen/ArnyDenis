...
%  $\vdots$ & $\vdots$ & $\ddots$
%\end{tabular}
  %MISSING      \includegraphics[scale=0.4]{FiguresArithmetic/PairingSquareShell}
\end{center}
...
\noindent {\rm (a)} There exists an injection from $\N$ to $\N \times
\N$.  Therefore, $\N \times \N$ is at least as big as $\N$.

\noindent {\rm (b)} There exists an injection from $\N \times \N$ to $\N$.
Therefore, $\N$ is at least as big as $\N \times N$.

\medskip

(a) Part (a) is clear via the following injection from $\N$ into $\N
\times \N$.
...
\noindent (b)
We define an injection from $\N \times \N$ into $\N$.

Inspired by Theorem~\ref{thm:Fund-Thm-Arith}, we now consider the
function
\[ f_2(p,q) \ \eqdef \ 2^p 3^q. \]

Thus defined, $f_2$ is a function from $\N \times \N$ into $\N$.
Moreover, Theorem~\ref{thm:Fund-Thm-Arith} guarantees that $f_2$ maps
$\N \times \N$ {\em one-to-one} into $\Z$.
...
interesting alternatives to the preceding proof.  Being {\em
  bijections} between $\N$ and $\N \times \N$, these mappings can be
adapted to prove Proposition~\ref{thm:|NxN|=|N|} {\em in a single
  step}.
