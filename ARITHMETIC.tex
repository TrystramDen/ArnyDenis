...
even for moderately low degrees, in a sense that can be made precise
but is beyond the scope of this text.  For polynomials of low degree,
there do exist computationally feasible algorithms for finding roots.
For polynomials of {\em very} low degree---such as degrees $2$ and
$3$, there actually exist {\em formulas} that specify the $2$ or $3$
roots, respectively.  This section is devoted to deriving the {\em
  quadratic} formula, i.e., the one that works for degree-$2$---{\it
  quadratic}---polynomials. \index{polynomials!quadratic}


**DEFINE solution by radicals
...
\paragraph{\small\sf B. Solving {\em quadratic}, {\em cubic}, and {\em
    quartic} polynomials by radicals} 
\label{sec:solve-quadratic-cubic-quartic}

We do only quadratic and cubic, leaving quartic as an exercise.
Quartic is not so hard conceptually, but it is hard clerically.

\noindent {\it i. Solving quadratic polynomials by radicals}

We derive a formula that {\it solves} an arbitrary quadratic polynomial
\index{Polynomial!quadratic!generic} with real coefficients:
\begin{equation}
\label{eq:generic-quadratic-1}
P(x) \ = \  ax^2 \ + \ bx \ + \ c \ \ \mbox{  where  } \ b,c \in \R;
\ a \in \R \setminus \{0\}
\end{equation}
\index{polynomial!solving a quadratic polynomial} 
While the formula and its derivation are specialized to the structure
of quadratic polynomials, the interested reader will observe aspects
of the derivation that can be extrapolated to polynomials of higher
degree.  The so-called {\em quadratic formula} that we derive is
announced in the following proposition.  \index{quadratic formula}

\begin{prop}
\label{thm:quadratic-formula}
The two roots of the generic quadratic polynomial
(\ref{eq:generic-quadratic-1}) are specified as follows.
\begin{equation}
\label{eq:generic-quadratic-4}
x \ = \  \frac{1}{2a} \left( -b \ \pm \
\sqrt{b^2 \ - \ 4ac} \right).
\end{equation}
\end{prop}
...
\begin{proof}
The problem of finding the roots of $P(x)$ is equivalent to the probem
of solving the polynomial equation $P(x) = 0$.  By elementary
calculation on expression (\ref{eq:generic-quadratic-1}), we reduce
the root-finding problem to the solution of the equation
\begin{equation}
...
technique mandates adding to both sides of the equation a
...
\label{eq:generic-quadratic-3}
  & = & \frac{1}{4a^2} \cdot (b^2 - 4ac).
\end{eqnarray}
An elementary algebraic calculation shows that equation
(\ref{eq:generic-quadratic-3}) identifies $P(x)$'s two roots, call
them $x_1$ and $x_2$:
\begin{eqnarray*}
x_1 & = & \frac{-b \ + \ \sqrt{b^2 -4ac}}{2a} \\
x_2 & = & \frac{-b \ - \ \sqrt{b^2 -4ac}}{2a}.
\end{eqnarray*}
The roots $x_1$ and $x_2$ are usually specified via the shorthand expression
(\ref{eq:generic-quadratic-4}), which is the well-known
{\it the quadratic formula}. \index{the quadratic formula}
\qed
\end{proof}

\smallskip

The reader can verify that our proof essentially proceeded by
...
of the cubic polynomial, we get around some of the computational
...
\end{equation}
While the formula and its derivation are specialized to the structure
of cubic polynomials, the interested reader should recognize aspects
of the derivation that are akin to our derivation of the quadratic
formula.  Because the so-called {\em cubic formula} that we derive is
so complex in form, we are going to present the proof/derivation {\em
...
D \ = \ \frac{d}{a}
\end{eqnarray}
One verifies easily that the polynomials $P(x)$ and $P^{(1)}(x)$ have
the same roots.
...
\end{eqnarray}
The absence of the quadratic term in $P^{(2)}(y)$---i.e., the term
involving $y^2$---defines the polynomial's being {\it in reduced
  form}.  For simplicity, let us rewrite $P^{(2)}(y)$ in the form
\begin{equation}
...
\noindent {\it Step 3.} Convert $P^{(2)}(y)$ to a quadratic polynomial.

\noindent
We apply a transformation attributed to Vieta ({\em CITATION, INDEX})
to convert $P^{(2)}(y)$ to a quadratic polynomial.  This transformaton
makes the variable-substitution
\begin{equation}
\label{eq:cubic-substitute-z-for-y}
y \ = \ z \ - \ \frac{E}{3z}
\end{equation}
in (\ref{eq:generic-cubic-4}).  We thereby obtain (after calculation
and cancellations)
\begin{eqnarray}
\nonumber
...
Let us view this equation as a quadratic equation in the ``variable''
$z^3$.  (We wrote both instances of $z^3$ within parentheses to
facilitate this view.)  The quadratic formula
(\ref{eq:generic-quadratic-4}) allows us to solve this equation for
$z^3$, yielding the following two solutions (abbreviated via the
...
Solving equation (\ref{eq:solve-cubic-for-z3}) for the variable $z$
requires a bit of background.  Theorem~\ref{thm:fund-thm-algebra}
assures us that the equation has three roots.  In order to complete
equation (\ref{eq:solve-cubic-for-z3})'s basis for these solutions, we
must employ a truly remarkable result that is known as \index{Euler's
  formula} {\it Euler's formula}, in honor of its discoverer, the
much-traveled $18$th-century mathematician Leonhard
Euler. \index{Euler, Leonhard}

\begin{theorem}[Euler's formula]
\label{thm:Euler's-formula}
\[ e^{i \pi} \ = \ -1, \]
where $i$ is the imaginary unit, \index{$i$: the imaginary unit} and
$e$ denotes Euler's constant, \index{Euler's constant} $e \ =
\ 2.718281828 \cdots$.
\end{theorem}
...
distinct roots; these numbers are traditionally called the {\em
  ...
  unity} and denoted $\omega_0$, $\omega_1$, and $\omega_2$.
Theorem~\ref{thm:Euler's-formula} identifies these numbers as:
\[ \omega_0 \ = \ 1; \ \ \ \ \
\omega_1 \ = \ e^{-i \pi/3}; \ \ \ \ \
\omega_2 \ = \ e^{-2i \pi/3}.
\]
...
\begin{equation}
\label{eq:cubic-solution}
\begin{array}{ccrrrccr}
z_1 & = &
{\displaystyle
\left[ \frac{1}{2} \left(-F \ + \ \sqrt{F^2 + 4 \frac{E^3}{27}}
\right) \right]^{1/3} 
}
  & & &
z_2 & = &
{\displaystyle
\left[
\frac{1}{2} \left(-F \ - \ \sqrt{F^2 + 4 \frac{E^3}{27}}
\right) \right]^{1/3}
} \\
z_3 & = &
{\displaystyle
...
these solutions for $z$, then we find that we find only {\em three}
solutions for $y$.
...
proposition.  \index{cubic formula}

\begin{prop}
\label{thm:cubic-formula}
The three roots of the generic cubic polynomial
(\ref{eq:generic-cubic-1}) are specified as follows.
\begin{equation}
\label{eq:generic-cubic}
x \ = \  \frac{1}{2a} \left( -b \ \pm \
\sqrt{b^2 \ - \ 4ac} \right).
\end{equation}
\end{prop}
...
\paragraph{\small\sf B. Solving general cubic polynomials}
