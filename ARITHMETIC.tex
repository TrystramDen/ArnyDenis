...
\begin{itemize}
\item
is a {\em total function} on the sets $\Q \ \{0\}$, $\R \ \{0\}$, and $\C \ \{0\}$; it replaces each number $a$ by its {\em reciprocal}, which is a number from the same class.
...
\sqrt{b^2 + c^2} &  \mbox{ if } \ \big[a \in \C-\R\big]  \ \ \mbox{ and }  \ \ \big[a = (b+ci)\big]
\end{array}
\right.
\]
...
\index{Euclid!{\it The Elements}} \index{Euclid!the euclidean or $L^2$ measure/nrm}
\index{$L^2$ measure/norm} \index{eucidean measure/norm}
