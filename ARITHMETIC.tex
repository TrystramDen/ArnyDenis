...
{\Arny Please edit the figure to follow this story}
...
vertex $A$ to vertex $C$ are called the {\em sides} \index{side of a
  right triangle} of $T$, while the line from vertex $B$ to vertex $C$
is called the {\em hypotenuse} \index{hypotenuse of a right triangle}
of $T$.  Triangle $T$ is {\em isosceles} \index{isosceles triangle}
precisely when its two sides have the same length.  The grey triangle
in Fig.~\ref{fig:unitsquare} is an isosceles right triangle.
...
rational \index{integers as rationals} number, namely, the rational
...
\begin{prop}
\label{thm:integer-iff-finite-numeral}
A number $n$ is an integer if, and only if, it has a finite numeral in
some base-$b$ positional number system.
\end{prop}



DEFINE ULTIMATELY PERIODIC
