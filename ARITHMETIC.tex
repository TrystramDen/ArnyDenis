...
{\Denis Here is a detailed (geometric) proof of the result.
...
geometrical proof.  
Assume like in the previous proof that $\sqrt{2}$ is rational, which means 
\[ \sqrt{2} \ = \ {a \over b} \]
for positive integers $a$ and $b$ and among all such possible pairs of numerators and denominators, 
$a$ and $b$ are those that donot share common prime factors. 
Notice also that $a > b$.
In other words, the fraction is the {\em simplified} one. 
Fig.~\ref{fig:irrationality1} gives a pictorial view of the equation $a^2 = 2 b^2$. 
Let put both little $b$ by $b$ squares into the bigger $a$ by $a$ square as shown in Fig.~\ref{fig:irrationality2}. 
The overlapping surface (in dark grey) is a square whose surface is equal to the
two smaller white squares because the initial surfaces are the same. 
More precisely, the side of the grey square is $2b-a$ while the side of the white ones is $a-b$.
This process showed a way to construct a similar figure of a big square equal to two smaller squares.
\[ \sqrt{2} \ = \ {a \over b} \ = \ {2b-a \over a-b} \]
both terms are strictly smaller than $a$ and $b$.
This contradicts the previous assumption assuming that $a \over b$ is the simplified fraction. 

\begin{figure}[htb]
\begin{center}
       \includegraphics[scale=0.4]{FiguresArithmetic/sqrt2initial}
\caption{{\it Geometric traduction of the basic equation $a^2 = b^2 + b^2$.}
...
\paragraph{\small\sf B. A consequence of the refined goal: a bijection
  $h: \N \leftrightarrow \R_{(0,1)}$}
...
\paragraph{\small\sf C. A ``problem'':  bijection $h$ ``missed'' some
  $x \in \R_{(0,1)}$}
...
\paragraph{\small\sf D. The denouement: The set  $\R_{(0,1)}$ is not countable}
