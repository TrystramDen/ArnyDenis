...
We begin to study the conceptof integer divisibility by considering a
variety of simple, yet significant, consequences of an integer $n$'s
being divisible by an integer $n$.  We leave the following
applications of the basic definitions as exercises for the reader.
Say that $m \mid n$.  Then:
\begin{enumerate}
\item
$m \mid cn$ for all integers $c$.
\item
If, moreover, $n \mid c$ for some integer $c$, then $n \mid c$.
\item
If, moreover, $m \mid (n+c)$ for some integer $c$, then $m \mid c$.
\item 
For any nonzero integer $c \neq 0$, $\left[[m \mid n] \ \ \mbox{ if, and
    only if} \ \  [ac \mid bc] \right]$.
\end{enumerate}

The following result now follows from the preceding facts.

\begin{prop}
If $m \mid n$ and $m \mid c$, then $m \mid (sn+tc)$ for all integers
$s$ and $t$.
\end{prop}
...
= c$.  We therefore have $(k_1 \cdot s + k_2 \cdot t)m \ = \ sn+tc$
for any $s$ and $t$.  \qed
\end{proof}
