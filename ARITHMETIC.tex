...
%
which is a {\em finite} string of digits from $B_b$.  We denote such
an integer-part string as: $\alpha_n \alpha_{n-1} \cdots \alpha_1
\alpha_0$.
...
. \beta_0 \beta_1 \beta_2 \cdots
\end{equation}
where the $\alpha_i$ and the $\beta_j$ are elements \index{positional
  number system!base-$b$ digits} {\em base-$b$ digits} i.e., elements
of the set $B_b$, and where ``$.$'' represents the {\em (base-$b$)
  radix point}.

The numeral (\ref{eq:real-numeral}) represents the (real)
number\footnote{The notation ``$\mbox{\sc value}(x)$'' in
  (\ref{eq:real-numeral-number}) denotes the {\em numerical value} of
  the numeral $x$.}
\begin{equation}
...
\sum_{i=0}^n \alpha_i \cdot b^i
\]
and the base-$b$ number represented by the numeral's fractional part
...
\subsection{Recognizing Integers and Rationals via Their Numerals}
