...
\noindent
Perform the following two-step iteration until the {\sc
  current-residue} is $1$.

\medskip

\begin{tabular}{|ll}
{\em Initialize:} &
 Set the {\sc current-residue} to $n$ \\
{\em Iterate:} &
Divide the {\sc current-residue} by $b$ \\
 & The {\em remainder} upon
division is the next lowest-order base-$b$ digit of $n$.
\end{tabular}

\medskip

\noindent
We illustrate this procedure by producing the normal-form base-$2$
(binary) numeral for $143$.
...
normal-form numeral,
\[  \alpha_2 \alpha_1 \alpha_0 \ . \ \beta_0 \beta_1 
...
so that
\begin{eqnarray*}
A & = & \alpha_2 \alpha_1 \alpha_0 \ . \ \beta_0 \beta_1 \\
B & = & \gamma_0 \gamma_1 \gamma_2
\end{eqnarray*}
...
\nonumber
r & = &
\mbox{\sc value}(\alpha_2 \alpha_1 \alpha_0 \ . \ \beta_0 \beta_1
\gamma_0 \gamma_1 \gamma_2
\gamma_0 \gamma_1 \gamma_2
\cdots
\gamma_0 \gamma_1 \gamma_2
...
\nonumber
  & = &
\mbox{\sc value}(\alpha_2 \alpha_1 \alpha_0)
 \ + \ \mbox{\sc value}(\beta_0 \beta_1) \cdot b^{-2}
 \ + \
\mbox{\sc value}(\gamma_0 \gamma_1 \gamma_2) \cdot b^{-5} \\
\label{eq:sum-in-numeral}
  &  &
 \ + \
\mbox{\sc value}(\gamma_0 \gamma_1 \gamma_2) \cdot b^{-8}
 \ + \
...
\mbox{\sc value}(\alpha_2 \alpha_1 \alpha_0)
 \ + \ \mbox{\sc value}(\beta_0 \beta_1) \cdot b^{-2}
 \ + \
\mbox{\sc value}(\gamma_0 \gamma_1 \gamma_2) \cdot \sum_{i=1}^\infty b^{-2-3i}
\end{eqnarray}

We shall learn in Section~\ref{sec:geometric-sums} that infinite
summations such as the summation
\[ \sum_{i=1}^\infty b^{-2-3i} \]
in (\ref{eq:sum-in-numeral}) {\em converge}, meaning that {\em they
...
In other words, the number $r$ is rational.

\bigskip

{\small\sf Part 2: the `` only if'' clause.}
Say next that the real number $r$ is rational---specifically, that $r
= p/q$ \qed
\end{proof}
