...
We are poised to find the contradiction to our argument thus
far---hence, to the assumption that $|\R_{(0,1)}| \leq |\N|$.
Specifically, we construct an infinite decimal numeral
\[ \Psi \ = \ \psi_0 \ \psi_1 \ \psi_2 \ \psi_3 \ \psi_4 \cdots, \]
that {\em does not} appear in $\Delta$, even though $\mbox{\sc
  num}_{10}(\Psi) \in \R_{(0,1)}$.
