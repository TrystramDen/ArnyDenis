...
\subsection{A Fun Result: A ``Trick'' for Squaring Some Integers}
...
form of ``single-variabled''; \index{polynomial!univariate} when
$k=2$, the polynomial is said to be {\it bivariate} (a Latinate form
of ``two-variabled''.  Much of our interest will be in univariate
polynomials; see Section~\ref{sec:univariate-polynomials}.

A problem of particular interest when discussing a polynomial $P(x_1,
x_2, \ldots, x_k)$ is to discover the vectors of values of the
variables $\{x_i\}$ that cause $P$ to {\em vanish}, i.e., evaluate to
$0$.  Each such vector, $\langle r_1, r_2, \ldots, r_k)$ is called a
{\it root} \index{polynomial!root} of $P$.  As one simple example, the
...
applications and its theoretical implications.  The next two
subsections are devoted to exploring significant aspects of both a
theoretical setting of the problem (in
Section~\ref{sec:Hilberts-Tenth}) and a practical setting (in
...
mathematical details necessary to fully describe the analyses that led
...
\begin{proof}
Focus on the generic $a$-letter alphabet $\a \ = \ \{\alpha_1,
\alpha_2, \ldots, \alpha_a\}$.  We argue by induction on $k$.

\noindent
{\it Bases.}
The induction we develop can start either at $k=0$ or at $k=1$.  Some
people are willing to recognize the {\it null word}, \index{word!the
  null word} which contains no letters, hence has length $0$; others
insist that the status ``word'' can be enjoyed only by non-null
strings.  This is purely a matter of taste.

At any rate, everyone agrees that, if you are willing to countenance
the null word over the alphabet, there is only $a^0 = 1$ such word;
this is the case $k=0$ of the proposition.  And, everyone agrees that,
if you insist on non-null words, then there are $a^1 = a$ such words,
one for each symbol in $\a$; this is the case $k=1$ of the
proposition.

\noindent
{\it The inductive hypothesis.}
Say that for all word-lengths $k$ up through $n$, there are $a^k$
distinct words of length $k$ over $\a$.

\noindent
{\it Extending the induction.}
Take each length-$n$ word $w$ over $\a$, and append each of $\a$'s $a$
symbols to $w$.  One thereby creates $a$, obviously distinct, words
from $w$, namely, $w \alpha_1$, $w \alpha_2$, \ldots, $w \alpha_a$.
We have thus created $a^{n+1}$ distinct length-$(n+1)$ words over $\a$
from $\a$'s $a^n$ distinct length-$n$ words.
\qed
\end{proof}
