...
\ignore{\Denis I put here a figure for proving the result, may be it
  is not at the right place, feel free to put it elsewhere... I developed later a graphical proof of the theorem...}
...
\ignore{\Denis I added a figure here, feel free to remove it if you believe it is not appropriate.
...
\subsection{Divisible or not divisible?}
\label{sec:}

{\Denis rephrase the first sentences.}
...
Equivalently, we say that $n$ {\it is divisible
  by}\index{number!integer!divisibility} $p$.  
Sometimes two integers are perfectly divisible, sometimes divisibility goes wrong.
Let us investigate this in the next two sections.

\subsubsection{Perfect divisibility}
\label{sec:divisibilityAndGCD}
...
%Let remark that according to this definition, every number divides $0$.
%
%The following properties are straightforward by applying directly the definitions:
%
%\begin{enumerate}
%\item If $a \mid b$  then $a \mid bc$ $\forall$ integers $c$
%\item If $a \mid b$ and $b \mid c$ then $a \mid c$
%\item If $a \mid b$ and $a \mid b+c$ then $a \mid c$ 
%\item $\forall c \neq 0$, $a \mid b$ {\it iif} $ac \mid bc$ 
%\end{enumerate}

Property:

If $a \mid b$ and $a \mid c$ then $a \mid sb+tc$ $\forall$ integers $s$ and $t$

Proof: there exist $k_1$ and $k_2$ such that $a.k_1 = b$ and $a.k_2 = c$,
which implies $a(k_1.s+k_2.t )= sb+tc$ for any $s$ and $t$. 
...
$GCD(a,b)$ is equal to the smallest positive linear combination of $a$ and $b$\footnote{This result is also known as Bezout identity.}.
\bigskip
...
\begin{quote}
We usually use the shorthand assertion, ``$p$ is a prime'' (or even simpler ``$p$ is prime'') instead of
...
{\Denis if it is not too hard, we should put it.}
...
{\Denis Yes, develop if not too complicated.}    
...
\end{itemize}
Thus, the set $\Z$ is a {\em proper subset} of the set $\Q$:
symbolically, $\Z \subset \Q$.
...
groundbreaking study of the relative ``sizes'' of infinite sets
\cite{Cantor74,Cantor78}.  We adapt enough of his formulation to
suggest how such issues can be dealt with mathematically.
...
approach fails us when we can infinitely much fruit!
...
\addcontentsline{toc}{subsubsection}{-- A fun result: There are as many
...
$\Q^+$ of {\em non-negative} rationals.  We specify $f_1$ as follows.
\[ f_1(p/q) \ = \ \left\{
\begin{array}{cl}
...
interesting alternatives to the preceding proof.  Being {\em
  bijections} between $\Z$ and $\Z \times \Z$, these mappings can be
adapted to prove Proposition~\ref{thm:|Q|=|Z|} {\em in a single
  step}.

\medskip

\paragraph{A digression: The Schr\"{o}der-Bernstein Theorem}
Although sets and their cardinalities are not the major focus of this
chapter, it is worth a short digression to expand on the preceding
remark about pairing functions.  It is not a coincidence that there
exists both \\
\hspace*{.35in}{\em a bijection between $\Z$ and $\Z \times \Z$} \\
and \\
\hspace*{.35in}{\em an injection from $\Z$ to $\Z \times \Z$ and an
  injection from $\Z \times \Z$ to $\Z$.} \\
The celebrated theorem of Schr\"{o}der and Bernstein
\index{The Schr\"{o}der-Bernstein Theorem} {\Arny Find reference}
...
{\Arny Should we prove the Schroeder-Bernstein theorem?  If we decide
  to include that,then it shuould likely go into the chapter on SETS.}
...
Keeping the same contradictory argument, it is possible to draw a geometrical proof. 
Fig.~\ref{fig:irrationalitySQRT2} gives a pictorial view of the basic pythagorian equation $1+1 = \sqrt{2}^2$.
Assume such square is the smallest one. 
Then, let put both little squares into the bigger one as shown in Fig.~\ref{fig:irrationality2}.
The overlapping surface (in dark grey) is a square which is equal to the two smaller extreme white sqaures (should we devlop here the symmetry argument for that?).
Thus, we build another smaller square equation, which is a contradiction.
...
\subsection{Comparing $\Q$ and $\R$ via cardinalities: There are ``more'' real numbers}
...
There is no injection $f: \R \rightarrow \N$.
\end{prop}

\begin{proof}
By the Schr\"{o}der--Bernstein theorem (Theorem~\ref{thm.S-B}), there
therefore exists a {\em bijection}
\[ 
h: \b \ \stackrel{1\mbox{\rm -}1, \mbox{\scriptsize\rm
    onto}}{\longrightarrow} \N.  
\] 

It is not hard to view the bijection $h$ as producing an
``infinite-by-infinite'' binary matrix $\Delta$, whose $k$th row is
the infinite binary string $h^{-1}(k)$.  Let's visualize $\Delta$:
\[ \Delta \ = \
\begin{array}{cccccc}
