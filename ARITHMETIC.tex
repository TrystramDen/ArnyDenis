...
mathematics enters every known proof in some fundamental
...
\item
**HERE
\noindent {\it Milestones in the quest for roots of polynomials}
...
There are currently proofs of Theorem~\ref{thm:fund-thm-algebra} that
are {\em constructive} in the following sense.  Given a degree-$d$
polynomial $P(x)$, the proof determines a disk $D$ in
(two-dimensional) space such all $d$ roots of $P(x)$ reside in disk
$D$.  How might such a disk emerge from a mathematical argument?  Here
...
\subsection{$\oplus$ Integer Roots of Multivariate Polynomials: Hilbert's Tenth Problem}
