...
\subsubsection{Solving general quadratic and cubic polynomials}
\label{sec:solve-quadratic}
...
there do exist computationally feasible algorithms for finding
roots.  For polynomials of {\em very} low degree---such as degrees $2$
and $3$, there actually exist {\em formulas} that specify the $2$ or
$3$ roots, respectively.  This section is devoted to deriving the
{\em quadratic} formula, i.e., the one that works for degree-$2$---{\it
  quadratic}---polynomials. \index{polynomials!quadratic}


\paragraph{\small\sf A. Solving general quadratic polynomials}

Consider the generic quadratic polynomial
...
We derive a formula that {\it solves} any quadratic polynomial, i.e.,
discovers the two roots promised by
Theorem~\ref{thm:fund-thm-algebra}, and we illustrate the derivation
with our generic polynomial $P(x)$.  \index{polynomial!solving a
  quadratic polynomial} While the formula and its derivation are
specialized to the structure of quadratic polynomials, the interested
reader will observe aspects of the derivation that can be extrapolated
to polynomials of higher degree.  The so-called {\em quadratic
  formula} that we derive is announced in the following proposition.
