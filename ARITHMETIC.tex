...
As a historical note, Proposition~\ref{thm:log-add-multiply} provides
the mathematical underpinning of the analogue computing device known
as a {\it slide rule}. \index{slide rule} Without going into details,
a slide rule is built from two straight sticks that are both marked
off into a logarithmic scale and that are attached in a way that
allows each to slide along the other (whence the name of the device);
see Fig.~\ref{fig:slide-rule}.
\begin{figure}[htb]
\[
\begin{array}{ll}
A \hspace*{.35in} &
...
In short, the device translates linear motion (the sliding of the
sticks relative to one another) into numerical multiplication.
