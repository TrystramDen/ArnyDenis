...
{\it diagonalization construction} \index{diagonalization}, this will
provide the most sophisticated proof by contradiction in our text.
The reader might want to review the reasoning underlying such
argumentation in Chapter~\ref{sec:Contradiction}, as a ``warm-up.''

\paragraph{\small\sf A. Refining our goal}

Using the definition of countability, our proof will begin with the
assumption that $|\R| \leq |\N|$ and will derive a contradiction from
that assumption.  In fact, we shall use other knowledge about the sets
$\N$ and $\R$ to recast our goal into a form that will be easier to
manage mathematically.  We begin with the following easy lemma.

\begin{lemma}
\label{lem:N-leq-R}
$|\N| \leq |\R|$.  Stated otherwise, there exists an injection $f: \N
  \rightarrow \R$.
\end{lemma}

\noindent {\it Verification}.
%
For each nonnegative integer $n \in \N$, let $\nu(n)$ denote the
base-$b$ numeral for $n$.  The mapping that associates each $n \in \N$
with the infinite string
\[ \nu(n) \ . \ 00 \cdots \]
is an injection from $\N$ into $\R$.  Given a real numeral that has
only $0$s to the right of the radix point, one produces the integer
$n$ by stripping the numerals of its radix point and all $0$s to the
right of the point, and then ``decoding'' the remaining string of
digits as the base-$b$ numeral of a nonnegative integer.  \qed.

\medskip

Lemma~\ref{lem:N-leq-R} converts our initial assumption that $|\R|
\leq |\N|$ to the stronger assumption
\[ |\R| \ = \ |\N| \]

**HERE

NEXT: Look only $0 < x < 1$ 
