...
(\ref{eq:generic-quadratic-4}) enables us to solve this equation for
$z^3$, yielding the following two solutions (abbreviated via the
shorthand operator $\pm$, for ``plus or minus''). \index{$\pm$: plus
  or minus}
\begin{equation}
...
assures us that the equation has three roots.  In order to compute
these roots, we must invoke a truly remarkable result that is known as
\index{Euler's formula} {\it Euler's formula}, in honor of its
discoverer, the much-traveled $18$th-century mathematician Leonhard
Euler. \index{Euler, Leonhard} This result/formula exposes a fundamental
relationship among:
\begin{itemize}
\item
...
these solutions for $z$, then we encounter cancellations in our
calculations, which leave us with only {\em three} solutions for $y$.
(We knew from the Fundamental Theorem of Algebra,
Theorem~\ref{thm:fund-thm-algebra}, that something like this had to
happen, because cubic polynolmials have only three roots.)
\begin{eqnarray}
