...
Part (a) is clear via the following injection from $\N$ into $\N \times \N$.
\[ (\forall n \in \N) \ [ f(n) \ = \ \langle n,n \rangle]. \]

\medskip

\noindent (b) We define an injection from $\N \times \N$ into $\N$
that is inspired by the Fundamental Theorem of Arithmetic
(Theorem~\ref{thm:Fund-Thm-Arith}).  The Teorem assures us that the
function
