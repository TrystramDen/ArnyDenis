...
\noindent
To establish part (b) We define an injection from $\N \times \N$ into
$\N$ that is inspired by the Fundamental Theorem of Arithmetic
(Theorem~\ref{thm:Fund-Thm-Arith}).  The Theorem assures us that the
function
...
that \\
\hspace*{.35in}{\em the set $\N$ of integers has the same cardinality
  as the set $\Q$ of rational numbers.}  \\
