...
{\sc Lesson:} Exercise in applying the Fundamental Theorem of Arithmetic
...
Let $m$, $n$, and $p$ be three integers that exceed $1$ (i.e., $m>1$, $n>1$, and $p >1$).  If $m \cdot n = m \cdot p$, then $n = p$.
\end{prop}

A particularly charming proof of Proposition~\ref{thm:NUM2-EX-CXL} builds upon the
Fundamental Theorem of Arithmetic (Theorem~\ref{thm:Fund-Thm-Arith}).

\item
{\bf Prime numbers and divisibility}

{\sc Lesson:} Reinforce understanding of basic concepts

\smallskip

{\em Prove that every integer $n>1$ is divisible by at least one prime number.}
...
\begin{prop}
When you do successive Euclidian divisions of the integer $n$ by the number-base $b$, the sequence of {\em remainders} that you generate are the successive digits of the base-$b$ numeral for $n$, from the lowest-order digit to the highest.
\end{prop}
...
\textit{Hint:}
Use the pigeonhole principle to classify pairs $(p, q > p)$ for divisibility.

\item
{\bf When is integer $n$ divisible by $9$?}

{\sc Lesson:} Enhance awareness of positional numerals's grounding in geometric summations
