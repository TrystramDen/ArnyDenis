...
\begin{proof} [Euler's 19th century proof]
...
\[ \frac{1}{1-1/p} \ = \ 
%\sum_{k=0}^{\infty} \left( \frac{1}{p} \right)^k \ = \ 
...
\end{equation}
We can now expand the product of sums on the righthand side Eq.~(\ref{eq:prod-widehat-P-2}).  After applying all of the relevant laws of arithmetic---the commutativity and associativity of both addition and multiplication, and the distributivity of multiplication over addition---we can rewrite the product of sums as the following summation
\begin{equation}
...
\item
a $k$-element sequence of nonnegative integers $a_1, \ a_2, \ \ldots,  \ a_k$
\end{itemize}
