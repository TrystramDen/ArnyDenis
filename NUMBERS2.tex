...
It is appropriate, therefore, that a special name has been associated with bijections between $\N^+ \times \N^+$ and $\N^+$.  These special bijections are called {\it pairing functions}.  \index{pairing functions!encodings}
...
We now describe our first explicit pairing function---the {\em diagonal} pairing function $\d(x,y)$.  This is probably one of the oldest known pairing functions, given that it was known as early as 1821.  We provide in Chapter~\ref{Appendix:building-pairing-functions} both a rather general methodology for crafting pairing functions and two explicit siblings of the diagonal pairing function $\d$, which are appealing as exemplars of the general construction methodology.   One of these siblings provides a storage mapping for extendible matrices that can be useful in certain algorithmic domains; the other answers a fundamental question about inherent limitations on the ``compactness" of pairing functions.
