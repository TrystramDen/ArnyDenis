...
\begin{enumerate}
\item
partitioning $\N^+ \times \N^+$ into ``diagonal shells''.  For each index $k \in \N^+$, shell \#$k$ is the set
\[
...
\item
The development in \cite{LewR78b} excludes large families of higher-degree polynomials from being pairing functions; e.g., a {\em super-quadratic} polynomial whose coefficients are all positive
cannot be a pairing function (because it cannot be surjective).
\end{enumerate}
\end{minipage}
}
