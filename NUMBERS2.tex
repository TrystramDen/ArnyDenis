...
We present below an alternative proof. 
It involves several pieces of Mathematics and thus, this is a wonderful example of connected and related techniques.
This is due to the Leonard Euler in the 19th century.

The idea is to prove that a given expression of products of primes is not bounded.

Let $\widehat{P}$ denote the set of primes and assume it is infinite.
...
\[
\frac{p}{p-1} = \frac{1}{1-1/p}\], it corresponds to the sum of a geometric series: 

\[
\frac{1}{1-1/p} \ = \ \sum_{k=0}^{\infty} \left( \frac{1}{p} \right)^k \ = \ \sum_{k=0}^{\infty} \frac{1}{p^k}
\]

\begin{eqnarray}
\prod_{p \in \widehat{P}} \frac{p}{p-1} & \ = & \ \prod_{p \in \widehat{P}} \left( \sum_{k=0}^{\infty} \frac{1}{p^k} \right) \\
& \ = & \ \sum_{k=0}^{\infty} \left( \prod_{p \in \widehat{P}} \frac{1}{p^k} \right)\\
& \ = & \ \sum_{k=0}^{\infty} \frac{1}{\prod_{p \in \widehat{P}} p^k} 
\end{eqnarray}

The fundamental theorem of arithmetic~\ref{thm:Fund-Thm-Arith} tells us that any integer is fully characterized by its (unique) decomposition over a product of powers of primes. In our context, this theorem means that the sum of all products of primes to the power $k$ scans the whole set of integers, thus
\[
\prod_{p \in \widehat{P}} \frac{1}{1-1/p} \ = \ \sum_{n=1}^{\infty} \frac{1}{n}
\]
...
\] 
is infinite.
By a straightforward contradiction, the only way that it is possible is that there are an infinity of primes.
...
\subsection{$\N \times \N$ Is \underline{Not} ``Larger Than" $\N$} 
...
\index{The Schr\"{o}der-Bernstein Theorem} \index{$\{0,1\}^\star$: the set of all binary strings}
