...
\noindent \fbox{
\begin{minipage}{0.95\textwidth}
The notion of \textit{invariant} is an extremely important conceptual tool when crafting proofs.
The underlying idea is to find an expression $\phi(\cdot)$ whose value is preserved (i.e., ``holds
...
edges of the graph $\g$ that we began with.  Specifically, the edge-removing
Phase 1 removes $e-n$ edges, while the leaf-removing Phase 2 removes $n$ vertices.
\item
...
\end{itemize}
We conclude, therefore, that at the start of the process, $\phi(n,e,f)$ had the value $2$.
...
\noindent \fbox{
\begin{minipage}{0.95\textwidth}
We have focused almost entirely on the problems of vertex-coloring planar and
outerplanar graphs because these problems can be studied within a mathematical setting
(which is our focus) rather than an algorithmic setting (which is not our focus).  In closing,
though, we remark that there are many interesting algorithmic problems relating to 
vertex-coloring arbitrary graphs.  A central such result dates to the earliest days of the 
...
Lending greater interest to this result is the existence of ``greedy'' vertex-coloring
algorithms that give quite good results.  {\Arny reference?}
\end{minipage}
}
...
As we embark on our adventure, we note the amusing fact that
...
We now develop simple, elegant characterizations of graphs that admit Eulerian cycles and paths.
These characterization are validated via simple and efficient algorithms for determining
whether a given graph admits such a path or such a cycle.  In the next two propositions, we
provide the characterizations for both cycles and paths, in both undirected and directed graphs.
...
a vertex of degree $1$.  Please look at that proof and at this proof to see whether a single
more general (abstract?) argument works for both.}
