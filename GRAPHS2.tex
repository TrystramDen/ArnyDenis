...
\noindent \fbox{ \begin{minipage}{0.96\textwidth}
The problem of discovering an Eulerian cycle in a graph $\g$ is (algorithmically) equivalent to the problem of drawing $\g$ ``cyclically"

\hspace*{.15in}i.e., starting and ending with the same vertex

without ever lifting one's pencil. 
\end{minipage}
}
...
These characterization are validated via simple and efficient algorithms that determine whether a given graph admits such a cycle or such a path.  To avoid ``special cases" that disrupt the flow of a proof, we restrict attention to graphs and digraphs that are {\em nontrivial} in the sense that
\begin{itemize}
\item
