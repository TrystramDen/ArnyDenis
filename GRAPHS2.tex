...
\noindent
A graph is {\it planar} precisely if it can be drawn {\em without any crossing edges}.  A graph $\g$ is {\it outerplanar} precisely if it can be drawn by {\em placing its vertices along a circle in such a way that its edges can be drawn as noncrossing chords of the circle}.  The latter condition is
...
In all remaining cases, vertex $v$ has precisely five neighbors---or else, we would have invoked Case 1 to color $\g$ with five colors.  We shall visualize vertex $v$ as in 
...
\end{figure}
In the figure, 
we use integer-label $i$ to denote, ambiguously, vertex $v_i$ and its assigned color $c_i$.  The question mark ``?'' that labels vertex $v$ indicates that we do not yet know what color to assign to $v$. 
...
Let us recolor the vertices of $\g_1$ so that vertex $v_1$ is now colored {\it green}.  (One need only switch the colors {\it red} and {\it green} in the existing coloring of $\g_1$ to achieve this.)  It is always possible to do this in a way that does not affect the valid coloring of $\g_2$ because $\g_1$ and $\g_2$ are mutually disjoint.
...
The two graphs in NEXT FIGURE depict schematically how we have dealt with Case 3 above.
\begin{figure}[hbt]
\begin{center}
...
\noindent {\bf (c)}.2.
When $mn$ is even, the Hamiltonicity of $\widetilde{\m}_{m,n}$ follows from the fact that $\m_{m,n}$ is a spanning subgraph of $\widetilde{\m}_{m,n}$.  (Think about it!)  When $mn$ is odd, one needs just traverse $\widetilde{\m}_{m,n}$'s vertices row by row, going to the cyclically next vertex after completing each row (see Fig.~\ref{fig:HamiltonTorus}).  Details are left to the reader.
\begin{figure}[hbt]
\begin{center}
