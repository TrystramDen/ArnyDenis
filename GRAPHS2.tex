...
Say that, under the $5$-coloring of $\g$ that we are focusing on, $v_1$ is colored {\it black} and $v_2$ is colored {\it grey}.  Figure~\ref{fig:5colorsCase3} illustrates the scenario.
\begin{figure}[hbt]
\begin{center}
...
We recolor the vertices of $\g_1$ so that vertex $v_1$ is now colored {\it grey}.  (One need only switch the colors {\it black} and {\it grey} in the existing coloring of $\g_1$ to achieve this.)  It is always possible to do this in a way that does not affect the valid coloring of $\g_2$ because $\g_1$ and $\g_2$ are mutually disjoint.
...
Now take $\g$ and focus only on the vertices that are colored $c_1$ or $c_3$ (as are $v_1$ and $v_3$, respectively) and on the vertices that are colored $c_2$ or $c_4$ (as are $v_2$ and $v_4$, respectively).  One sees from Fig.~\ref{fig:5colorsCase2} that:
\begin{itemize}
\item
...
These two paths, if they existed, would cross one another---which is forbidden because $\g$ is a {\em planar} graph,  See Fig.~\ref{fig:5colorsCase2}.
