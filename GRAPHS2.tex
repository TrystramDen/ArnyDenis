...
\begin{figure}[hbt]
\begin{center}
       \includegraphics[scale=0.4]{FiguresGraph/EulerienVodd}
       \caption{$\g$ with its $4$ odd-degree vertices highlighted.}
              \label{fig:EulerianVodd}
\end{center}
\end{figure}

{\Arny XXX We need labeling on the vertices and edges so that we can refer to them}

\medskip

We sketch a process that adds a minimally many edges to $\g$, which convert converts $\g$ to a  multigraph which admits a ``distorted" Euler tour. 
\begin{enumerate}
\item
...
Computing the lengths $\ell(u,v)$ can be accomplished efficiently, i.e., in low-degree polynomial time; see \cite{CLRS}.
...
Fig.~\ref{fig:Eulerianperfectmatching} illustrates the results of this process on our sample graph $\g$.
\begin{figure}[hbt]
\begin{center}
...
Our augmented multigraph $\g^+$ now has all even vertex-degrees.  We can, therefore, adapt the Euler-tour algorithm from . in order to produce a tour of $\g^+$ that crosses each multi-edge precisely once.
