...
A {\it multigraph} is a graph-like structure that allows a {\em multiset} of edges, rather than a set.  In detail, a multigraph $\h$ has a set $N_{\fh}$ of vertices and a {\em multiset} of edges, each being a $2$-element set of vertices.  Viewed differently, a pair of vertices in a multigraph can be connected by more than one edge.  {\Arny a picture?}

Proposition~\ref{thm:eulerian-cycle} completely solves the  ``pure'' version of the Euler-tour problem for {\em graphs}.  We learn from that result that a graph $\g$ admits a tour that crosses each edge precisely once if, and only if, every vertex $v \in N_{\fg}$ has even degree.

From an applied vantage point, if one is confronting a situation that does not satisfy the criteria that allow an efficient solution to the ``pure'' version of a practically important problem, then one is often willing to ``bend the rules"---if such distortion will give one access to an efficient solution to a version of the problem that is not ``not quite pure''.  When some manner of uniformity of vertex-degrees---such as parity---is the inaccessible feature of the ``pure'' version of a problem, then multigraphs can sometimes give one access to ``impurities" that can be overcome by clever algorithmics.  

This section is devoted to such a ``distorted" solution to the Euler-tour problem.  The problem we study exposes a form of the Euler-tour problem that admits optimal tours even for graphs that have some odd-degree vertices.  The ``distortion" that enables such solutions is the license to replace the edge-set of a graph $\g$ to an edge-{\em multiset} that preserves the neighbor relation of $\g$.  In other words, as we add the new edges that convert $\g$'s edge-set to an edge-{\em multiset}, we never create new adjacencies, i.e., we never add an edge that connects vertices that were not connected in $\g$.

 \index{Route Inspection Problem} \index{Chinese Postman Problem} \index{Kwan Mei-Ko}

As we convert $\g$ from a graph to a multgraph, we convert the Euler-tour problem to a problem that is known either as the {\it Route Inspection Problem} or the {\it Chinese Postman Problem}---in honor of the Chinese mathematician Kwan Mei-Ko who invented the problem in \cite{Kwan60}.  This weighted version of the Euler-tour problem challenges one to {\em find the {\em smallest} augmentation of $\g$---measured in terms of the number of added multi-edges---under which the multigraph $\g$ admits a tour that crosses each edge precisely once.}
