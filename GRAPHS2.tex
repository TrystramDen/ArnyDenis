...
\noindent {\em A large outerplanar graph.}
...
Because $\g$ has $n>3$ vertices, and because all of its vertices lie on circle, there must be pairs of vertices of $\g$ that {\em are not adjacent} as one walks along the circle (following ring edges).  Let $u$ and $v$ be a pair of nonadjacent vertices whose distance---measured in terms of the number of ring edges that one must traverse to reach one vertex from the other---is {\em minimal} among pairs of nonadjacent vertices.  We distinguish two cases.
\begin{itemize}
\item
If the distance between $u$ and $v$ is {\em exactly} $2$, then the unique vertex that lies between $u$ and $v$ along the circle would have degree $2$.
\medskip\item
If the distance between $u$ and $v$ {\em exceeds} $2$, then there must be {\em at least two} vertices that lie between $u$ and $v$ along the circle.  But in this case, the path between $u$ and $v$ contains two nonadjacent vertices that are closer to one another than $u$ and $v$ are.  This, however, contradicts our choice of $u$ and $v$ as a pair of {\em closest} nonadjacent vertices.
\end{itemize}
We conclude that $\g$ must have a vertex of degree $2$.  \qed
\end{proof}
