...
such summations is valuable to consider here.

\medskip

{\it (i)}
We begin by contemplating the following vexing infinite summation.
\[ S \ = \ 1 \ - \ 1 \ + \ 1 \ - \ 1 \ + 1 \ - \ 1 \ + \cdots \]
There are many conflicting, but well-reasoned, answers to questions such as the following.

\medskip

\noindent
{\bf Questions}:  (1) {\it Does summation $S$ have a finite sum?}  (2) If so: {\it Is this sum positive or negative?}

\smallskip

\noindent
Here are four plausible answers to these questions; the final one, in particular, merits strong contemplation.
\begin{enumerate}
\item
YES: $S \ = \ 0$
...
This response is justified by the following association of terms in summation $A$.
\begin{figure}[h]
\begin{center}
...
This response is justified by the fact that the problem statement does not specify how to associate the terms of $S$.  As we see, ``mischievously" playing with parentheses enables us to arrive at many ``plausible'' sums for summation $S$.
\end{enumerate}

\smallskip

{\it (ii)}
The lessons that one can learn from summation $S$ are not exhausted by playing with parentheses.  Consider two additional infinite summations.
\begin{eqnarray*}
S' & = &  2 \ - \ 3 \ + \ 4 \ - \ 5 \ + \ 6 \ - \cdots + \cdots \\
S" & = & 1 \ + \ 2 \ + \ 3 \ + \ 4 \ + \ 5 \ + \cdots
\end{eqnarray*}
...
The preceding examples illustrate how summations having infinitely many terms behave differently from those having finitely many terms.  {\em Infinity is not a number!}
