...
{\Arny The curve for $f$ should be labeled, and all fonts should be italic}
\begin{figure}[htb]
\begin{center}
       \includegraphics[scale=0.4]{FiguresArithmetic/NotationAsymptotic}
\caption{Plots of three functions: $f$ (solid curve), $g$ (dotted
  curve that starts out above $f$, and $h$ (dotted curve that starts
  out below $f$).  Assume that the three iluustrated trajectories
  continue in the indicated relationships for all $x > x2$---i.e.,
  with the plot for $f$ strictly below that for $g$ and strictly above
  that for $h$.  With this assumption, the figure illustrates the
  relations: $g = O(f)$ and $h = \Omega(f)$.}
...
can be made precise.  One instance of the former class of summation is:
\[ 1 \ + \ {1 \over 2} \ + \ {1 \over 2^2} \ + \cdots + \ {1 \over
  2^k} \ + \cdots \ \ = \ \ 2;
\]
one instance of the latter class of summation is:
\[ 1 \ + \ {1 \over 2} \ + \ {1 \over 3} \ + \cdots + \ {1 \over k} \ + \cdots \]
whose sum increases forever as more terms are added in.
\end{enumerate}
