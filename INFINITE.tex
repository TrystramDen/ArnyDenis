...
Dealing with the Very Large and the Infinite}
\label{ch:infinity}
...
Of course, we have already dealt successfully with a number of quite
...
\paragraph{A.  G\"{o}del's paradox: Self-referentiality in language}
...
\paragraph{B.  Russell's paradox: There is no ``anti-universal'' set}
...
each sentence within a language $L$ a {\it type}\index{typed utterances in languaes}
...
and G\"{o}del's paradox. Bu, it does so at the cost of adding an
