...
{\bf 1.} {\it Variables and unknowns}. \\
...
{\bf 2.} {\it When ``equals" is not ``equals"}. \\
The ``equals sign'' in the notation $f(x) = O(f_1(x))$ does {\em not} mean ``equals''.
...
The assertion $f(x) = \Omega(f_2(x))$ says, intuitively, that the function $f$ {\em grows at least as fast as} function $f_2$.  It is, thus, the asymptotic analogue of ``greater than or equal to''.
...
In his celebrated {\it Paradox of Achilles and the Tortoise}, Zeno of Elea  presented a problem whose solution had to await the seventeenth century.
